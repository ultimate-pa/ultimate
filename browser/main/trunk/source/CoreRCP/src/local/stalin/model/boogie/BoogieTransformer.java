package local.stalin.model.boogie;
...
import local.stalin.model.boogie.ast.IfStatement;
...
import local.stalin.model.boogie.ast.WhileStatement;
 ...
 * created.
 * 
 * As an example usage see local.stalin.boogie.preprocessor.FunctionInliner.
...
public abstract class BoogieTransformer {
	...
	protected Declaration processDeclaration(Declaration decl) {
		...
		if (decl instanceof TypeDeclaration) {
			...
			if (newAttrs != attrs || newSyntype != syntype)
				return new TypeDeclaration(tdecl.getFilename(), tdecl.getLineNr(),
			...
			if (newAttrs != attrs || newVarList != varList)
				return new ConstDeclaration(cdecl.getFilename(), cdecl.getLineNr(), 
			...
			if (newIns != ins || newOut != out
				...
				return  new FunctionDeclaration(fdecl.getFilename(), 
						fdecl.getLineNr(), 
			...
			if (ax != newAx || attrs != newAttrs)
				return new Axiom(decl.getFilename(), decl.getLineNr(), 
						newAttrs, newAx);
			...
			if (newAttrs != attrs || newBody != body || newSpecs != specs
				...
				return new Procedure(proc.getFilename(), proc.getLineNr(), 
			...
			if (attrs != newAttrs || vls != newVls)
				return new VariableDeclaration(
						decl.getFilename(), decl.getLineNr(), 
						newAttrs, newVls);
		}
		return decl;
	}
	...
	protected ASTType processType(ASTType type) {
		if (type instanceof ArrayType) {
			...
			if (newIndexTypes != indexTypes
				...
				return new ArrayType(arrtype.getBoogieType(), arrtype.getTypeParams(), newIndexTypes, newValueType);
			...
			if (newArgTypes != argTypes)
				return new NamedType(ntype.getBoogieType(), ntype.getName(), newArgTypes);
		}
		return type;
	}
	...
	protected VarList processVarList(VarList vl) {
		...
		if (newType != type || newWhere != where)
			return new VarList(vl.getIdentifiers(), newType, newWhere); 
		return vl;
	}
	...
	protected Body processBody(Body body) {
		...
		if (newLocals != locals || newStatements != statements)
			return new Body(newLocals, newStatements);
		return body;
	}
	...
	protected VariableDeclaration processLocalVariableDeclaration(VariableDeclaration local) {
		...
		if (vl != newVl || attrs != newAttrs) {
			return new VariableDeclaration(local.getFilename(), local.getLineNr(), 
						newAttrs, newVl);
		}
		return local;
	}
		...
		if (statement instanceof AssertStatement) {
			...
			if (expr != newExpr)
				return new AssertStatement(
						statement.getFilename(), statement.getLineNr(), newExpr);
			...
			if (lhs != newLhs || rhs != newRhs)
				return new AssignmentStatement(
						statement.getFilename(), statement.getLineNr(), 
						newLhs, newRhs);
			...
			if (args != newArgs)
				return new CallStatement(call.getFilename(), call.getLineNr(), 
						call.isForall(),
			...
			if (newCond != cond || newThens != thens || newElses != elses)
				return new IfStatement(ifstmt.getFilename(), ifstmt.getLineNr(),
			...
			if (newCond != cond || newInvs != invs || newBody != body)
				return new WhileStatement(
						whilestmt.getFilename(), whilestmt.getLineNr(),
		...
		for (int i = 0; i < newSpecs.length; i++) {
			...
			if (expr != newExpr) {
				...
				newSpecs[i] = new LoopInvariantSpecification(
						specs[i].getFilename(), specs[i].getLineNr(), 
	...
	protected LeftHandSide processLeftHandSide(LeftHandSide lhs) {
		if (lhs instanceof ArrayLHS) {
			...
			if (array != newArray || indices != newIndices)
				return new ArrayLHS(alhs.getType(), newArray, newIndices);
		}
		return lhs;
	}
		...
		if (spec instanceof EnsuresSpecification) {
			...
			if (expr != newExpr) {
				return new EnsuresSpecification(
						spec.getFilename(), spec.getLineNr(), 
						spec.isFree(), newExpr);
			}
	...
	protected Attribute processAttribute(Attribute attr) {
		if (attr instanceof Trigger) {
			...
			if (newExprs != exprs)
				return new Trigger(newExprs);
		} else if (attr instanceof NamedAttribute) {
			Expression[] exprs = ((NamedAttribute)attr).getValues();
			Expression[] newExprs = processExpressions(exprs);
			if (newExprs != exprs)
				return new NamedAttribute(((NamedAttribute)attr).getName(), newExprs);
		}
		return attr;
	}
	...
	protected Expression processExpression(Expression expr) {
		if (expr instanceof BinaryExpression) {
			...
			if (left != binexp.getLeft()
				...
				return new BinaryExpression(binexp.getType(),
			...
			if (subexpr != unexp.getExpr()) {
				return new UnaryExpression(unexp.getType(),
			...
			if (changed)
				return new ArrayAccessExpression(aaexpr.getType(), arr, newIndices);
			...
			if (changed)
				return new ArrayStoreExpression(aaexpr.getType(), arr, newIndices, value);
			...
			if (bv != bvaexpr.getBitvec())
				return new BitVectorAccessExpression(bvaexpr.getType(), bv, 
			...
			if (args != app.getArguments())
				return new FunctionApplication(app.getType(), name, args);
			...
			if (cond != ite.getCondition()
				...
				return new IfThenElseExpression(ite.getType(), cond, thenPart, elsePart);
			...
			if (subform != quant.getSubformula()
				|| attrs != newAttrs
				|| params != newParams)
				return new QuantifierExpression(quant.getType(), quant.isUniversal(), 
