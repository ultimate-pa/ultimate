/*
 * Project:	CoreRCP
 * Package:	local.stalin.logging
 * File:	StalinLoggers.java created on Feb 23, 2010 by Björn Buchhold
 *
 */
package local.stalin.logging;
...
import local.stalin.core.coreplugin.preferences.LoggingDetailsPreferenceWrapper;
import local.stalin.core.coreplugin.preferences.LoggingToolDetailsPreferenceWrapper;
 ...
 * 
 */
public class StalinLoggerFactory {

	/**
	 * the singleton
	 */
	private static StalinLoggerFactory instance;

	private IPreferenceStore preferenceStore;
	private List<String> presentLoggers;

	public static final String LOGGER_NAME_CORE = "stalin";
	...
	private void initializeLog4J() {
		...
		try {
			...
			PatternLayout layout = new PatternLayout(
					local.stalin.plugins.Constants.getLoggerPattern());
	...
	/**
	 * StalinLoggerFactory getInstance getter for the singleton. lazily creates
	 * the object
	 * 
	 * @return the singleton instance of the StalinLoggerFactory
	 */
	public static StalinLoggerFactory getInstance() {
		// lazily initialize the factory
		if (instance == null) {
			instance = new StalinLoggerFactory();
		}
		return instance;
	}
	...
	private Logger lookupLoggerInHierarchie(String id) {
		...
		// it is a controller
		if (id.equals(StalinServices.getInstance().getActiveControllerId())) {
