...
import local.stalin.plugins.output.jungvisualization.actions.JVContextMenu;
import local.stalin.plugins.output.jungvisualization.selection.JungSelection;
...
public class GraphListener implements MouseListener, GraphMousePlugin {
	...
	private Set<INode> selectedNodes = null;
	private Set<IEdge> selectedEdges = null;
	...
	public void mousePressed(MouseEvent e) {
		...
		while (elementIt.hasNext()){
			...
			if (currentElement instanceof INode){
				GraphHandler.getInstance().getVisualizationViewer().getPickedVertexState().pick((INode)currentElement, false);
			}
//			else
//			{
//				GraphHandler.getInstance().getVisualizationViewer().getPickedEdgeState().pick((IEdge)currentElement, false);
//			}
...
//			}
//			else
			if (currentElement instanceof IEdge)
			{
				GraphHandler.getInstance().getVisualizationViewer().getPickedEdgeState().pick((IEdge)currentElement, false);
			}
		...
		if (selectedNodes.size() > 0) {	
			...
			{
				Iterator<INode> nodeIt = selectedNodes.iterator();
				final INode currentNode = (INode) nodeIt.next();
		    ...
		    IEdge currentEdge = selectedEdges.iterator().next();
