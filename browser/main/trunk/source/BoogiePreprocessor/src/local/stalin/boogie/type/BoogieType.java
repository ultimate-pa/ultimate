...
public abstract class BoogieType implements IType {
	...
	private final static ArrayList<PlaceholderType> s_placeholders = 
		new ArrayList<PlaceholderType>();
	private final static ArrayList<PrimitiveType> s_bitvectypes = 
		new ArrayList<PrimitiveType>();
	private final static UnifyHash<BoogieType> s_globalTypes = 
		new UnifyHash<BoogieType>();
	public final static PrimitiveType intType = new PrimitiveType(PrimitiveType.INT);
	public final static PrimitiveType realType = new PrimitiveType(PrimitiveType.REAL);
	public final static PrimitiveType boolType = new PrimitiveType(PrimitiveType.BOOL);
	public final static PrimitiveType errorType = new PrimitiveType(PrimitiveType.ERROR);
	
	/**
	 * Create a bit vector type; reuses an old instance if it already exists.
	 * @param len the number of bits in this type
	...
	/**
	 * Create a new placeholder type; reuses an old instance if it already exists.
	 * A placeholder is a type variable and is represented by the number giving the
	 * position of the scope (de Bruijn's normal form). The placeholders are 
	 * numbered from inside to outside
	 * and in the same scope from left to right. For example the type
	 * <code>
	 * 	  &lt;x,y&gt;[&lt;z&gt;[x, y, z]y, x, y]x
	 * </code> 
	 * is represented as
	 ...
	 *    &lt;2&gt;[&lt;1&gt;[1,2,0]2, 0, 1]0
	 * </code>
	 * This weird numbering makes it easy to substitute the outermost placeholders.
	 * @param i the index of the typevar argument.
	...
	/**
	 * Create a new constructed type; reuses an old instance if it already exists.
	 * A constructed type is build from a TypeConstructor and some type arguments.
	 * @param constr the type constructor.
	 * @param params the arguments.  The length must be equal to constr.getParamCount().
	...
	/**
	 * Creates a new constructed type without parameters; reuses an old instance if it already exists.
	 * A constructed type is build from a TypeConstructor and some type arguments.
	 * @param constr the type constructor, constr.getParamCount() must be 0.
	 * @return The constructed type.
	 */
	public static ConstructedType createConstructedType(TypeConstructor constr) 
	{
	...
	/**
	 * Creates a new array type; reuses an old instance if it already exists.
	 * An array has a number of placeholders (type variables), some index types
	 * and a value type. 
	 * @param numPlaceholders number of declared placeholders
	 * @param indexTypes the types used in the array indices.  One entry for each
	 * dimension of the array.
	 * @param valueType the type of the values stored in the array.
	 * @return The array type.
	 */
	public static ArrayType createArrayType
		(int numPlaceholders, BoogieType[] indexTypes, BoogieType valueType)
	{
		...
		ArrayType newType = new ArrayType(numPlaceholders, indexTypes, valueType);
		s_globalTypes.put(hashcode, newType);
		return newType;
	}
	 ...
	 * @param depth the depth into the type to substitute.  Normally 0.
	 * @param substType The types to substitute.
	 * @return The substituted type
	 */
	protected abstract BoogieType substitutePlaceholders(int depth, BoogieType[] substType);

	/**
	 * Increment all placeholders in given type.  This is used to adapt the place holders
	 * in a substitution if it is applied deep inside another type.
	 * @param depth the depth into the type to substitute.  Normally 0.
	 * @param incDepth the depth by which to increment placeholders.
	...
	/**
	 * Returns true if this type is a synonym of the give object o.  This can only
	 * be the case if o is a BoogieType.
	 */
	...
	/**
	 * Unify the this type (which contains Placeholders) with another type (which doesn't)
	 * and compute a suitable substitution.
	 * @param depth the depth into the type to unify.  Normally 0.
	 * @param other the other type
	 * @param substitution the substitution array. should be initialized to null and 
	 ...
	 * @param minDepth the minimum index of the placeholder. Measured from the start of this type.
	 * @param maxDepth the maximum index of the placeholder.
	...
	/**
	 * Check whether this and the other type are unifiable by replacing Placeholders.
	 * This is symmetric, i.e. place holders are considered in both types.
	 * @param depth the depth
	 * @param other the other type
	 * @param subst the partial substitution
	 * @returns true if unification was successful, false on type mismatch.
	 */
