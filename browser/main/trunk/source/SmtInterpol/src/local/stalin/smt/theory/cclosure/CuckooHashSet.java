...
public class CuckooHashSet<E> extends AbstractSet<E> {
	...
	/**
	 * Default hash size.  Must be a power of two.
	 */
	...
	public CuckooHashSet(int size) {
		this.buckets = new Object[makepow2(2*size)];
	}
	public CuckooHashSet() {
		this.buckets = new Object[defaultSize];
	}
	...
	protected static int hashSbox(int hash) {
		return (((sbox[hash >>> 24]*3
				^ sbox[(hash >>> 16)&0xff]) * 3
				^ sbox[(hash >>> 8)&0xff]) * 3
				^ sbox[hash & 0xff]);
		/*hash ^= (hash >>> 3) ^ (hash >>> 7);
		return hash ^ (hash >>> 11) ^ (hash >>> 15);*/
	}
	
	protected final int hash1(int hash) {
		return hashSbox(hash) & (buckets.length-1);
	}
	
	protected final int hash2(int hash) {
		int nhash = (hash ^ (hash >>> 8) ^ (hash >>> 14)) & (buckets.length - 1);
		if (nhash == 0)
			nhash = hash & (buckets.length-1);
		return nhash == 0 ? 1 : nhash;
	}
	...
	private void resize() {
		...
		stashList = null;
		buckets = new Object[2*oldbuckets.length];
	...
	private final static int makepow2(int size) {
		int i;
		for (i = 4; i < size; i+=i)
			/*empty*/;
		return i;
