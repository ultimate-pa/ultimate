...
/**
 * Controller for the interactive console mode of Stalin 
 * which is started when calling Stalin with "--console" argument.
 * 
...
public class InteractiveConsoleController implements IController {
	...
	public static final String s_PLUGIN_ID = "local.stalin.InteractiveConsoleController";
	...
	/* (non-Javadoc)
	 * @see local.stalin.ep.interfaces.IController#selectModel(java.util.List)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see local.stalin.ep.interfaces.IController#selectParser(java.util.Collection)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see local.stalin.ep.interfaces.IController#selectTools(java.util.List)
	 */
	@Override
	...
	public int init(Object controlledCore) {
		...
		this.m_Tools = StalinServices.getInstance().getActiveTools();
		
		// this is subject to change during the run of a toolchain
		// so, don't forget to update it frequently!
		this.m_Models = StalinServices.getInstance().getAllModels();

		
		System.err.println("Welcome to Stalin's Interactive Console.");
	...
	private void interactiveConsole() throws IOException {
		...
		while (true) {
	    System.out.println("Enter your Stalin command : ");
	 
	    String str = br.readLine();
	    
	    CharStream cs = new ANTLRStringStream(str);
	    ConsoleLexer lexer = new ConsoleLexer(cs);
