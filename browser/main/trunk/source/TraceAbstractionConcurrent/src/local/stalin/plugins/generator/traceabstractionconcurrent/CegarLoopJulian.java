...
public class CegarLoopJulian extends CegarLoopSequential {
	
	PetriNetJulian<TransAnnot, IProgramState> m_Abstraction;
	Object m_ArtifactAutomaton;

	public CegarLoopJulian(RootNode rootNode, SmtManager smtManager,
	...
	protected void getInitialAbstraction() {
		...
		Cfg2NetJulian cFG2Automaton =
			new Cfg2NetJulian(m_RootNode,m_ContentFactory);
	...
	protected boolean isAbstractionCorrect() {
		new TestFileWriter<String,String>(m_Abstraction,"TheNet",m_PrintAutomataLabeling);
...
//			new TestFileWriter<TransAnnot, IProgramState>(dia, true);
			...
			m_Abstraction = m_Abstraction.differenceBlackAndWhite(m_Abstraction, (NestedWordAutomaton<TransAnnot, IProgramState>) dia);
//			new OperationsWithTests<TransAnnot, IProgramState>().concurrentProduct(m_Abstraction, nia);
		}
		if (m_BiggestAbstractionSize < m_Abstraction.getPlaces().size()){
			m_BiggestAbstractionSize = m_Abstraction.getPlaces().size();
	...
	public INode getArtifact() {
		if (m_Artifact == Artifact.ABSTRACTION ||
				...
				if (m_ArtifactAutomaton == null) {
					...
					return myTransformer.getStalinModelOfPetriNet((IPetriNet<TransAnnot, IProgramState>) m_ArtifactAutomaton);
				}
			}
