...
import local.stalin.model.IExplicitNode;
import local.stalin.model.INode;
import local.stalin.plugins.output.prefusevisualization.displays.CompleteHorizontalTreeView;
import local.stalin.plugins.output.prefusevisualization.displays.CompleteRadialGraphView;
import local.stalin.plugins.output.prefusevisualization.displays.CompleteVerticalTreeView;
import local.stalin.plugins.output.prefusevisualization.displays.PartialHorizontalTreeView;
import local.stalin.plugins.output.prefusevisualization.displays.PartialVerticalTreeView;
import local.stalin.plugins.output.prefusevisualization.displays.RotateRadialGraphView;
import local.stalin.plugins.output.prefusevisualization.editor.PrefuseEditor;
import local.stalin.plugins.output.prefusevisualization.editor.PrefuseEditorInput;
import local.stalin.plugins.output.prefusevisualization.gui.DisplayChooseDialog;
...
public class PrefuseGraphObserver implements IUnmanagedObserver {
	...
	static {
		s_Logger = StalinServices.getInstance().getLogger(Activator.PLUGIN_ID);
	}
	...
	public boolean process(IElement root) {
		walk(root, new HashMap<IElement, Node>(), createNode(root.getPayload()
				.getName(), root.getPayload().getID().toString()));
		return false;
	}

	public void walk(IElement current, HashMap<IElement, Node> visited,
			Node parent) {
		if (current instanceof INode) {
			for (INode n : ((INode) current).getOutgoingNodes()) {
		...
		} else if (current instanceof IExplicitNode) {
			for (IEdge n : ((IExplicitNode) current).getOutgoing()) {
				if (visited.containsKey(n)) {
					createEdge(parent, visited.get(n));
					return;
				}
		...
		} else if (current instanceof IEdge) {
			INode n = ((IEdge) current).getTarget();
			if (visited.containsKey(n)) {
				createEdge(parent, visited.get(n));
				return;
			}
