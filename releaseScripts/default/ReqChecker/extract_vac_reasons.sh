...
# The original requirements file.
orig_req_file="${1}"
# The log Ultimate produced when analyzing the requirements file
ultimate_log="${2}"
# This is the path to the repository, which contains the requirements-folder
requirement_repo="${3}"
# This is the path to the repository, which contains ultimate
ultimate_repo="${4}"

# Below here you do not have to change anything, except you know what you are doing.
explode="${ultimate_repo}/releaseScripts/default/ReqChecker/explode_script.py"
...
function check_params(){
    if [[ $PWD != *"$dump_dir" ]]; then
        echo "Not in directory $dump_dir"req
        exit 1;
    fi
}
