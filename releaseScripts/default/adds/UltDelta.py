...
def parse_args():
    ...
    parser.add_argument(
        "--output", metavar="<file>", nargs=1, help="File to which output is written"
    )
    parser.add_argument(
        "--mode",
        metavar="<mode>",
...
def poll_processes(
    list_of_processes, is_interesting, is_interesting_initial, write_output
):
    # poll the output
...
def delta_debug_smtinterpol(input_file, verbose, output, extras):
    ...
    poll_processes(
        processes, [ult_interesting, z3_interesting], [False, False], verbose
    )
...
def delta_debug_creduce(input_file, verbose, output, extras):
    ...
    ult_interesting = (
        lambda acc, line: acc
        or line.find(
            "java.lang.ClassCastException: de.uni_freiburg.informatik.ultimate.cdt.translation.implementation.container.c.CNamed cannot be cast to de.uni_freiburg.informatik.ultimate.cdt.translation.implementation.container.c.CPrimitive"
        )
        != -1
    )
    ...
    poll_processes(
        processes, [ult_interesting, gcc_interesting], [False, True], verbose
    )
