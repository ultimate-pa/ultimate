...
if [ "$2" == "linux" ]; then
  ...
  ADDS+=("adds/z3" "adds/bitwuzla" "adds/mathsat" "adds/ltl2ba" "adds/smoketest.sh")
elif [ "$2" == "win32" ]; then
  echo "Building .zip for win32..."
  ARCH="win32"
  ARCHPATH="products/CLI-E4/win32/win32/x86_64"
  ADDS+=("adds/z3.exe" "adds/bitwuzla.exe" "adds/mathsat.exe" "adds/mpir.dll" "adds/mathsat.dll" "adds/ltl2ba.exe")
else
