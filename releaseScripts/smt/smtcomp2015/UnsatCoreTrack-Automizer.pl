...
This script contains the SMT commands that are used by Ultimate Automizer
for one subtask, namely the computation of an inductive sequence of state
assertions along a trace.
