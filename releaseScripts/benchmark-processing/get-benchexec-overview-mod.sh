...
# modified to be used for labeling benchmarks 
...
do 
    ...
    for i in "${known_unsafe[@]}"; do 
        if echo "$content" | grep -q "$i"; then
            ...
            filepath=`head -n 1 "${line}" | grep -oP '\-\-file .*? ' | cut -d ' ' -f 2`
            filename="${filepath##*/}"
            dir="${filepath%/*}"
            filenamewoext=`echo $filename | cut -d '.' -f 1`
            extension=`echo $filename | cut -d '.' -f 2-`
			for j in "${label_undefinedbehavior[@]}"; do 
				if echo "$filename" | grep -q "$j"; then 
					continue 3
				fi				
			done
			
            if echo "$filename" | grep -q "true-termination"; then 
                echo "echo "'"'"$filepath wrong result!"'"'
            fi
            
            if ! echo "$filename" | grep -q "false-termination"; then 
                echo "mv "'"'"$filepath"'"'" "'"'"$dir""/""$filenamewoext""_false-termination"".""$extension"'"'
            fi
    ...
    for i in "${known_safe[@]}"; do 
        if echo "$content" | grep -q "$i"; then
            ...
            if ! echo "$filename" | grep -q "true-termination"; then 
                echo "mv "'"'"$filepath"'"'" "'"'"$dir""/""$filenamewoext""_true-termination"".""$extension"'"'
            fi
            continue 2
        fi
    done
