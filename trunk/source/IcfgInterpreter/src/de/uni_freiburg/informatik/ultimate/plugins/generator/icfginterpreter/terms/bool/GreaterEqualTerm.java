...
public class GreaterEqualTerm extends BooleanTerm {
	...
	public GreaterEqualTerm(final IntegerTerm... mSubTerms) {
	...
	public BooleanTerm negate() {
		if (subTerms.length == 2) {
			return new LessTerm(subTerms[0], subTerms[1]);
		}

		final LessTerm[] pairs = new LessTerm[subTerms.length - 1];

		for (int i = 0; i < subTerms.length - 1; i++) {
			pairs[i] = new LessTerm(subTerms[i], subTerms[i + 1]);
		}
	...
	public boolean equals(final Object b) {
		if (!(b instanceof GreaterEqualTerm)) {
			return false;
		}
		final LessEqualTerm castB = ((GreaterEqualTerm) b).simplify();
