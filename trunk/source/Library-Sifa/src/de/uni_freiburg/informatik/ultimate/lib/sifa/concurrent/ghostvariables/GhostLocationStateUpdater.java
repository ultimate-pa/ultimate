...
public final class GhostLocationStateUpdater {

	private final IUltimateServiceProvider mServices;
	private final ManagedScript mManagedScript;
	private final BasicPredicateFactory mPredicateFactory;
	private final GhostVariableManager mGhostVariables;
	private final Map<String, IdentityHashMap<Term, Term>> mProjectionCache = new HashMap<>();

	public GhostLocationStateUpdater(final IUltimateServiceProvider services, final ManagedScript managedScript,
			final BasicPredicateFactory predicateFactory, final GhostVariableManager ghostVariables) {
		mServices = services;
	...
	public IPredicate addLocationUpdate(final IPredicate postState, final String threadId,
			final IcfgLocation targetLocation) {
		if (!isEnabled() || !mGhostVariables.tracksLocationPrecisely(threadId)
		...
		if (SmtUtils.isTrueLiteral(postState.getFormula())) {
			return mPredicateFactory.newPredicate(locConstraint);
		}
