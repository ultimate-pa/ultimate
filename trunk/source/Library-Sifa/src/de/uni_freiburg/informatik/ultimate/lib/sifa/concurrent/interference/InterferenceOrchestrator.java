...
/**
 * Orchestrates interference collection, transformation, merging, and application.
 */
public class InterferenceOrchestrator {
	...
	private void logItfs(final InterferenceAbstraction interferences) {
		for (final String threadId : interferences.getThreadIds()) {
			...
			System.out.println("[DEBUG] Interferences from thread " + threadId + ": " + threadItfs.size());
			for (final IPredicate itf : threadItfs) {
				System.out.println("[DEBUG]   " + itf.getFormula());
			}
		}
	}
	...
	public IPredicate itfFixpoint(final IPredicate state, final String threadId) {
		...
		if (interferences.isEmpty()) {
			System.out.println("[DEBUG] itfFixpoint for " + threadId + ": no interferences, returning state unchanged");
			return state;
		}
		System.out.println("[DEBUG] itfFixpoint for " + threadId + ": applying " + interferences.size()
				+ " interferences to state " + state.getFormula());
		final IPredicate result = mFixpointStrategy.computeFixpoint(state, interferences, mDomain, mPostcondition);
		System.out.println("[DEBUG] itfFixpoint result: " + result.getFormula());
		return result;
	}
}
