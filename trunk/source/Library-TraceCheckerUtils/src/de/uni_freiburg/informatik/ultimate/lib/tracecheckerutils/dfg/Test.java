...
public class Test {
	public static void test(final IIcfg<?> icfg, final ILogger logger) {
		logger.info("Building Dfg for InitialNode: " + icfg.getInitialNodes());
		final DfgContainer dfg = DfgBuilder.buildDfg((Set<IcfgLocation>) icfg.getInitialNodes(), logger);
		...
		logger.debug(
				"UseToUse Dfg: " + DfgBuilder.buildDfgUseToUse((Set<IcfgLocation>) icfg.getInitialNodes(), logger));
