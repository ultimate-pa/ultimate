...
public class RootNode extends IcfgLocation {
	...
	@Deprecated
	public RootNode(final ILocation location, final BoogieIcfgContainer rootAnnot, final String pluginId) {
		super(ArtificialRootDebugIdentifier.DEFAULT, "rootNode");
		mPluginId = pluginId;
		getPayload().getAnnotations().put(mPluginId, rootAnnot);
		location.annotate(this);
	}

	@Deprecated
	public BoogieIcfgContainer getRootAnnot() {
		return (BoogieIcfgContainer) getPayload().getAnnotations().get(mPluginId);
	}
