...
import de.uni_freiburg.informatik.ultimate.plugins.generator.icfgbuilder.cfg.Call;
...
import de.uni_freiburg.informatik.ultimate.plugins.generator.icfgbuilder.cfg.ForkThreadOther;
...
import de.uni_freiburg.informatik.ultimate.plugins.generator.icfgbuilder.cfg.JoinThreadOther;
import de.uni_freiburg.informatik.ultimate.plugins.generator.icfgbuilder.cfg.ParallelComposition;
import de.uni_freiburg.informatik.ultimate.plugins.generator.icfgbuilder.cfg.Return;
import de.uni_freiburg.informatik.ultimate.plugins.generator.icfgbuilder.cfg.SequentialComposition;
...
public class IcfgBacktranslator extends
	...
	public IcfgBacktranslator(final ILogger logger) {
	...
	public List<BoogieASTNode> translateTrace(final List<IIcfgTransition<IcfgLocation>> trace) {
		...
		for (final IIcfgTransition<IcfgLocation> elem : cbTrace) {
			if (!(elem instanceof CodeBlock)) {
				throw new AssertionError("unknown icfg element");
			}
	...
	public IProgramExecution<BoogieASTNode, Expression>
		...
		if (icfgProgramExecution.getInitialProgramState() != null) {
			programStateMapping.put(-1, translateProgramState(icfgProgramExecution.getInitialProgramState()));
		}
		for (int i = 0; i < icfgProgramExecution.getLength(); i++) {
			final AtomicTraceElement<IIcfgTransition<IcfgLocation>> ate = icfgProgramExecution.getTraceElement(i);
			final Map<TermVariable, Boolean>[] branchEncoders = icfgProgramExecution.getBranchEncoders();
			...
			final ProgramState<Term> programState = icfgProgramExecution.getProgramState(i);
