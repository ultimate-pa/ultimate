...
public class StateChangeLogger implements IAbstractStateChangeListener {
	...
	public StateChangeLogger(Logger logger, boolean logToConsole,
			boolean logToFile, String fileName) {
		...
		if (m_logToFile) {
			...
			try {
				...
				if (file.isFile() && file.canWrite() || !file.exists()) {
					if (file.exists()) {
						m_logger.info(String
								.format("File \"%s\" already exists and will be overwritten.",
										file.getAbsolutePath()));
					}
					file.createNewFile();
					m_logger.info(String.format(
							"Logging state changes to \"%s\"",
							file.getAbsolutePath()));
				...
				m_logger.error(
						String.format("Can't open file \"%s\"", fileName), e);
			}
	 ...
	 */
	@Override
	public void onStateChange(RCFGEdge viaEdge, List<StackState> oldStates,
			StackState newState, StackState mergedState) {
	...
	private void logState(StackState state, String targetName,
		...
		for (ScopedAbstractState cse : callStack) {
			...
			/*
			 * Map<Pair, IAbstractValue<?>> values = cse.getValues(); if
			 * (!values.isEmpty()) { output.append("\t\tValues:\n"); for (Pair
			 * identifier : values.keySet()) { IAbstractValue<?> value =
			 * values.get(identifier);
			 * output.append(String.format("\t\t\t%s -> %s\n", identifier,
			 * value)); } } Map<Pair, ArrayData> arrays = cse.getArrays(); if
			 * (!arrays.isEmpty()) { output.append("\t\tArrays:\n"); for (Pair
			 * identifier : arrays.keySet()) { ArrayData array =
			 * arrays.get(identifier);
			 * output.append(String.format("\t\t\t%s -> %s\n", identifier,
			 * array.getValue())); if (array.getIndicesUnclear())
		...
		for (CodeBlock c : trace) {
			output.append("\t\t").append(c.getPrettyPrintedStatements())
		...
		for (LoopStackElement l : loopsies) {
			...
			if (l.getLoopNode() != null) {
				output.append("\t\t")
						.append(String.format("%s -> ... -> %s -> %s", l
								.getLoopNode(), (ProgramPoint) l.getExitEdge()
								.getSource(), l.getLoopNode())).append("\n");
			}
		}
	}
