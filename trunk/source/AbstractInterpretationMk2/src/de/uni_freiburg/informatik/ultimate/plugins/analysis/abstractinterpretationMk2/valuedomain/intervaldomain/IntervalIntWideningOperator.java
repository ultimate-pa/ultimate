 ...
 * 
 * Given an interval [m, n], here derived as [min(K), max(K)] of the set K of
 * numbers found in the preferences and/or collected from literals in the RCFG.
 * 
 * [a, b] widen [x, y] = | [a, b] union [x, y] if ([a, b] sub [m, n]) or ([x, y]
 * sub [a, b]) | (-infinity, +infinity) otherwise
 * 
 * @author Christopher Dillo
 */
public class IntervalIntWideningOperator implements
	...
	public IntervalIntWideningOperator(IntervalValueFactory factory,
		...
		m_logger.debug(String.format("WIDEN WITH BOUNDS [%s, %s]",
				minInt.toString(), maxInt.toString()));
	}

	public IntervalIntWideningOperator(IntervalValueFactory factory,
			IntervalValue bounds, Logger logger) {
		m_factory = factory;
		m_logger = logger;
		m_bounds = bounds;
	}
	...
	public IValueWideningOperator<Interval> copy() {
		return new IntervalIntWideningOperator(m_factory, m_bounds.copy(),
				m_logger);
	}
