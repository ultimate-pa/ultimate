...
public class LiteralCollector extends RCFGEdgeVisitor {
	...
	public LiteralCollector(INestedWordAutomaton<CodeBlock, Object> nwa,
			Logger logger) {
		m_logger = logger;
	...
	public void visit(Statement statement) {
		if (statement instanceof ReturnStatement) {
			...
			m_logger.error(String.format("Unsupported statement type %s",
					statement.getClass()));
		}
	}
	...
	protected void visit(Expression expr) {
		if (expr instanceof ArrayAccessExpression) {
			...
			m_logger.error(String.format("Extend this with new type %s",
					expr.getClass()));
		}
	}
	...
	protected void visit(RealLiteral expr) {
		if (m_literals.add(expr.getValue()))
			m_logger.debug(String.format("Collected real literal \"%s\"",
					expr.getValue()));
	}

	protected void visit(IntegerLiteral expr) {
		if (m_literals.add(expr.getValue()))
			m_logger.debug(String.format("Collected int literal \"%s\"",
					expr.getValue()));
	}
