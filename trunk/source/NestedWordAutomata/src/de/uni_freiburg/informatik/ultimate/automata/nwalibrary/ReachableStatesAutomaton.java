...
public class ReachableStatesAutomaton<LETTER,STATE> implements INestedWordAutomaton<LETTER,STATE> {
	...
	public class CommonEntriesComponent {
		final Collection<Entry> m_Entries;
		
		public CommonEntriesComponent(Collection<Entry> entries) {
	...
	private void addInitialStates(Iterable<STATE> initialStates) {
		for (STATE state : initialStates) {
			...
			Set<Entry> entries = new HashSet<Entry>(1);
			entries.add(entry);
			CommonEntriesComponent cec = new CommonEntriesComponent(entries);
	...
	private StateContainer getStateContainer(STATE state, CommonEntriesComponent cec) {
		...
		if (sc == null) {
			...
			if (sc.getCommonEntriesComponent() != cec) {
				updateCECs(state, cec);
			}
		}
		return sc;
	}
	
	private void updateCECs(STATE state, CommonEntriesComponent cec) {
	...
	private void buildAllStates() {
		...
		while (!worklist.isEmpty()) {
			...
			for (OutgoingInternalTransition<LETTER, STATE> trans : m_Operand.internalSuccessors(state)) {
				...
				StateContainer succSC = getStateContainer(succ, cec);
