...
public class AbstractInterpretationBenchmark<ACTION, LOCATION> implements ICsvProviderProvider<Object> {
	...
	public AbstractInterpretationBenchmark() {
		mAction2Visits = new HashMap<Integer, Integer>();
		mAction2Merges = new HashMap<Integer, Integer>();
		mAction2Widen = new HashMap<Integer, Integer>();
	...
	public void addIteration(WorklistItem<?, ACTION, ?, LOCATION> currentItem) {
		final ACTION action = currentItem.getAction();
		mLastAction = action.hashCode();
		addOrIncrement(mAction2Visits);
	}

	public void addMerge(int availablePostStatesCount) {
	...
	public String toString() {
		...
		final Optional<Integer> visits = mAction2Visits.entrySet().stream().map(a -> a.getValue())
				.reduce((a, b) -> a + b);
		...
		if (mAction2Merges.isEmpty()) {
			...
			final Optional<Integer> merges = mAction2Merges.entrySet().stream().map(a -> a.getValue())
					.reduce((a, b) -> a + b);
		...
		if (mAction2Widen.isEmpty()) {
			...
			final Optional<Integer> widen = mAction2Widen.entrySet().stream().map(a -> a.getValue())
					.reduce((a, b) -> a + b);
		...
		if (mAction2Fixpoints.isEmpty()) {
			...
			final Optional<Integer> fixpoints = mAction2Fixpoints.entrySet().stream().map(a -> a.getValue())
					.reduce((a, b) -> a + b);
