 ...
 * <p>
 * This representation is similar to the prenex normal form for quantifiers. {@link IfThenElseExpression} are lifted to
 * the top and build a binary tree, where the edges are labeled with the assumed conditions from the
 * {@linkplain IfThenElseExpression}s. The leafs of the tree are if-free expressions.
 * <p>
...
public final class IfExpressionTree {
	 ...
	 * @param exprTransformer
	 *            Transformer to be used for transformation (contains cache for negations of expressions).
	...
	private static IfExpressionTree treeFromIfExpr(final IfThenElseExpression expr,
			final ExpressionTransformer exprTransformer) {
		...
		return new IfExpressionTree(thenCondition, buildTree(expr.getThenPart(), exprTransformer), elseCondition,
				buildTree(expr.getElsePart(), exprTransformer));
	}

	/** @see #buildTree(Expression, ExpressionTransformer) */
	private static IfExpressionTree treeFromBinaryExpr(final BinaryExpression binExpr,
			final ExpressionTransformer exprTransformer) {
	...
	/** @see #buildTree(Expression, ExpressionTransformer) */
	private static IfExpressionTree treeFromUnaryExpr(final UnaryExpression unExpr,
			final ExpressionTransformer exprTransformer) {
	...
	/**
	 * IfThenElseExpression-free content of either the {@code then} or {@code else} part of an IfThenElseExpression. Not
	 * null <=> this is a leaf. All other attributes are null <=> this is a leaf.
	 */
	 ...
	 *
	 * @param expressionWithoutIf
	 *            Effective, if-free expression, when assuming all expressions on the path to this leaf.
	 */
	...
	/**
	 * Construct a new root node or internal node of an IfExpressionTree. The constructed node represents an
	 * {@link IfThenElseExpression}.
	 *
	 * @param thenCondition
	 *            Condition of the if.
	 * @param thenChild
	 *            IfExpressionTree node when taking the then-branch.
	 * @param elseCondition
	 *            Negated condition of the if.
	 * @param elseChild
	 *            IfExpressionTree node when taking the else-branch.
	 */
	private IfExpressionTree(final Expression thenCondition, final IfExpressionTree thenChild,
			final Expression elseCondition, final IfExpressionTree elseChild) {
	...
	/**
	 * Used to assert that the attributes {@link #mThenCondition}, {@link #mThenChild}, {@link #mElseCondition},
	 * {@link #mElseChild} are all {@code null}.
	 *
	 * @param not
	 *            Negate the result: Attributes must NOT be null.
	...
	private boolean compareNonLeafAttributesToNull(final boolean not) {
		return (not ^ (mThenCondition == null)) && (not ^ (mThenChild == null)) && (not ^ (mElseCondition == null))
				&& (not ^ (mElseChild == null));
	}

	/**
	 * Apply a function to all leaf expressions from left to right and overwrite the leaf expressions with the result of
	 * the function.
	 *
	 * @param function
	...
	/**
	 * Appends a tree (called suffix) to each leaf of this tree. Leafs of this tree are replaced by the suffix, and the
	 * leafs of the suffix are replaced by the result of a function, which uses both old leafs as arguments.
	 * <p>
	 * Example: The input ...
	 *
	 * <pre>
	 ...
	 *
	 * @param suffix
	 *            tree to be appended (may be modified)
	 * @param function
	 *            f(thisLeaf, suffixLeaf) = resultLeaf
	 */
	...
	private void appendNonLeaf(final IfExpressionTree suffix,
			final BiFunction<Expression, Expression, Expression> function) {
		if (isLeaf()) {
			assert !suffix.isLeaf();
			mThenCondition = suffix.mThenCondition;
			mElseCondition = suffix.mElseCondition;
			final Function<Expression, Expression> curriedFunction =
					suffixLeaf -> function.apply(mLeafExpr, suffixLeaf);
	...
	/**
	 * Assumes conditions of {@link IfThenElseExpression} represented by this tree. The result is a list of if-free
	 * expressions and a list of abstract states for each of these expressions. Each if-free expressions corresponds to
	 * a path through the ifs of the original expression. The associated list of abstract states contains the
	 * pre-states, to which the if-free expression should be applied.
	 * <p>
	 * The name <i>assume leafs</i> can be misleading. We do not "{@code assume} leaf expressions" but assume the leafs
	 * to be the expression to be evaluated, that is, we "@code{assume} expressions on the path to the leafs".
	 * <p>
	 * This method respects the setting {@link OctPostOperator#getMaxParallelStates()} as far as possible. The length of
	 * the returned list does not depend on the setting, but each list entry will have at most {@code maxParallelStates}
	 * abstract pre-states.
	 *
	 * @param postOp
	 *            Post operator to be used to assume the if-conditions.
	 * @param oldStates
	 *            Abstract octagon states - will be modified in-place.
	 * @return If-free expressions with lists of corresponding abstract pre-states
	 */
	public List<Pair<Expression, List<OctDomainState>>> assumeLeafs(final OctPostOperator postOp,
