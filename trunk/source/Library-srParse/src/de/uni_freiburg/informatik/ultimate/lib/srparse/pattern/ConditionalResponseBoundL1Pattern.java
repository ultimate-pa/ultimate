...
/**
 * {scope}, it is always the case that if "R" holds succeeded by "S" for at least "c1" time units, then "T" holds
 * afterwards
 *
 * @author Elisabeth Henkel (henkele@informatik.uni-freiburg.de)
 *
 */
public class ConditionalResponseBoundL1Pattern extends PatternType<ConditionalResponseBoundL1Pattern> {
	public ConditionalResponseBoundL1Pattern(final SrParseScope<?> scope, final String id, final List<CDD> cdds,
	...
	public List<CounterTrace> transform(final CDD[] cdds, final int[] durations,
		...
		if (scope instanceof SrParseScopeGlobally) {
			ct.add(counterTrace(phaseT(), phase(R), phase(S, BoundTypes.GREATEREQUAL, c1), phase(T.negate()),
					phaseT()));
		} else {
