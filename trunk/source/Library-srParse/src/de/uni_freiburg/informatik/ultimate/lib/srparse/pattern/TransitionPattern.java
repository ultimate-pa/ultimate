...
/**
 * Globally, if in location {R} then transition to {S} is enabled.
 */
public class TransitionPattern extends AutomatonPatternType<TransitionPattern> {

	public TransitionPattern(final SrParseScope<?> scope, final String id, final List<CDD> cdds,
			final List<Rational> durations, final List<String> durationNames) {
		super(scope, id, cdds, durations, durationNames);
	}

	// CDDs stored in reverse order: S = cdds[0], R = cdds[1]
	...
	public String toString() {
		...
		sb.append("if in location \"").append(getCdds().get(1).toBoogieString())
				.append("\" then transition to \"").append(getCdds().get(0).toBoogieString())
				.append("\" is enabled");
		return sb.toString();
	}
}
