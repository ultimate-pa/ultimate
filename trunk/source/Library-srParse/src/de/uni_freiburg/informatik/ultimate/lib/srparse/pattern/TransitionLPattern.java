...
/**
 * Globally, if in location {R} for at least {T} time units transition to {S} is enabled.
 *
 * CDDs (reverse order): S = cdds[0], R = cdds[1]. Duration: T = durations[0].
 */
public class TransitionLPattern extends AutomatonPatternType<TransitionLPattern> {

	public TransitionLPattern(final SrParseScope<?> scope, final String id, final List<CDD> cdds,
	...
	public String toString() {
		...
		sb.append("if in location \"").append(getCdds().get(1).toBoogieString()).append("\" for at least ")
				.append(getDurations().get(0)).append(" time units transition to \"")
				.append(getCdds().get(0).toBoogieString()).append("\" is enabled");
		return sb.toString();
	}
}
