...
/**
 * Globally, if in location {R} for at least {T} time units transition to {S} is enabled if guard {V} holds.
 *
 * CDDs (reverse order): V = cdds[0], S = cdds[1], R = cdds[2]. Duration: T = durations[0].
 */
public class TransitionLGPattern extends AutomatonPatternType<TransitionLGPattern> {

	public TransitionLGPattern(final SrParseScope<?> scope, final String id, final List<CDD> cdds,
	...
	public String toString() {
		...
		sb.append("if in location \"").append(getCdds().get(2).toBoogieString()).append("\" for at least ")
				.append(getDurations().get(0)).append(" time units transition to \"")
				.append(getCdds().get(1).toBoogieString()).append("\" is enabled if guard \"")
