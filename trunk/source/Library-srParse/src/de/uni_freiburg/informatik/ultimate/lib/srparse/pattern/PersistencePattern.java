/*
 * Copyright (C) 2022 Elisabeth Henkel (henkele@informatik.uni-freiburg.de)
...
/**
 * {scope}, it is always the case that if "R" holds, then it holds persistently
 *
 * @author Elisabeth Henkel (henkele@informatik.uni-freiburg.de)
 *
 */
public class PersistencePattern extends PatternType<PersistencePattern> {

	public PersistencePattern(final SrParseScope<?> scope, final String id, final List<CDD> cdds,
	...
	public List<CounterTrace> transform(final CDD[] cdds, final int[] durations) {
		...
		if (scope instanceof SrParseScopeGlobally) {
			ct = counterTrace(phaseT(), phase(R), phase(R.negate()), phaseT());
		} else if (scope instanceof SrParseScopeBefore) {
			final CDD P = scope.getCdd1();
			ct = counterTrace(phase(P.negate()), phase(P.negate().and(R)), phase(P.negate().and(R.negate())), phaseT());
		} else if (scope instanceof SrParseScopeAfterUntil) {
			final CDD P = scope.getCdd1();
			final CDD Q = scope.getCdd2();
			ct = counterTrace(phaseT(), phase(P), phase(Q.negate()), phase(R.and(Q.negate())),
					phase(R.negate().and(Q.negate())), phaseT());
		} else if (scope instanceof SrParseScopeAfter) {
			final CDD P = scope.getCdd1();
			ct = counterTrace(phaseT(), phase(P), phaseT(), phase(R), phase(R.negate()), phaseT());
		} else if (scope instanceof SrParseScopeBetween) {
			final CDD P = scope.getCdd1();
			final CDD Q = scope.getCdd2();
			ct = counterTrace(phaseT(), phase(P.and(Q.negate())), phase(Q.negate()), phase(R.and(Q.negate())),
					phase(R.negate().and(Q.negate())), phase(Q.negate()), phase(Q), phaseT());
		} else {
	...
	public String toString() {
		...
		sb.append("\" holds, then it holds persistently");
		return sb.toString();
	}
