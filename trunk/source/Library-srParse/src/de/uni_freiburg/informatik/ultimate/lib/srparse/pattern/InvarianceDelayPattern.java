/*
 * Copyright (C) 2025 Elisabeth Henkel (henkele@informatik.uni-freiburg.de)
...
/**
 * {scope}, it is always the case that if "R" holds, then "S" holds as well after at most "c1" time units
 *
 * @author Elisabeth Henkel (henkele@informatik.uni-freiburg.de)
 *
 */
public class InvarianceDelayPattern extends PatternType<InvarianceDelayPattern> {
	public InvarianceDelayPattern(final SrParseScope<?> scope, final String id, final List<CDD> cdds,
	...
	public List<CounterTrace> transform(final CDD[] cdds, final int[] durations,
		...
		if (scope instanceof SrParseScopeGlobally) {
			ct.add(counterTrace(phaseT(), phase(R.and(S)), phase(R.and(S.negate())), phaseT()));
			ct.add(counterTrace(phaseT(), phase(R.and(S.negate())), phase(S.negate(), BoundTypes.GREATER, c1),
					phaseT()));
	...
	public String toString() {
		...
		sb.append("\" holds as well after at most \"");
