...
/**
 * Globally, if in location {R} then transition to {S} if event {U} fires.
 *
 * CDDs (reverse order): U = cdds[0], S = cdds[1], R = cdds[2].
 */
public class TransitionEPattern extends AutomatonPatternType<TransitionEPattern> {

	public TransitionEPattern(final SrParseScope<?> scope, final String id, final List<CDD> cdds,
	...
	public String toString() {
		...
		sb.append("if in location \"").append(getCdds().get(2).toBoogieString())
				...
				.append("\" if event \"").append(getCdds().get(0).toBoogieString())
				.append("\" fires");
		return sb.toString();
	}
}
