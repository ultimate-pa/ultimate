...
/**
 * Globally, if in location {R} for at most {T} time units transition to {S} if event {U} fires.
 *
 * CDDs (reverse order): U = cdds[0], S = cdds[1], R = cdds[2]. Duration: T = durations[0].
 */
public class TransitionUEPattern extends AutomatonPatternType<TransitionUEPattern> {

	public TransitionUEPattern(final SrParseScope<?> scope, final String id, final List<CDD> cdds,
