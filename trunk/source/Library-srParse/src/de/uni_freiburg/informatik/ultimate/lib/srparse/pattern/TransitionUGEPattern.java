...
/**
 * Globally, if in location {R} for at most {T} time units transition to {S} if event {U} fires and guard {V} holds.
 *
 * CDDs (reverse order): V = cdds[0], U = cdds[1], S = cdds[2], R = cdds[3]. Duration: T = durations[0].
 */
public class TransitionUGEPattern extends AutomatonPatternType<TransitionUGEPattern> {

	public TransitionUGEPattern(final SrParseScope<?> scope, final String id, final List<CDD> cdds,
	...
	public String toString() {
		...
		sb.append("if in location \"").append(getCdds().get(3).toBoogieString()).append("\" for at most ")
				.append(getDurations().get(0)).append(" time units transition to \"")
				.append(getCdds().get(2).toBoogieString()).append("\" if event \"")
				.append(getCdds().get(0).toBoogieString()).append("\" fires and guard \"")
				.append(getCdds().get(1).toBoogieString()).append("\" holds");
		return sb.toString();
	}
}
