...
/**
 * Globally, if in location {R} for at least {T} time units transition to {S} if event {U} fires.
 *
 * CDDs (reverse order): U = cdds[0], S = cdds[1], R = cdds[2]. Duration: T = durations[0].
 */
public class TransitionLEPattern extends AutomatonPatternType<TransitionLEPattern> {

	public TransitionLEPattern(final SrParseScope<?> scope, final String id, final List<CDD> cdds,
	...
	public String toString() {
		...
		sb.append("if in location \"").append(getCdds().get(2).toBoogieString()).append("\" for at least ")
				...
				.append(getCdds().get(1).toBoogieString()).append("\" if event \"")
				.append(getCdds().get(0).toBoogieString()).append("\" fires");
		return sb.toString();
	}
}
