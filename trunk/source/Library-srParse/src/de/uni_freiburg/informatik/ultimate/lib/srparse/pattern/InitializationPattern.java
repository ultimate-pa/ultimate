...
/**
 * {scope}, it is always the case that initially "R" holds.
 *
 * @author Elisabeth Henkel (henkele@informatik.uni-freiburg.de)
 */
public class InitializationPattern extends PatternType<InitializationPattern> {
	public InitializationPattern(final SrParseScope<?> scope, final String id, final List<CDD> cdds,
	...
	public List<CounterTrace> transform(final CDD[] cdds, final int[] durations) {
		...
		if (scope instanceof SrParseScopeGlobally) {
			ct = counterTrace(phase(R.negate()), phaseT());
		} else if (scope instanceof SrParseScopeBefore) {
			final CDD P = scope.getCdd1();
			ct = counterTrace(phase(P.negate().and(R.negate())), phaseT());
		} else if (scope instanceof SrParseScopeAfterUntil) {
			final CDD P = scope.getCdd1();
			final CDD Q = scope.getCdd2();
			ct = counterTrace(phaseT(), phase(P), phase(Q.negate().and(R.negate())), phaseT());
		} else if (scope instanceof SrParseScopeAfter) {
			final CDD P = scope.getCdd1();
			ct = counterTrace(phaseT(), phase(P), phase(R.negate()), phaseT());
	...
	public String toString() {
		...
		sb.append("it is always the case that initially \"");
