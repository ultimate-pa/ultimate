...
public class Trace2PeaCompilerStateless {
	...
	private void precomputeCDDs() {
		...
		for (int p = 0; p < mCountertrace.getPhases().length; p++) {
			...
			mKeep[p] = CDD.TRUE;
			if (!mCountertrace.getPhases()[p].getForbid().isEmpty()) {
				/*
		...
		while (mLastphase > 0 && mCountertrace.getPhases()[mLastphase].isAllowEmpty()
	 ...
	 */
	private void buildNewTrans(final Phase<CDD> src, CDD guard, final CDD stateInv, final String[] resets,
			final PhaseBits destBits) {
		Phase<CDD> dest;
		if (mAllPhases.containsKey(destBits)) {
			...
			for (int p = 0, pbit = 1; pbit <= destBits.active; p++, pbit += pbit) {
				if ((destBits.waiting & pbit) != 0
					 ...
					 */
					if (p == mLastphase && mCountertrace.getPhases()[p].getBoundType() > 0
			...
			dest = new Phase<CDD>(phaseName(destBits.toString()), stateInv, clockInv);
		...
		src.addTransition(dest, guard, resets);
	}
	...
	private void recursiveBuildTrans(final PhaseBits srcBits, final Phase<CDD> src, final CDD guard, CDD stateInv,
		...
		if (mLogger.isDebugEnabled()) {
			mLogger.debug("recursiveBuildTrans: " + srcBits + "->" + new PhaseBits(active, exactbound, waiting) + " ("
		...
		if (p == mCountertrace.getPhases().length) {
			...
			if ((active & 1 << p - 1) == 0) {
				...
				buildNewTrans(src, guard, stateInv, resets, new PhaseBits(active, exactbound, waiting));
			}
			return;
		}
	...
	private Phase<CDD> buildExitSyncTransitions() {
		...
		while (iter.hasNext()) {
			...
			if (guard != CDD.FALSE) {
				ph.addTransition(exit, guard.and(exitGuard).prime(mConstantIds), noResets);
			}
		}
		return exit;
	}
