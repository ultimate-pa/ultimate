...
/**
 * @author Roland Meyer Generates a Uppaal-readable XML-Document for a given net of PEAs. Clocks are not generated
 *         automatically and need to be added by hand. Document type definition of Uppaal-files needs to be added by
 *         hand. String for detecting final states is static. This class is a prototype. PEAs should be checked using
 *         ARMC.
 */
public class PEAJ2UPPAALConverter {
	...
	public Document convert(final PhaseEventAutomata<CDD>[] peas) {
		...
		systemNode.appendChild(document.createTextNode("system " + system + ";"));
	...
	protected Element createPhaseEventAutomaton(final PhaseEventAutomata<CDD> pea) {
		...
		for (int i = 0; i < phases.length; i++) {
			...
			while (transitionsIterator.hasNext()) {
				final Transition actTransition = (Transition) transitionsIterator.next();
				final Element[] actTransitionNode = createTransitionNodes(actTransition);
	...
	protected Element[] createTransitionNodes(final Transition<CDD> transition) {
		final String source = transition.getSrc().getName();
		final String target = transition.getDest().getName();
		if (source.equals("") || target.equals("")) {
			throw new RuntimeException("Source and target of a transition are not allowed to be empty");
		}
		...
		for (int i = 0; i < guards.length; i++) {
			...
			actResetNode.appendChild(document.createTextNode(assignment.toString()));
	...
	protected Element createPhaseNode(final Phase<CDD> phase) {
		...
		if (phase.getClockInvariant() != CDD.TRUE) {
			...
			if (clockInv.length != 1) {
				throw new IllegalArgumentException("Non-convex clock invariant: " + phase.getClockInvariant());
			}
