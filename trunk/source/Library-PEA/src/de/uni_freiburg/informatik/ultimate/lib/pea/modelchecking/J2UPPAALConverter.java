...
/**
 * The class <code>J2UPPAALConverter</code> transforms a Phase Event automaton object into a xml-File that can be
 * read via UPPAAL.
...
public class J2UPPAALConverter {
        ...
        if (init.length > 1) {
            for (int i = 0; i < init.length; i++) { //for all init states
                ...
                if (initClock != CDD.TRUE) {
                    writer.write(" <label kind = \"assignment\">" + reset +
                        "</label>\n");
                }
                ...
                if (guard.and(destInv) == CDD.FALSE) {
                    ...
                    try {
                        ...
                        if (disjuncts[i].isEmpty()) {
                            writer.write("  <label kind = \"guard\"> timer " +
                                XMLString.GREATER + " 0 </label>\n");
                        } else {
                            writer.write("  <label kind = \"guard\">" +
                                disjuncts[i] + XMLString.AND + "timer" +
                                XMLString.GREATER + "0 </label>\n");
                        }

                        writer.write("  <label kind = \"assignment\">" +
    ...
    public void writePEA2UppaalFile(String file, PhaseEventAutomata pea) {
        ...
        System.out.println("The Timed Automaton has " + transitionsInS +
            " transitions");
