...
/**
 * The class <code>J2UPPAALWriterV4</code> transforms a Phase Event automaton object into a xml-File that can be read
...
public class J2UPPAALWriterV4 {
	...
	private String createPEAString(PhaseEventAutomata<CDD> pea) {
		...
		while (transIter.hasNext()) {
			final Transition<CDD> actTrans = (Transition<CDD>) transIter.next();
			buf.append(createPEATransitionString(actTrans));
		}
		return buf.toString();
	}

	private String createPEAPhaseString(Phase<CDD> phase) {
		final StringBuffer buf = new StringBuffer();
	...
	private String createPEATransitionString(Transition<CDD> transition) {
		final StringBuffer buf = new StringBuffer();
	...
	public static void main(String[] param) {
		...
		// private static final String PATH = "C:/Documents and Settings/oea1lr/My
		// Documents/Test/PEA/peatoolkit-0.89b-src/";
		try {
			final PEAXML2JConverter xml2JConverter = new PEAXML2JConverter(false);
			PhaseEventAutomata[] systemPeas = xml2JConverter.convert(
					"C:/Documents and Settings/oea1lr/My Documents/Test/PEA/peatoolkit-0.89b-src/src/pea/modelchecking/CaseStudy/Environment.xml");
			PhaseEventAutomata toUppaal = systemPeas[0];
			for (int i = 1; i < systemPeas.length; i++) {
				toUppaal = PEAUtils.parallel(toUppaal, systemPeas[i]); // toUppaal.parallel(systemPeas[i]);
			}
			systemPeas = xml2JConverter.convert(
					"C:/Documents and Settings/oea1lr/My Documents/Test/PEA/peatoolkit-0.89b-src/src/pea/modelchecking/CaseStudy/TrainEmergency.xml");
			for (int i = 0; i < systemPeas.length; i++) {
				toUppaal = PEAUtils.parallel(toUppaal, systemPeas[i]);
			}
			systemPeas = xml2JConverter.convert(
					"C:/Documents and Settings/oea1lr/My Documents/Test/PEA/peatoolkit-0.89b-src/src/pea/modelchecking/CaseStudy/TrainReact.xml");
			for (int i = 0; i < systemPeas.length; i++) {
				toUppaal = PEAUtils.parallel(toUppaal, systemPeas[i]);
			}
			systemPeas = xml2JConverter.convert(
					"C:/Documents and Settings/oea1lr/My Documents/Test/PEA/peatoolkit-0.89b-src/src/pea/modelchecking/CaseStudy/ComNW.xml");
			for (int i = 0; i < systemPeas.length; i++) {
				toUppaal = PEAUtils.parallel(toUppaal, systemPeas[i]);
			}
			systemPeas = xml2JConverter.convert(
					"C:/Documents and Settings/oea1lr/My Documents/Test/PEA/peatoolkit-0.89b-src/src/pea/modelchecking/CaseStudy/RBC.xml");
			for (int i = 0; i < systemPeas.length; i++) {
				toUppaal = PEAUtils.parallel(toUppaal, systemPeas[i]);
			}
			final PhaseEventAutomata[] propertyPeas = xml2JConverter.convert(
					"C:/Documents and Settings/oea1lr/My Documents/Test/PEA/peatoolkit-0.89b-src/src/pea/modelchecking/CaseStudy/Property0.xml");
			for (int i = 0; i < propertyPeas.length; i++) {
				toUppaal = PEAUtils.parallel(toUppaal, systemPeas[i]);
			}
			final J2UPPAALWriter j2uppaalWriter = new J2UPPAALWriter();
			j2uppaalWriter.writePEA2UppaalFile(
					"C:/Documents and Settings/oea1lr/My Documents/Test/PEA/peatoolkit-0.89b-src/src/pea/modelchecking/CaseStudy/toCheck.xml",
					toUppaal);
