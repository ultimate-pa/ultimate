...
public class PEA2ARMCConverter {
	...
	public void convert(PhaseEventAutomata<CDD> pea, String file, List<String> addVars, List<String> addTypes) {
	...
	public void convert(PhaseEventAutomata<CDD> pea, String file, List<String> addVars, List<String> addTypes,
		...
		try {
			...
			System.out.println("Errors writing the TCS-ARMC representation of pea");
			e.printStackTrace();
		}
	}
	...
	protected void createPhaseEventAutomaton(PhaseEventAutomata<CDD> pea) throws IOException {
		...
		for (int i = 0; i < phases.length; i++) {
			...
			while (transIter.hasNext()) {
				...
				createTransitionNode(trans, variableBuffer.toString(), variablePrimedBuffer.toString());
			}
	 ...
	 * 
	 * List<String> primed = new ArrayList<String>(); List<String> unprimed = new ArrayList<String>();
	 * this.fillPrimedAndUnprimedLists(sepContConj, primed, unprimed);
	 * 
	 * for (int i = 0; i < inv.length; i++) { String[] sepInvConj = inv[i].split("/\\\\");
	 * 
	 * List<String> primedForSepInvConj = new ArrayList<String>(primed); List<String> unprimedForSepInvConj = new
	 * ArrayList<String>(unprimed);
	 * 
	 * this.fillPrimedAndUnprimedLists(sepInvConj, primedForSepInvConj, unprimedForSepInvConj);
	 * 
	 * this.writeTransition(phase.getName(), phase.getName(), primedForSepInvConj, unprimedForSepInvConj, varString,
	 * primedVarString); } }
	 */

	protected void createInitEdge(Phase<CDD> phase, String varString, String primedVarString) throws IOException {
		...
		if (!clocks.isEmpty()) {
			...
			while (clocksIterator.hasNext()) {
				...
				initConstraints.add("_" + actClock + "' = " + (phase.isStopped(actClock) ? "0" : "_len'"));
			}
		}
	...
	protected void createTransitionNode(Transition<CDD> trans, String varString, String primedVarString)
			throws IOException {
		...
		final String[] guardDis = formulaConverter.getDisjuncts(false, trans.getGuard(), numberOfDNFs);
		...
		for (int i = 0; i < resets.length; i++) {
			clockExprBuf.append(" /\\ _" + resets[i] + "' = " + (trans.getDest().isStopped(resets[i]) ? "0" : "_len'"));
		}
		...
		for (int i = 0; i < invp2.length; i++) {
			...
			fillPrimedAndUnprimedLists(clockExprConj, primedForSepInvConj, unprimedForSepInvConj);

			// Add invariant expressions
			if (invp2[i] != null) {
				final String[] sepInvConj = invp2[i].split("/\\\\");
				fillPrimedAndUnprimedLists(sepInvConj, primedForSepInvConj, unprimedForSepInvConj);
			}
			...
			for (int j = 0; j < guardDis.length; j++) {
				...
				writeTransition(source, dest, primed, unprimed, varString, primedVarString);
			}
		}
	}

	protected String[] compInvariantsExpression(Phase<CDD> phase) {
		final String[] stateInvDis = formulaConverter.getDisjuncts(true, phase.getStateInvariant(), numberOfDNFs);
		final String[] clockInvDis = formulaConverter.getDisjuncts(true, phase.getClockInvariant(), numberOfDNFs);
	...
	/*
	 * protected void computeCont() { StringBuffer result = new StringBuffer("");
	 * result.append("_disc < 1 /\\ _disc' = 1"); if (!this.clocks.isEmpty()) { Iterator clocksIterator =
	 * this.clocks.iterator(); while (clocksIterator.hasNext()) { String actClock = (String) clocksIterator.next();
	 * result.append(" /\\ _" + actClock + "' = _" + actClock + " + _len "); } } Iterator varIterator =
	 * this.additionalVariables.iterator(); while (varIterator.hasNext()) { String actVariable = (String)
	 * varIterator.next(); result.append(" /\\ _" + actVariable + "' = _" + actVariable); } this.cont =
	 * result.toString(); }
	 */
	...
	protected void fillPrimedAndUnprimedLists(String[] clauses, List<String> primed, List<String> unprimed) {
	...
	protected void writeTransition(String source, String dest, List<String> primed, List<String> unprimed,
			String varString, String primedVarString) throws IOException {
