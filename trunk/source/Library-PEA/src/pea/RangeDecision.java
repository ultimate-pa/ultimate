...
public class RangeDecision extends Decision {
    ...
    public boolean equals(Object o) {
        ...
        RangeDecision rd = (RangeDecision) o;
    ...
    public CDD simplify(CDD[] childs) {
        ...
        if (elems < limits.length) {
            int[] nlimits = new int[elems++];
            CDD[] nchilds = new CDD[elems];
            elems = 0;
    ...
    public static CDD create(String var, int op, int value) {
        ...
        switch (op) {
        case OP_EQ:
            cdd = CDD.create(new RangeDecision(var,
                        new int[] { 2 * value, (2 * value) + 1 }), FTF);

            break;

        case OP_NEQ:
            cdd = CDD.create(new RangeDecision(var,
                        new int[] { 2 * value, (2 * value) + 1 }), TFT);

            break;

        case OP_LT:
            cdd = CDD.create(new RangeDecision(var, new int[] { 2 * value }),
                    CDD.trueChilds);
        ...
        case OP_LTEQ:
            cdd = CDD.create(new RangeDecision(var,
                        new int[] { (2 * value) + 1 }), CDD.trueChilds);

            break;

        case OP_GT:
            cdd = CDD.create(new RangeDecision(var,
                        new int[] { (2 * value) + 1 }), CDD.falseChilds);
    ...
    public CDD and(Decision other, CDD[] childs, CDD[] ochilds, HashMap<CDD,HashMap<CDD,CDD>> cache) {
        int[] olimits = ((RangeDecision) other).limits;
        ...
        while ((optr < olimits.length) || (tptr < limits.length)) {
            ...
            if ((optr == olimits.length) ||
                    ((tptr < limits.length) && (limits[tptr] < olimits[optr]))) {
        ...
        if (nptr != nlimits.length) {
            int[] nnlimits = new int[nptr];
            System.arraycopy(nlimits, 0, nnlimits, 0, nptr);
            nlimits = nnlimits;

            CDD[] nnchilds = new CDD[nptr + 1];
    ...
    public CDD assume(Decision other, CDD[] childs, CDD[] ochilds) {
        ...
        while ((optr < olimits.length) || (tptr < limits.length)) {
            ...
            if ((nptr > 0) &&
                    nchilds[nptr].and(ochilds[optr]).implies(nchilds[nptr - 1]) &&
    ...
    public String toSmtString(int childs, int index) {
        String var = "(- T_" + index + " " + this.var + "_" + index + ")";

        if (childs == 0) {
            return "( " + (((limits[0] & 1) == 0) ? " < " : " <= ") + var +
            " " + (limits[0] / 2) + ".0)";
        }

        if (childs == limits.length) {
            return "( " +
            (((limits[limits.length - 1] & 1) == 1) ? " > " : " >= ") + var +
        ...
        return "(and (" + (((limits[childs - 1] & 1) == 1) ? " < " : " <= ") +
        (limits[childs - 1] / 2) + ".0 " + var + ") (" +
        (((limits[childs] & 1) == 0) ? " < " : ".0 <= ") + var + " " +
        (limits[childs] / 2) + "0. ))";
    }
    ...
    public static void main(String[] param) {
        CDD[] a = new CDD[] {
                ...
                RangeDecision.create("x", OP_LT, 3),
                RangeDecision.create("x", OP_GT, 0),
    ...
    public Decision prime() {
        String primed = this.var + RangeDecision.PRIME;
        int[] limits = this.limits.clone();
    ...
    public Decision unprime() {
        String unprimed = this.var;

        if (this.var.endsWith(PRIME)) {
            unprimed = this.var.substring(0, this.var.length() - 1);
        }
    ...
    public String toTexString(int childs) {
        ...
        if (childs == limits.length) {
            return var +
            (((limits[limits.length - 1] & 1) == 1) ? " > " : " \\geq ") +
            (limits[limits.length - 1] / 2);
        }
