...
public class Transition {
	...
	if (destName.length() < 33)
	    destName = (destName + "                                 ")
		.substring(0, 33);
	StringBuffer result = new StringBuffer(" -> ").append(destName)
    ...
    public void simplifyGuard(){
    	CDD[] guardDNF= guard.toDNF();
    	int length = guardDNF.length;
    	...
    	{	
    		...
    		CDD[] simplifiedGuardDNF = new CDD[length];
    		int j=0;
    		for (int i=0; i< length; i++){
    			CDD guardPart = guardDNF[i];
    			CDD guardPartUnprimed = guardPart.unprime();
    			
    			if (dest.getStateInvariant().and(guardPartUnprimed)!=CDD.FALSE){
    				String guardPartString = guardPart.toString();
