...
/**
 * ZDecision is used to create CDDs containing Z terms. Please note that
 * these terms may contain quantifiers. Currently predicates below quantifiers are
 * only represented as unicode strings and not as CDDs again. This may change in the future.
 *
...
public final class ZDecision extends Decision {
    ...
    /**
     * Creates a CDD for a Z predicate given as Unicode String. It simplifies the
     * predicate according to very simple rules, assuming that the Z predicate has a simple
     * pattern (similar to the pattern allowed in boolean expressions).
     * Please use this method only if you know what you are doing.
     *
     * @param predicate  the Z predicate as Unicode String to represent as CDD
     ...
     * @throws ParseException if the Unicode String could not be parsed.
    ...
    public static boolean cddContainsZDecisionWith(final CDD cdd, final String text) {
        ...
        if (cdd.decision instanceof ZDecision &&
                ((ZDecision) cdd.decision).predicate.contains(text)) {
            return true;
        }
    ...
    /* try to rewrite all decisions in terms of < or <=
     * this might enable further CDD simplifications down the road
     * we also remove spaces around the operator for that reason */
    @Deprecated
