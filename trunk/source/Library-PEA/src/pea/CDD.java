...
/**
 * A CDD (constraint decision diagram) represents a logical quantifier
 * free formula in a BDD like structure.  This formula can have
 * arbitrary atomic propositions that are represented by subclasses of
 * the Decision class.  Each of this decision can have an arbitrary
 * (but finite) number of branches, e.g. an enumeration type of n
 * values can be represented by a decision with n childs.  In the BDD
 * case a decision has just two childs, one for true, one for false.
 *
 * All decisions need to be comparable to each other, and this
 * determines the variable order.  This class implements logical
 * operations like and/or/negate, however the decision needs to
 * implement and/or for two CDD with the same decision at the top
 * node.
 *
 * The normal unification process of BDDs is also done for CDDs:
 * Whenever a new CDD is needed that equals a previously created one,
 * the previous CDD is reused.  If two CDDs represent the same
 * formula, they are always identical.  A CDD that does not equal
 * FALSE is always satisfiable (provided that the decisions are
...
try_next: 
    ...
    /**
     * Check if other CDD is implied by this one.  This is equivalent
     * to &quot;<code>this.negate().or(other) == CDD.TRUE</code>&quot;
     * but more efficient.
    ...
    /**
     * Negate the current CDD.  This means to recursively swap the
     * TRUE and FALSE leafs.
    ...
    /**
     * Simplify the current CDD under the given assumption.
     * This should remove all parts that are already implied
     * by the assumption.  The correctness criteria is
     * <pre>
    ...
    public CDD assume(CDD assumption) {
        while (true) {
            ...
            if (assumption.decision.compareTo(decision) < 0) {
                ...
                return decision.assume(assumption.decision, childs,
                    assumption.childs);
            } else {
                CDD[] newChilds = null;
     ...
     * @return It returns an array with CDDs for each conjunction term of
     * the normal form.
     */
    public CDD[] toDNF() {
        if (this == TRUE) {
            ...
            if (childDominates(i)) {
                ...
                for (int j = 0; j < cdnf.length; j++) {
                    ...
                    if (cddDominates(i, cdnf[j])) {
                        ...
                        CDD newCDD = create(decision, newchilds);
     ...
     * @return It returns an array with CDDs for each disjunction term of
     * the normal form.
     */
    public CDD[] toCNF() {
        if (this == TRUE) {
    ...
    /**
     * Check whether a given childs in the current node dominates
     * (i.e. logically implies) all sub-childs.  This function should
     * only be called by subclasses of Decision.
     *
    ...
    /**
     * Check whether a given childs in the current node is dominated
     * (i.e. logically implied) by all other childs.  This function should
     * only be called by subclasses of Decision.
     *
     * @param i the index of the child to check.
    ...
    /**
     * Check whether a given CDD dominates
     * (i.e. logically implies) all childs (besides the child given by i)
     * of this CDD.
     *
     * @param i the index of the child to be excluded from the check.
    ...
    /**
     * Check whether a given CDD is dominated
     * (i.e. logically implied) by all childs (besides the child given by i)
     * of this CDD.
     *
     * @param i the index of the child to be excluded from the check.
     * @param cdd the constraint to check the dominates relation
        ...
        for (int i = 0; i < childs.length; i++) {
            ...
            if (childDominates(i)) {
                ...
                if (format.matches("tex")) {
                    ...
                    if (childs[i] != CDD.TRUE) {
                        sb.append(" \\wedge ")
                          .append(childs[i].toString("tex", true));
                    }
                }
                ...
                if (format.matches("uppaal")) {
                    ...
                    if (childs[i] != CDD.TRUE) {
                        sb.append(" && ")
                          .append(childs[i].toString("uppaal", true));
                    }
                }
                ...
                if (format.matches("general")) {
                    ...
                    if (childs[i] != CDD.TRUE) {
                        sb.append(" \u2227 ")
                          .append(childs[i].toString("general", true));
                    }
                }
            }
