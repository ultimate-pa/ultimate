...
/**
 * The class <code>Compiler</code> offers the functionality needed to
 * construct phase event automate nets as specified in
 * <code>pea.modelchecking.schemas.PEA.xsd</code>. The compilation is done
 * via the public method <code>compile</code>. The class also provides a
 * static main method which takes one, two, or four arguments:
 * <ul>
 * <li>Argument "-tf": XML-file of the formula to compile. It has to match the
 * schema <code>pea.modelchecking.schemas.TestForm.xsd</code>. Argument is
 * required.
 * <li>Argument "-mc": XML-file of the formula in model-checkable form. If the
 * file does not exists the compile will create it. The file matches
 * <code>pea.modelchecking.schemas.ModelCheckForm.xsd</code>. Default value
 * is <i>FileToTranslate </i> <code>_mc.xml</code>, where <i>FileToTranslate
 * </i> is the full path of the file to translate without <code>.xml</code> in
 * the end. Argument is optional.
 * <li>Argument "-pea": Prefix for name-attributes of phase event automata as
 * defined in <code>pea.modelchecking.schemas.PEA.xsd</code>. Several phase
 * event automata are stored in a phase event automata net. Default value is
 * <code>pea</code>. The automata are numbered. Argument is optional.
 * <li>Argument "-net": Prefix of xml-files the generated phase event automata
 * nets are stored in. Default is <code>peaNet</code> and the files are
 * numbered. The compiler will create the files if they are not present.
 * Argument is optional.
 * <li>Argument "-parallel": If set to "true", the parallel composition of all 
 * PEAs in a net is computed and saved in a file <code>netName_par.xml</code>.
 * This is done for every net.
 * <li>Argument "-lc": Configuration file to initialise the built in log4j
 * logger. Default value is
 * <code>src/pea/modelchecking/CompilerLog.config</code>. Argument is
 * optional.
...
public class Compiler {
    ...
    /**
     * Initialises the Compiler object. Takes as parameter a string that defines
     * the loggername for the built in log4j logger. If the string is empty, the
     * default name <code>Compiler</code> is used. An application using a
     * Compiler object has to make sure that the logger is initialised via
     ...
     */
    public Compiler(boolean useZDecision) throws Exception {
        this("", useZDecision);
    }

    /** @return an array of transition to phaseset maps
     * (if you call it after calling compile()) */
    public HashMap<Transition,PhaseSet>[] getTrans2Phases() {
        return trans2phases;
    }

    /**
     * Generates a fresh phase event automata name
     * <code>peaprefix+peaCounter</code> and increments the counter
     * 
     * @return The name
     */
    ...
    /**
     * Generates a fresh phase event automata net name
     * <code>peaNetprefix+peaNetCounter</code> and increments the counter
     * 
     * @return The name
     */
    ...
    /**
     * Constructs several phase event automata nets 
     * (test automata) from a given test formula.
     * <p>
     * The transformation is done in several steps:
     * <ul>
     * <li>The test formula is transformed into a formula in model-checkable
     * form. This mcForm document is stored in the xml-file given by
     * <code>mcFile</code>. If this argument is empty the file <i>file </i>
     * <code>_mc.xml</code> will be chosen, where <i>file </i> is the file
     * without <code>.xml</code> in the end.
     * <p>
     * The compilation to model-checkable form generates the following files:
     * <ul>
     * <li><i>file </i> <code>_bin.xml</code> which contains the file but the
     * tree structure resolved to binary trees.
     * <li><i>file </i> <code>_sync.xml</code> which contains the file
     * <i>file </i> <code>_bin.xml</code> with chops replaced by sync events
     * <li><i>file </i> <code>_nf.xml</code> which contains the file <i>file
     * </i> <code>_sync.xml</code> but converted to almost normal form, this
     * means disjunction is the top level operator, followed by conjunction,
     * followed by sync events.
     * <li><code>mcFile</code> which contains the model-checkable form of
     * <code>file</code>. For every disjunction a new <code>mcForm</code>
     * tree is created. The file satisfies
     * <code>pea.modelchecking.schemas.ModelCheckForm.xsd</code>.
     * </ul>
     * <li>Every <code>mcForm</code> in <code>mcFile</code> is converted
     * into an array of <code>MCTrace</code> java objects.
     * <li>All <code>MCTrace</code> objects are converted into
     * <code>PhaseEventAutomata</code> objects.
     * <li>For every <code>mcForm</code> the phase event automata are written
     * into a file <i>peaNetPrefix </i> <code>i.xml</code>, where
     * <i>peaNetPrefix </i> is the attribute of the object and <code>i</code>
     * is the index of the model-checkable form starting with 0.
     * </ul>
     * 
     * @param file
     *            File of the test formula to compile into phase event automata
     *            nets. Needs to satisfy
     *            <code>pea.modelchecking.schemas.TestForm.xsd</code>.
     * @param mcFile
     *            XML-file where the model-checkable form of <code>file</code>
     *            is stored. Satisfies
     *            <code>pea.modelchecking.schemas.ModelCheckForm.xsd</code>.
     * @return
     * 		    Returns an ArrayList containing Arrays with all generated test 
     *          automata.
     * 			The automata in one Array are considered to run in parallel.
     * @throws Exception
     *             See exceptions in
     *             <ul>
     *             <li><code>TestForm2MCFormCompiler</code>, constructor and
     *             <code>compile</code> method
     *             <li><code>MCTraceXML2JConverter</code>, constructor and
     *             <code>convert</code> method
     *             <li><code>PEAJ2XMLConverter</code>, constructor
     *             <li><code>XMLWriter</code>,
     *             <code>writeXMLDocumentToFile</code> method
     *             </ul>
     * @see pea.modelchecking.schemas
     * @see pea.PhaseEventAutomata
     * @see pea.modelchecking.MCTrace
     * @see pea.modelchecking.TestForm2MCFormCompiler
     * @see pea.modelchecking.MCTraceXML2JConverter
     * @see pea.Trace2PEACompiler
     * @see pea.modelchecking.PEAJ2XMLConverter
     * @see pea.modelchecking.XMLWriter
     */
    ...
    /**
     * Collects all events in a given CDD and puts them into the syncEvent member
     * set.
     * 
     * @param events
     ...
     */
    public void setPeaNetPrefix(String peaNetPrefix) {
     ...
     */
    public void setPeaPrefix(String peaPrefix) {
     ...
     * 
     * @param args
     *            Parameters to start the compiler. The class description tells
     *            the use.
     */
