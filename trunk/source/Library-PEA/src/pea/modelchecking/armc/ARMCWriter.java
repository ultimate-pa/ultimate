...
package pea.modelchecking.armc;
...
public class ARMCWriter extends TCSWriter {
     ...
     */
    public ARMCWriter(String fileName) {
        super(fileName);
    }
     ...
     * @param rename
     */
    public ARMCWriter(String file, boolean rename) {
        this(file);
        this.rename = rename;
    }
     ...
     *          thrown if writing of output file fails 
     */
    protected void writeConstraintSorted(CDD constraint) throws IOException {
     ...
     *          (confer {@link pea.BooleanDecision#toString(int)})
     * @throws IOException
     *          thrown if writing of output file fails 
     */
    protected void writeDecisionSorted(Decision decision, int child) throws IOException {
...
//            writeDecision(decision, child, primedWriter);
     ...
     */
    @Override
    protected void writeDecision(Decision dec, int i, Writer writer)
            throws IOException {
        if(writer == null){
            writeDecisionSorted(dec, i);
            return;
        ...
        if(dec instanceof RangeDecision){
            ...
            if (i == limits.length) {
                ...
                writer.append("" + limits[limits.length - 1] / 2);
                return;
            }
            if (limits[i - 1] / 2 == limits[i] / 2) {
                writer.append(" > ");
                writer.append("" + limits[i] / 2);
                return;
            }
            if ((limits[i - 1] & 1) == 1) {
                writer.append(" > ");
