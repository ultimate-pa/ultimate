...
public class CaseStudy {
    ...
    public static void main(String[] args) {
    	...
    	try {
            ...
            systemPeas = xml2jconverter.convert(CaseStudy.PATH
                    + "src/pea/modelchecking/CaseStudy/TrainEmergency.xml");
            for (int i = 0; i < systemPeas.length; i++) {
                toUppaal = toUppaal.parallel(systemPeas[i]);
            }
            systemPeas = xml2jconverter.convert(CaseStudy.PATH
                    + "src/pea/modelchecking/CaseStudy/TrainReact.xml");
            for (int i = 0; i < systemPeas.length; i++) {
                toUppaal = toUppaal.parallel(systemPeas[i]);
            }
            systemPeas = xml2jconverter.convert(CaseStudy.PATH
                    + "src/pea/modelchecking/CaseStudy/ComNW.xml");
            for (int i = 0; i < systemPeas.length; i++) {
                toUppaal = toUppaal.parallel(systemPeas[i]);
            }
            systemPeas = xml2jconverter.convert(CaseStudy.PATH
                    + "src/pea/modelchecking/CaseStudy/RBC.xml");
            for (int i = 0; i < systemPeas.length; i++) {
                toUppaal = toUppaal.parallel(systemPeas[i]);
            }
