	...
	private final class DiffTransformer extends TermTransformer {
		...
		private void checkOrAddAxiom(final Term diffTerm, final Doubleton<Term> arrays) {
			if (mDiffAxioms.add(arrays)) {
				...
				/*
				 * If one of the arguments of diff contains a quantified variable, the axiom is not a closed formula.
				 * The problem was discussed in Issue #577. The quantification of free vars is a quick solution. An
				 * probably more performant alternative would be to instantiate the axiom inside the quantified formula.
				 */
