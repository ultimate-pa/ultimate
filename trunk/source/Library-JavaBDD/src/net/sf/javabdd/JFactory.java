...
public class JFactory extends BDDFactory {
    ...
    public static BDDFactory init(int nodenum, int cachesize) {
        final BDDFactory f = new JFactory();
    ...
    private class bdd extends BDD {
		...
		public BDD replaceWith(BDDPairing pair) {
            final int x = _index;
            final int y = bdd_replace(x, ((bddPair) pair));
            bdd_delref(x);
            bdd_addref(y);
            _index = y;
            return this;
		...
		public boolean equals(BDD that) {
            final boolean b = _index == ((bdd) that)._index; 
            return b;
        }
    ...
    private final boolean HASREF(int node) {
        final boolean r = (bddnodes[node*__node_size + offset__refcou_and_level] & REF_MASK) != 0;
        return r;
    }
    ...
    private final void DECREF(int node) {
        final int rc = bddnodes[node*__node_size + offset__refcou_and_level] & REF_MASK;
        if (rc != REF_MASK && rc != 0) {
			bddnodes[node*__node_size + offset__refcou_and_level] -= REF_INC;
		}
    }
    ...
    private static class BddCacheDataI extends BddCacheData {
        ...
        @Override
		BddCacheData copy() {
            final BddCacheDataI that = new BddCacheDataI();
            that.a = a;
            that.b = b;
            that.c = c;
            that.res = res;
    ...
    private static class BddCacheDataD extends BddCacheData {
        ...
        @Override
		BddCacheData copy() {
            final BddCacheDataD that = new BddCacheDataD();
            that.a = a;
            that.b = b;
            that.c = c;
            that.dres = dres;
    ...
    private static class BddCache {
        ...
        BddCache copy() {
            final BddCache that = new BddCache();
            final boolean is_d = table instanceof BddCacheDataD[];
            if (is_d) {
    ...
    int bdd_not(int r) {
        ...
        again : for (;;) {
            ...
            } catch (final ReorderException x) {
                bdd_checkreorder();
                if (firstReorder-- == 1) {
					continue again;
				}
    ...
    int relprod_rec(int l, int r) {
        ...
        final int LEVEL_l = LEVEL(l);
        final int LEVEL_r = LEVEL(r);
    ...
    int appquant_rec(int l, int r) {
        ...
        if (ISCONST(l) && ISCONST(r)) {
            ...
            if (INVARSET(lev)) {
                final int r2 = READREF(2), r1 = READREF(1);
                switch (applyop) {
    ...
    int satone_rec(int r) {
        ...
        if (ISZERO(LOW(r))) {
            final int res = satone_rec(HIGH(r));
            final int m = bdd_makenode(LEVEL(r), BDDZERO, res);
            PUSHREF(m);
            return m;
        } else {
            final int res = satone_rec(LOW(r));
            final int m = bdd_makenode(LEVEL(r), res, BDDZERO);
            PUSHREF(m);
            return m;
        }
    }
    ...
    int satoneset_rec(int r, int var) {
        ...
        if (LEVEL(r) < LEVEL(var)) {
            if (ISZERO(LOW(r))) {
                final int res = satoneset_rec(HIGH(r), var);
                final int m = bdd_makenode(LEVEL(r), BDDZERO, res);
                PUSHREF(m);
                return m;
            } else {
                final int res = satoneset_rec(LOW(r), var);
                final int m = bdd_makenode(LEVEL(r), res, BDDZERO);
                PUSHREF(m);
                return m;
            }
        } else if (LEVEL(var) < LEVEL(r)) {
            final int res = satoneset_rec(r, HIGH(var));
            if (satPolarity == BDDONE) {
                final int m = bdd_makenode(LEVEL(var), BDDZERO, res);
                PUSHREF(m);
                return m;
            } else {
                final int m = bdd_makenode(LEVEL(var), res, BDDZERO);
                PUSHREF(m);
                return m;
            }
        } else /* LEVEL(r) == LEVEL(var) */ {
            if (ISZERO(LOW(r))) {
                final int res = satoneset_rec(HIGH(r), HIGH(var));
                final int m = bdd_makenode(LEVEL(r), BDDZERO, res);
                PUSHREF(m);
                return m;
            } else {
                final int res = satoneset_rec(LOW(r), HIGH(var));
                final int m = bdd_makenode(LEVEL(r), res, BDDZERO);
                PUSHREF(m);
                return m;
            }
    ...
    int[] bdd_varprofile(int r) {
        ...
        final int[] varprofile = new int[bddvarnum];
    ...
    void allsat_rec(int r, List result) {
        if (ISONE(r)) {
            ...
            final byte[] b = new byte[bddvarnum];
    ...
    void bdd_operator_noderesize() {
        if (cacheratio > 0) {
            final int newcachesize = bddnodesize / cacheratio;
    ...
    BddCache BddCacheI_init(int size) {
        ...
        final BddCache cache = new BddCache();
    ...
    int BddCache_resize(BddCache cache, int newsize) {
        ...
        final boolean is_d = cache.table instanceof BddCacheDataD[];
    ...
    *************************************************************************/
    ...
    int bdd_pairs_resize(int oldsize, int newsize) {
        ...
        for (p = pairs; p != null; p = p.next) {
            final int[] new_result = new int[newsize];
    ...
    void bdd_reorder(int method) {
        BddTree top;
        final int savemethod = bddreordermethod;
    ...
    void varseq_qsort(int[] target, int from, int to) {
        ...
        final int r = target[from];
        final int s = target[(from + to) / 2];
        final int t = target[to - 1];
        ...
        final int mid = target[(from + to) / 2];
    ...
    BddTree reorder_swapwin3(BddTree dis, BddTree[] first) {
        final boolean setfirst = dis.prev == null;
        BddTree next = dis;
    ...
    static int siftTestCmp(Object aa, Object bb) {
        final sizePair a = (sizePair) aa;
        final sizePair b = (sizePair) bb;
    ...
    void blockdown(BddTree left) {
        final BddTree right = left.next;
        int n;
        final int leftsize = left.last - left.first;
        final int rightsize = right.last - right.first;
        final int leftstart = bddvar2level[left.seq[0]];
        final int[] lseq = left.seq;
        final int[] rseq = right.seq;
    ...
    int bdd_setmaxincrease(int size) {
        final int old = bddmaxnodeincrease;
	...
	public double setIncreaseFactor(double x) {
        ...
        final double old = increasefactor;
        increasefactor = x;
        return old;
    }
	...
	public int duplicateVar(int var) {
        ...
        final int newVar = bddvarnum;
        final int lev = bddvar2level[var];
    ...
    int bdd_setvarnum(int num) {
        int bdv;
        final int oldbddvarnum = bddvarnum;
        ...
        if (bddvarset == null) {
            ...
            final int[] bddvar2level2 = new int[num + 1];
            System.arraycopy(
                bddvar2level,
                0,
                bddvar2level2,
    ...
    int bdd_reorder_verbose(int v) {
        final int tmp = verbose;
        verbose = v;
        return tmp;
    }

    int bdd_autoreorder(int method) {
        final int tmp = bddreordermethod;
        bddreordermethod = method;
        bddreordertimes = -1;
        return tmp;
    }
    ...
    void bdd_setvarorder(int[] neworder) {
        ...
        for (level = 0; level < bddvarnum; level++) {
            final int lowvar = neworder[level];
    ...
    void reorder_localGbc(int var0) {
        final int var1 = bddlevel2var[bddvar2level[var0] + 1];
    ...
    void reorder_localGbcResize(int toBeProcessed, int var0) {
        ...
        while (toBeProcessed != 0) {
            final int next = NEXT(toBeProcessed);
            final int hash = NODEHASH2(VARr(toBeProcessed), LOW(toBeProcessed), HIGH(toBeProcessed));
    ...
    int mark_roots() {
        final boolean[] dep = new boolean[bddvarnum];
        int n;
    ...
    static String right(String s, int w) {
        ...
        final StringBuffer b = new StringBuffer(w);
        for (int i = n; i < w; ++i) {
            b.append(' ');
        }
        b.append(s);
    ...
    BddTree bddtree_addrange_rec(
        ...
        if (last < t.first) {
            final BddTree tnew = bddtree_new(id);
            if (tnew == null) {
				return null;
			}
            tnew.first = first;
    ...
    *************************************************************************/
    ...
    public JFactory cloneFactory() {
        final JFactory INSTANCE = new JFactory();
        if (applycache != null) {
