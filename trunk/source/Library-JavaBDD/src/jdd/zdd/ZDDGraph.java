...
public class ZDDGraph  extends ZDD  {
	...
	public int allEdge(int from, int to) {
		...
		for(int i = from+1; i < to; i++) {
			final int tmp1 = work_stack[work_stack_tos++]  = mk(i,left, right);
			final int tmp2 = work_stack[work_stack_tos++]  = mk(i,right, 1);
			work_stack_tos -= 4;
			left = work_stack[work_stack_tos++]  = tmp1;
			right = work_stack[work_stack_tos++]  = tmp2;
		}
		final int ret = mk(to, left, right);
		work_stack_tos -= 2;
		return ret;
	}
    ...
    public final int noSubset(int F, int C) {        
        ...
        if(graph_cache.lookup(F, C, CACHE_NOSUBSET)) {
			return graph_cache.answer;
		}
		final int hash = graph_cache.hash_value;

        int ret;
        final int fvar = getVar(F);
        final int cvar = getVar(C);
        if(fvar > cvar) {
            final int tmp1 = work_stack[work_stack_tos++] = noSubset(getLow(F), C);
            ret = mk (fvar, tmp1, getHigh(F));
            work_stack_tos--;
        } else if(fvar < cvar) {
            final int tmp1 = work_stack[work_stack_tos++] = noSubset(F, getLow(C));
            final int tmp2 = work_stack[work_stack_tos++] = noSubset(F, getHigh(C));
    ...
    private final int noSupset_rec(int F, int C) {
		...
		if(graph_cache.lookup(F, C, CACHE_NOSUPSET)) {
			return graph_cache.answer;
		}
		final int hash = graph_cache.hash_value;
	 ...
	 */
	 public int maxSet(int X) {
		...
		final int T1 = work_stack[work_stack_tos++] = maxSet( getHigh(X));
		final int T2 = work_stack[work_stack_tos++] = noSubset(T0, T1);
	 ...
	 public int MaxDot(int X, int Y) {
		...
		final int v = Math.max( getVar(X), getVar(Y) );
		final int Xh = work_stack[work_stack_tos++]  = subset1( X, v);
		final int Yh = work_stack[work_stack_tos++]  = subset1( Y, v);
		final int Xl = work_stack[work_stack_tos++]  = subset0( X, v);
		final int Yl = work_stack[work_stack_tos++]  = subset0( Y, v);
	...
	public static void internal_test() {
		...
		final ZDDGraph g = new ZDDGraph(2000, 10000);
		final int v1 = g.createVar();
		final int v2 = g.createVar();
		final int v3 = g.createVar();
		final int v4 = g.createVar();
		final int v5 = g.createVar();

        
        final int x = g.cubes_union("00011 00010 00110");
        final int y = g.cubes_union("00011 00010 10011 00101");
        final int p = g.cubes_union("00011 00111 01110");
        final int q = g.cubes_union("00110 00111");
        
        
		// test no-subset
		final int ns1 = g.noSubset(x,y);
		final int ns2 = g.noSubset(y,x);
        final int ns3 = g.noSubset(p,q);
        final int ns4 = g.noSubset(q,p);
        final int ns1_answer = g.cube("00110");
        final int ns2_answer = g.cubes_union("00101 10011");
        final int ns3_answer = g.cube("01110");
        final int ns4_answer = 0;
		...
		// test no-superset
		final int ns5 = g.noSupset(x,y);
		final int ns6 = g.noSupset(y,x);
        final int ns7 = g.noSupset(p,q);
		final int ns8 = g.noSupset(q,p);        
        final int ns5_answer = 0;
        final int ns6_answer = g.cube("00101");
        final int ns7_answer = g.cube("00011");
        final int ns8_answer = g.cube("00110");                         
		...
		// test maxset:
		final int ms1 = g.maxSet(x);
		final int ms2 = g.maxSet(y);
