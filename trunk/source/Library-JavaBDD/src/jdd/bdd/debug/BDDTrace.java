...
public class BDDTrace {
	class TracedNames extends BDDNames {
		...
		public String variable(int n) {
			...
			final TracedVariable t = (TracedVariable)variables.elementAt(n);
			return t.name;
		}
	}
	...
	class TracedDebugOperation extends TracedOperation {
		public String text;
		@Override
		public void execute() {  if(verbose) {
	...
	class TracedPrintOperation extends TracedOperation {
		...
		public void execute() {
			if(graph) {
				bdd.printDot(v.name, v.bdd);
			} else { JDDConsole.out.println(v.name + ":"); bdd.printSet(v.bdd); }
		}
		@Override
		public void show_code() {
			if(graph) {
				JDDConsole.out.println(v.name + ".printDot();");
			} else {
				JDDConsole.out.println(v.name + ".printSet();");
			}
		}
	}
	...
	class TracedCheckOperation extends TracedOperation {
		...
		public void execute() throws IOException {
			final boolean equal = (t1.bdd == t2.bdd);
			if(size != -1) {
				final boolean expected =  (size ==  0 ? false : true);
				if(equal != expected) {
	...
	class TracedBDDOperation extends TracedOperation {
		...
		public void show() {
			...
			if(op.equals("=") ) {
				...
				boolean first = true;
				for (final Enumeration e = operands.elements() ; e.hasMoreElements() ;) {
					final TracedVariable v = (TracedVariable)e.nextElement();
					if(first) {
						first = false;
					} else {
						JDDConsole.out.print(", ");
		...
		public void execute() throws IOException {
			...
			if(size != -1) {
				final int size2 = node_count(ret);
				if(size != size2) {
					JDDConsole.out.println("\n*************************************************************************");
		...
		private void do_or() {
			if(ops == 2) {
				ret.bdd = bdd.or(op1.bdd, op2.bdd);
			} else {
				for (final Enumeration e = operands.elements() ; e.hasMoreElements() ;) {
				...
				for (final Enumeration e = operands.elements() ; e.hasMoreElements() ;) {
					final TracedVariable v = (TracedVariable)e.nextElement();
					final int tmp2 = bdd.ref( bdd.or(tmp, v.bdd) );
		...
		private void do_and() {
			if(ops == 2) {
				ret.bdd = bdd.and(op1.bdd, op2.bdd);
			} else {
				for (final Enumeration e = operands.elements() ; e.hasMoreElements() ;) {
				...
				for (final Enumeration e = operands.elements() ; e.hasMoreElements() ;) {
					final TracedVariable v = (TracedVariable)e.nextElement();
					final int tmp2 = bdd.ref( bdd.and(tmp, v.bdd) );
					bdd.deref(tmp); tmp = tmp2;
				}
				ret.bdd = bdd.deref(tmp);
			}
		...
		private void do_nand() {
			if(ops == 2) {
				ret.bdd = bdd.nand(op1.bdd, op2.bdd);
			} else {
				do_and();
				final int tmp = bdd.ref( ret.bdd) ;
				ret.bdd = bdd.not(tmp);
				bdd.deref( tmp);
			}
		}
		...
		private void do_nor() {
			if(ops == 2) {
				ret.bdd = bdd.nor(op1.bdd, op2.bdd);
			} else {
				do_or();
				final int tmp = bdd.ref( ret.bdd) ;
				ret.bdd = bdd.not(tmp);
		...
		public void show_code() {
			final String code;
			final Enumeration e = operands.elements();
			TracedVariable v = (TracedVariable)e.nextElement();
			if(op.equals("=")) {
				JDDConsole.out.println("BDD " + ret.name + " = " + v.name + ";");
			} else {
				JDDConsole.out.print("BDD " + ret.name + " = " +v.name + "." +op);
				JDDConsole.out.print("(");
				final boolean mode2 = op.equals("ite");
				int i = 0;
	...
	public BDDTrace(String file, InputStream is, int nodes) throws IOException {
		...
		stack = new int[64];
		stack_tos = 0;
		cache = Math.max( Math.min(nodes / 10, 5000), 50000);
		...
		final boolean save = Options.verbose ; Options.verbose  = false;	// to avoid GC messaages
		execute();
	...
	private void show_code() {
		...
		int i = 0;
		for (final Enumeration e = variables.elements() ; e.hasMoreElements() ;) {
			final TracedVariable v = (TracedVariable)e.nextElement();
			if(v.is_var) {
				if(i != 0) {
					JDDConsole.out.print(",");
		...
		for (final Enumeration e = operations.elements() ; e.hasMoreElements() ;) {
			final TracedOperation v = (TracedOperation)e.nextElement();
			v.show_code();
		}
	...
	private void alloc_var(String name) {
		final TracedVariable vret = new TracedVariable();
		vret.last_use = 0;
	...
	private void checkVar(TracedBDDOperation tp) {
		checkVar(tp.ret);
		for (final Enumeration e = tp.operands.elements() ; e.hasMoreElements() ;) {
			final TracedVariable v = (TracedVariable)e.nextElement();
			checkVar(v);
		}
	}
	...
	private TracedPrintOperation createPrintOperation(boolean graph, TracedVariable v) {
		final TracedPrintOperation tp = new TracedPrintOperation();
		tp.index = op_count;
		tp.graph = graph;
		tp.v = v;
		operations.add( tp );
	...
	private TracedSaveOperation createSaveOperation(TracedVariable v) {
			final TracedSaveOperation ts = new TracedSaveOperation();
			ts.index = op_count;
			ts.v = v;
			operations.add( ts );
			return ts;
	...
	private TracedCheckOperation createCheckOperation(TracedVariable v1, TracedVariable v2) {
		final TracedCheckOperation tp = new TracedCheckOperation();
		tp.index = op_count;
		tp.t1 = v1;
		tp.t2 = v2;
		operations.add( tp );
	...
	private TracedDebugOperation createDebugOperation(String text) {
		final TracedDebugOperation tp = new TracedDebugOperation();
		tp.index = op_count;
		tp.text = text;
		operations.add( tp );
		return tp;
	...
	private TracedBDDOperation createBDDOperation() {
		final TracedBDDOperation tp = new TracedBDDOperation();
		tp.index = op_count;
		operations.add( tp );
	...
	private void show_results() {
		...
		if(Options.verbose) {
			if(last_assignment != null) {
				final int size =  node_count(last_assignment);
	...
	private void execute() throws IOException {
		...
		for (final Enumeration e = operations.elements() ; e.hasMoreElements() ;) {
			final TracedOperation tp = (TracedOperation)e.nextElement();
			op_count = tp.index;
	...
	private void read_structure() throws IOException {
		...
		while(true) {
			...
			if(ret.equals("trace_verbose_print")) {
				need("("); final String str = getString(); need(")"); need(";");
				createDebugOperation(str);
			} else if(ret.equals("are_equal")) {
				need("("); String str = need();  final TracedVariable t1 = needVar(str);
				need(","); str = need();  final TracedVariable t2 = needVar(str); need(")"); need(";");
				createCheckOperation(t1,t2);
			} else if(ret.equals("print_bdd") || ret.equals("show_bdd") ) {
				need("("); final String str = need();   final TracedVariable v = needVar(str);need(")"); need(";");
				...
				need("=");
				final String op = need();


				updateUsage(vret);


				final TracedBDDOperation tp = createBDDOperation();
				final TracedVariable var = (TracedVariable) map.get(op);
				...
				if(var != null) {	// asignment!
					...
					if(op.equals("new_int_leaf")) {
						need("("); final String c = need(); need(")"); 	need(";");
	...
	private void read_input() throws IOException {
		...
		final int [] v1  = new int[size /2];
		final int [] v2  = new int[size /2];

		final Enumeration e =variables.elements();
	...
	private TracedVariable needVar(String str) throws IOException {
		final TracedVariable ret = (TracedVariable) map.get(str);
		if(ret == null ) {
	...
	private int read() {
		...
		if(stack_tos > 0) {
			...
			try { c = is.read(); } catch(final IOException exx) {  }
		}
		if(c == '\n') {
			line_count++;
		}
	...
	private String next() {
		...
		if(isAlnum(c)) {
			...
			if(c == '%' || c == '#') {
				final int old_line = line_count;
				if(c == '%') {
					final String count = next();
					final TracedOperation tp = (TracedOperation) operations.lastElement();
					if(tp.size  == -1) {
