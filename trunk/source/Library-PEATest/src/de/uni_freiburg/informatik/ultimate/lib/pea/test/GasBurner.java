...
public class GasBurner {
	...
	public void buildTimings() {
		...
		dc3 = compiler.compile("Syn1",
				new CounterTrace(new CounterTrace.DCPhase[] { new CounterTrace.DCPhase(CDD.TRUE),
						new CounterTrace.DCPhase(idle.and(heat), CounterTrace.BOUND_GREATER, 8),
						new CounterTrace.DCPhase(CDD.TRUE) }));
		dc4 = compiler.compile("Syn2", new CounterTrace(new CounterTrace.DCPhase[] { new CounterTrace.DCPhase(CDD.TRUE),
				new CounterTrace.DCPhase(burn.and(heat.negate().or(flame.negate())), CounterTrace.BOUND_GREATER, 8),
				new CounterTrace.DCPhase(CDD.TRUE) }));
		dc5 = compiler.compile("Syn3",
				new CounterTrace(new CounterTrace.DCPhase[] { new CounterTrace.DCPhase(CDD.TRUE),
						new CounterTrace.DCPhase(gas.and(idle.or(purge)), CounterTrace.BOUND_GREATER, 8),
						new CounterTrace.DCPhase(CDD.TRUE) }));
		dc6 = compiler.compile("Syn4",
				new CounterTrace(new CounterTrace.DCPhase[] { new CounterTrace.DCPhase(CDD.TRUE),
						new CounterTrace.DCPhase(gas.negate().and(ignite.or(burn)), CounterTrace.BOUND_GREATER, 8),
						new CounterTrace.DCPhase(CDD.TRUE) }));
		/* Stability */
		dc7 = compiler.compile("Stab2",
				new CounterTrace(new CounterTrace.DCPhase[] { new CounterTrace.DCPhase(CDD.TRUE),
						new CounterTrace.DCPhase(purge.negate()),
						new CounterTrace.DCPhase(purge, CounterTrace.BOUND_LESSEQUAL, 3000),
						new CounterTrace.DCPhase(purge.negate()), new CounterTrace.DCPhase(CDD.TRUE) }));
		dc8 = compiler.compile("Stab3",
				new CounterTrace(new CounterTrace.DCPhase[] { new CounterTrace.DCPhase(CDD.TRUE),
						...
						new CounterTrace.DCPhase(ignite.negate()), new CounterTrace.DCPhase(CDD.TRUE) }));
	}
	...
	public void buildReqLeak() {
		...
		final Phase[] p = new Phase[] { new Phase("init", CDD.TRUE, CDD.TRUE), new Phase("wleak", leak, lteq),
				new Phase("wnleak", leak.negate(), CDD.TRUE, stopleak), new Phase("leak", leak, CDD.TRUE),
				new Phase("nleak", leak.negate(), gteq, stopleak), new Phase("final", CDD.TRUE, CDD.TRUE) };
		for (int i = 0; i < 6; i++) {
			p[i].addTransition(p[i], nosync, noresets);
		}
		p[0].addTransition(p[1], s1.and(s2.negate()), new String[] { "cleak", "cell" });
		p[0].addTransition(p[2], s1.and(s2.negate()), new String[] { "cleak", "cell" });
		p[0].addTransition(p[3], s1.and(s2.negate()), new String[] { "cleak", "cell" });
		p[0].addTransition(p[4], s1.and(s2.negate()), new String[] { "cleak", "cell" });
	...
	public void buildReq60() {
		...
		final Phase[] p = new Phase[] { new Phase("init", CDD.TRUE, CDD.TRUE), new Phase("wait", CDD.TRUE, lteq),
				new Phase("finished", CDD.TRUE, CDD.TRUE), new Phase("final", CDD.TRUE, CDD.TRUE) };
		for (int i = 0; i < 4; i++) {
			p[i].addTransition(p[i], nosync, noresets);
		}
		p[0].addTransition(p[1], s1.and(s2.negate()), new String[] { "c" });
	...
	public static void main(final String[] param) {
		...
		gb.all = spea.mergeFinalLocations((PEATestAutomaton) gb.all, SimplifyPEAs.BADSTATESTRING);
		tcsConverter.convert(gb.all, "gasburner.pl", mergedVariables0, mergedTypes0, false);
