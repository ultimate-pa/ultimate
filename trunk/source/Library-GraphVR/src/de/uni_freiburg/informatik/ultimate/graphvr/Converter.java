...
public class Converter {
	...
	public static TraceAbstractionProtos.NestedWordAutomaton fromAutomaton(
		...
		builder.setCall(fromAlphabet(callAlphabet))
			.setInternal(fromAlphabet(internalAlphabet))
		...
		for (IPredicate state : states) {
			...
			stream(automaton.internalSuccessors(state))
				.map(getTransition(state, internalAlphabet, states))
				.forEach(builder::addInternalEdges);
			stream(automaton.callSuccessors(state))
				.map(getTransition(state, callAlphabet, states))
				.forEach(builder::addCallEdges);
			stream(automaton.returnSuccessors(state))
				.map(getTransition(state, returnAlphabet, states))
