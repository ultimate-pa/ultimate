...
import de.uni_freiburg.informatik.ultimate.plugins.generator.rankingfunctions.RankingFunctionsSynthesizer.VariableDomain;
 ...
 * 
 * The prevalent semantics of an affine Term is
 * <pre>Σ c_i * x_i + c &leq; 0.</pre>
 * 
...
public class AffineTerm {
	 ...
	 */
	public static AffineTerm fromTerm(Term term, VariableDomain domain)
		...
		if (term instanceof ConstantTerm) {
			...
			if (appt.getFunction().getName() == "+") {
				...
				for (Term u : appt.getParameters()) {
					AffineTerm atu = fromTerm(u, domain);
					if (at.isConstant()) {
				...
				AffineTerm divident = fromTerm(appt.getParameters()[0], domain);
				AffineTerm divisor  = fromTerm(appt.getParameters()[1], domain);
