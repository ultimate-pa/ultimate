...
public class BoogieRequirementsParserTestAllPatterns {
	...
	public void TestPatternParse() throws Exception{
		srParsePattern[] parsedPatterns = GenPatterns(this.t.testString);
		
		Assert.assertNotNull(parsedPatterns);
		...
		Assert.assertTrue("fail recognize: "+ parsedPatterns[0].getPattern().getClass().toString()
				+ "["+this.t.patternName+"]:\n"+this.t.testString,
				parsedPatterns[0].getPattern().getClass().toString().endsWith(this.t.patternName));
	}
