...
public class BoogieBooleanExpressionDecisionTest {
	...
	/**
	 * Test CDD together with BoogieBooleanExpressionDecision, especially if negation and logic
	 * operations work properly.
	 */
	@Test
	public void TestBoogieBooleanExpressinDecision(){
		ILocation dummyLocation = new BoogieLocation("test",0,0,0,0,false);
		
		Expression e = new BinaryExpression(dummyLocation,BinaryExpression.Operator.COMPEQ,
					new IdentifierExpression(dummyLocation, "crew"),
					new IntegerLiteral(dummyLocation, "47")
				);
		
		Expression e2 = new BinaryExpression(dummyLocation,BinaryExpression.Operator.COMPLT,
				new IdentifierExpression(dummyLocation, "test"),
				new RealLiteral(dummyLocation, "5.0")
			);
		
		Expression e3 = new BinaryExpression(dummyLocation,BinaryExpression.Operator.COMPNEQ,
				new IdentifierExpression(dummyLocation, "answer"),
				new BooleanLiteral(dummyLocation, true)
			);
		...
		Assert.assertEquals("(answer != true ∨ test < 5.0 ∧ !(crew == 47))",
				cdd.toString(true)
				);
		
		cdd = cdd.negate();
		Assert.assertEquals("!(answer != true) ∧ (crew == 47 ∨ !(test < 5.0))",
				cdd.toString()
				);
	}
	...
	public void TestBoogieBooleanExpressinDecisionIt(){
		...
		Expression e = new IdentifierExpression(dummyLocation,"e");
		...
		Expression e2 = new IdentifierExpression(dummyLocation,"f");
		...
		cdd = cdd.negate();
		Assert.assertEquals("f ∧ !e", cdd.toString(true));
		Expression f = new IdentifierExpression(dummyLocation,"f");
		cdd = cdd.and(BoogieBooleanExpressionDecision.create(f));
		Assert.assertEquals("f ∧ !e", cdd.toString(true));
		//conjuction disjunction
		Expression e3 = new IdentifierExpression(dummyLocation,"g");
