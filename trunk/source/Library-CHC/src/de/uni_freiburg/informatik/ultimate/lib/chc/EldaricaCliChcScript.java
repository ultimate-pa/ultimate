...
public class EldaricaCliChcScript implements IChcScript {
	...
	private static final boolean USE_PORTFOLIO = false;
	private static final boolean USE_DISJUNCTIVE_INTERP = true;
	...
	public LBool solve(final HcSymbolTable symbolTable, final List<HornClause> system, final long timeout) {
		...
		final File tmpFile;
		try {
			tmpFile = File.createTempFile("eldarica_", ".smt2");
			mLogger.info("Writing script to file " + tmpFile.getAbsolutePath());

			final var dumperScript = new LoggingScript(tmpFile.getAbsolutePath(), true);
