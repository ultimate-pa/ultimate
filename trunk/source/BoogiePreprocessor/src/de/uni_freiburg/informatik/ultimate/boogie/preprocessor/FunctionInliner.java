...
public class FunctionInliner extends BoogieTransformer implements IUnmanagedObserver {
	...
	/**
	 * A map containing the functions that should be inlined. The key is the
	 * function name.
	 */
			...
			for (Declaration decl : unit.getDeclarations()) {
				if (decl instanceof FunctionDeclaration) {
					...
					if (!inlined) {
						...
						Expression funcApp = new FunctionApplication(fdecl.getLocation(),
	...
	/**
	 * Changes the declaration information of identifier expression to
	 * QUANTIFIED if they are inparams
	 * 
	 * @param inparams
	 */
	private Expression quantifyBody(VarList[] inparams, final Expression body) {
		final Set<String> inParamIds = new HashSet<>();
		for (final VarList inparam : inparams) {
			for (final String id : inparam.getIdentifiers()) {
				inParamIds.add(id);
			}
		}
		return new BoogieTransformer() {
			@Override
			protected Expression processExpression(final Expression expr) {
				if (expr instanceof IdentifierExpression) {
					final IdentifierExpression iexpr = (IdentifierExpression) expr;
					if (inParamIds.contains(iexpr.getIdentifier())) {
						final Expression rtr = new IdentifierExpression(iexpr.getLocation(), iexpr.getType(),
								iexpr.getIdentifier(), new DeclarationInformation(StorageClass.QUANTIFIED, null));
						return rtr;
					}
				}
	...
	@Override
	/**
	 * Process a variable declaration list. This adds all variables to the name
	 * clash set of the current scope.
	 */
	 ...
	 * renaming (if present), inlines function calls, and renames identifiers in
	 * quantifiers in case a name from an inlined function clashes with a name
