...
public class TypeChecker implements IUnmanagedObserver {
    ...
    private BoogieType typecheckExpression(Expression expr) {
        ...
        if (expr instanceof BinaryExpression) {
            ...
            switch (binexp.getOperator()) {
                ...
                case COMPPO:
                    if (!left.equals(right) && !left.equals(errorType)
                        ...
                        typeError(expr, "Type check failed for "
                                + expr + ": " + left.getUnderlyingType()
                ...
                case BITVECCONCAT:
                    ...
                    resultType = BoogieType.createBitvectorType(leftLen
                            + rightLen);
                    break;
                default:
                    internalError("Unknown Binary operator "
                            + binexp.getOperator());
                    resultType = errorType;
            }
            ...
            BoogieType e = typecheckExpression(sae.getStruct())
                    .getUnderlyingType();
            if (!(e instanceof StructType)) {
                if (!e.equals(errorType))
                    typeError(expr,
                            "Type check failed (not a struct): " + expr);
                resultType = errorType;
            } else {
            ...
            if (!(e instanceof ArrayType)) {
                if (!e.equals(errorType))
                    typeError(expr,
                            "Type check failed (not an array): " + expr);
                resultType = errorType;
            } else {
            ...
            if (info == null) {
                typeError(expr, "Undeclared identifier " + name
                        + " in " + expr);
                resultType = errorType;
            } else {
            ...
            if (fi == null) {
                typeError(expr, "Undeclared function " + name
                        + " in " + expr);
                resultType = errorType;
            } else {
            ...
            if (!condType.equals(errorType) && !condType.equals(boolType)) {
                typeError(expr, "if expects boolean type: "
                        + expr);
            }
            ...
            if (!typeParams.fullyUsed())
                typeError(expr,
                        "Type args not fully used in variable types: " + expr);
    ...
    private BoogieType typecheckLeftHandSide(LeftHandSide lhs) {
        ...
        if (lhs instanceof VariableLHS) {
            ...
            if (!(type instanceof ArrayType)) {
                if (!type.equals(errorType))
                    typeError(lhs,
                            "Type check failed (not an array): " + lhs);
                resultType = errorType;
            } else {
    ...
    public void processProcedureDeclaration(Procedure proc) {
        ...
        for (Specification s : proc.getSpecification()) {
            if (s instanceof RequiresSpecification) {
                ...
                if (!t.equals(boolType) && !t.equals(errorType))
                    typeError(s,
                            "Requires clause is not boolean: " + s);
    ...
    private void typecheckStatement(Stack<String> outer,
        ...
        if (statement instanceof AssumeStatement) {
            ...
            if (lhs.length != rhs.length) {
                typeError(statement,
                        "Number of variables do not match in " + statement);
            } else {
                for (int i = 0; i < lhs.length; i++) {
                    ...
                    if (!lhsType.equals(errorType)
                        ...
                        typeError(statement, "Type mismatch ("
                                + lhsType + " != " + rhsType + ") in "
                                + statement);
                    }
                }
            ...
            for (String label : ((GotoStatement) statement).getLabels()) {
                if (!allLabels.contains(label)) {
                    typeError(statement, "Goto label not found: "
                            + statement);
                }
            }
            ...
            if (procInfo == null) {
                typeError(statement,
                        "Calling undeclared procedure " + call);
                return;
            }
            ...
            for (int i = 0; i < arguments.length; i++) {
                ...
                if (!inParams[i].getType().unify(t, typeParams)) {
                    typeError(statement,
                            "Wrong parameter type at index " + i + ": " + call);
                }
            }
    ...
    private void processImplementation(Procedure impl) {
        ...
        if (procInfo == null) {
            typeError(impl, "Implementation without procedure: "
                    + impl.getIdentifier());
            return;
        }
        ...
        for (VarList vl : impl.getInParams()) {
            ...
            for (String id : vl.getIdentifiers()) {
                if (i >= procInParams.length) {
                    typeError(vl, "Too many input parameters in "
                            + impl);
                } else if (!procInParams[i++].getType().equals(type)) {
                    typeError(vl, "Type differs at parameter "
                            + id + " in " + impl);
                }
                checkIfAlreadyInOutLocal(vl, id);
                m_InParams.add(id);
        ...
        if (!typeParams.fullyUsed())
            typeError(impl,
                    "Type args not fully used in implementation: " + impl);
        i = 0;
        for (VarList vl : impl.getOutParams()) {
            ...
            for (String id : vl.getIdentifiers()) {
                if (i >= procOutParams.length) {
                    typeError(vl,
                            "Too many output parameters in " + impl);
        ...
        if (i < procOutParams.length)
            typeError(impl, "Too few output parameters in "
                    + impl);
        ...
        if (root instanceof WrapperNode) {
            ...
            for (Declaration decl : unit.getDeclarations()) {
                ...
                if (decl instanceof ConstDeclaration)
                    ...
                    /* check where clauses */
                    for (VarList vl : ((VariableDeclaration) decl)
                            .getVariables()) {
                        if (vl.getWhereClause() != null) {
                            BoogieType t = typecheckExpression(vl
                                    .getWhereClause());
                            if (!t.equals(boolType) && !t.equals(errorType))
                                typeError(vl.getWhereClause(),
                                        "Where clause is not boolean: " + decl);
    ...
    @Deprecated
    private static boolean getDefaultPerformedChanges() {
        return false;
    }
