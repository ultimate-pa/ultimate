...
public class CommandLine implements Serializable {
	...
	public Object[] getParsedOptionValues(final String opt) throws ParseException {
		final String[] res = getOptionValues(opt);
		final Option option = resolveOption(opt);
		...
		final Object[] rtr = new Object[res.length];
		for (int i = 0; i < res.length; ++i) {
			rtr[i] = TypeHandler.createValue(res[i], type);
		}
		return rtr;
	}
	...
	/**
	 * Retrieve the map of values associated to the option. This is convenient for options specifying Java properties
	 * like <tt>-Dparam1=value1
	 * -Dparam2=value2</tt>. The first argument of the option is the key, and the 2nd argument is the value. If the
	 * option has only one argument (<tt>-Dfoo</tt>) it is considered as a boolean flag and the value is
	 * <tt>"true"</tt>.
	 *
	 * @param opt
	 *            name of the option
