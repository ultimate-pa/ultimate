		...
		if (mApplyQuantifierElimination) {
			final QuantifierEliminationPostprocessor qePostproc = 
					new QuantifierEliminationPostprocessor(mServices, mLogger, 
							smtManager.getBoogie2Smt(), smtManager.getPredicateFactory(), mSimplificationTechnique, mXnfConversionTechnique);
		...
		for(int i = startPosition+1; i<= endPosition; i++){
			...
			if(subBranch){
				...
				combinedTransitionFormula = TransFormula.sequentialComposition(mLogger, mServices, 
						smtManager.getBoogie2Smt(), false, false, false, mXnfConversionTechnique, mSimplificationTechnique, combinedTransitionFormula,subBranchMarkhorFormula);
			} else{ 
				...
				combinedTransitionFormula = TransFormula.sequentialComposition(mLogger, mServices, 
						smtManager.getBoogie2Smt(), false, false, false, mXnfConversionTechnique, mSimplificationTechnique, combinedTransitionFormula,transitionFormula);
			}
		}
	...
	private Term computeWp(final IPredicate successor, final TransFormula tf, final Script script, 
		...
		if (applyQuantifierElimination) {
			result = PartialQuantifierElimination.tryToEliminate(mServices, 
					mLogger, script, freshTermVariableConstructor, result, mSimplificationTechnique, mXnfConversionTechnique);
		}
		return result;
	}
