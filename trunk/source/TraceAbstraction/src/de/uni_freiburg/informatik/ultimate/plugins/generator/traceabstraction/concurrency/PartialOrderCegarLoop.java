...
public class PartialOrderCegarLoop<L extends IIcfgTransition<?>>
	...
	protected boolean refineAbstraction() throws AutomataLibraryException {
		...
		if (mPref.getPartialOrderMode() == PartialOrderMode.DYNAMIC_ABSTRACTIONS) {
			...
			mAbstraction = new InformationStorage<>(program, mItpAutomata, mFactory, x -> false);
		} else {
			mAbstraction =
					new InformationStorage<>(program, mItpAutomata, mFactory, PartialOrderCegarLoop::isFalseLiteral);
		}
	...
	private boolean isErrorState(final IPredicate state) {
		return PredicateUtils.streamLocations(state).anyMatch(mErrorLocs::contains);
	}
