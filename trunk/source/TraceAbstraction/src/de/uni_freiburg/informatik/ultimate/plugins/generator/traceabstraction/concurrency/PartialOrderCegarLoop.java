...
public class PartialOrderCegarLoop<L extends IIcfgTransition<?>>
	...
	protected boolean refineAbstraction() throws AutomataLibraryException {
		...
		mAbstraction = new MonitorProduct<>(program, mItpAutomata, mFactory, PartialOrderCegarLoop::isFalseLiteral);
	...
	private static <S, T> Predicate<S> compose(final Predicate<T> pred, final Function<S, T> func) {
		return s -> pred.test(func.apply(s));
	}

	private <R> IDfsVisitor<L, R> createVisitor(final Function<R, IPredicate> getOriginal) {
		IDfsVisitor<L, R> visitor = new AcceptingRunSearchVisitor<>(compose(this::isGoalState, getOriginal));
		if (mPOR.getDfsOrder() instanceof BetterLockstepOrder<?, ?>) {
	...
	private final class InformationStorageFactory
			implements IMonitorStateFactory<IPredicate, IPredicate, IPredicate>, IUnionStateFactory<IPredicate> {
		@Override
