...
public class IndependenceProviderFactory<L extends IIcfgTransition<?>> {
	...
	// TODO Refactor and see if this method can be streamlined with createProviders()
	public IRefinableAbstraction<NestedWordAutomaton<L, IPredicate>, ?, L>
		...
		assert mIndependenceScript != null : "Independence script not yet initialized";
		assert mTransferrer != null : "Transferrer to independence script not yet initialized";
		return constructAbstraction(icfg, mPref.getStratifiableAbstraction(), mIndependenceScript);
	}
	...
	private IRefinableIndependenceProvider<L> constructIndependenceProvider(final IIcfg<?> icfg,
		...
		if (settings.getAbstractionType() == AbstractionType.NONE) {
			...
			// condition predicates) to mIndependenceScript -- unless there is a dynamically-stratified abstraction,
			...
			final var independence =
					constructIndependence(settings, tfsAlreadyTransferred, predicateFactory, mPredicateTransferrer);
			return new StaticIndependenceProvider<>(independence);
		}

		// Construct the abstraction function.
		final var letterAbstraction = constructAbstraction(icfg, settings.getAbstractionType(), mIndependenceScript);
