...
/**
 * Tests for convergence checking on InterferenceAbstraction.
 */
public class SubsumptionConvergenceTest {
	...
	public void convergedWhenNewSubsumedByOld() {
		...
		final DefaultInterferenceAbstraction newInterferences = createSet(Map.of("thread1", Set.of(pNew)));
		final DefaultInterferenceAbstraction oldInterferences = createSet(Map.of("thread1", Set.of(pOld)));

		// pNew is subsumed by pOld
		final var domain = new SubsumptionMockDomain(Map.of(pNew, Set.of(pOld)));

		assertTrue(newInterferences.hasConverged(oldInterferences, domain));
	}
	...
	public void notConvergedWhenNewNotSubsumed() {
		...
		assertFalse(newInterferences.hasConverged(oldInterferences, domain));
	}

	@Test
	public void convergedWhenBothEmpty() {
		final DefaultInterferenceAbstraction newInterferences = createSet(Map.of("thread1", Set.of()));
		final DefaultInterferenceAbstraction oldInterferences = createSet(Map.of("thread1", Set.of()));

		assertTrue(newInterferences.hasConverged(oldInterferences, new SubsumptionMockDomain(Map.of())));
	}
	...
	public void notConvergedWhenOldIsEmpty() {
		...
		assertFalse(newInterferences.hasConverged(oldInterferences, new SubsumptionMockDomain(Map.of())));
	}
	...
	public void multipleThreadsAllMustConverge() {
		...
		final DefaultInterferenceAbstraction newInterferences = createSet(newMap);
		final DefaultInterferenceAbstraction oldInterferences = createSet(oldMap);
	...
	private static DefaultInterferenceAbstraction createSet(final Map<String, Set<IPredicate>> map) {
		return DefaultInterferenceAbstraction.ofForTesting(map);
	}
