...
public class PredicateUnifierTest {
	...
	private IProgramNonOldVar a, b;
	private BasicPredicateFactory mBasicFactory;
	private Term zero, one, two;
	...
	public void setUp() {
		...
		a = mFactory.constructProgramVar("a");
		b = mFactory.constructProgramVar("b");
		mTable.add(a);
		mTable.add(b);
		
		zero = mScript.numeral(String.valueOf(0));
		one = mScript.numeral(String.valueOf(1));
		two = mScript.numeral(String.valueOf(2));
		
		mBasicFactory = new BasicPredicateFactory(mServices, mMgdScript, 
				mTable, SimplificationTechnique.NONE, 
	...
	public void testGetOrConstructPredicateForConjunction() {
		...
		final Term singleTerm1 = mScript.term("=", a.getTermVariable(), one);
		final Term singleTerm2 = mScript.term(">", a.getTermVariable(), zero);
		final Term singleTerm3 = mScript.term("<", a.getTermVariable(), two);
		final Term singleTerm4 = mScript.term("<", b.getTermVariable(), one);
		...
		final IPredicate pred4 = unifier.getOrConstructPredicate(
				mScript.term("and", singleTerm1, singleTerm4));
		...
		final IPredicate oPred4 = oUnifier.getOrConstructPredicate(
				mScript.term("and", singleTerm1, singleTerm4));
	...
	public void testGetOrConstructPredicateForDisjunction() {
		...
		final Term singleTerm1 = mScript.term("=", a.getTermVariable(), one);
		final Term singleTerm2 = mScript.term(">", a.getTermVariable(), zero);
		final Term singleTerm3 = mScript.term("=", a.getTermVariable(), zero);
	...
	public void testGetOrConstructPredicate() {		
		...
		final Term term1 = mScript.term("=", a.getTermVariable(), one);
		final Term term2 = mScript.term(">", a.getTermVariable(), zero);
		final Term term3 = mScript.term("<", a.getTermVariable(), two);
		final Term term4 = mScript.term("=", a.getTermVariable(), two);
	...
	public void testIsRepresentative() {
		...
		final TestPredicate pred3 = pred("=", a, 1);
		final IPredicate pred4 = unifier.getOrConstructPredicate(
				and(neg(pred(">", a, 2)), neg(pred("<", a, 2))));
		final IPredicate oPred4 = oUnifier.getOrConstructPredicate(and(neg(pred(">", a, 2)), neg(pred("<", a, 2))));
		final TestPredicate pred5 = and(neg(pred(">", a, 2)), neg(pred("<", a, 2)));
	...
	public void testSipleGetFunctions() {
		...
		Assert.assertThat("2", unifier.getTruePredicate().getFormula(), Is.is(oUnifier.getTruePredicate().getFormula()));
		Assert.assertThat("3", unifier.getFalsePredicate().getFormula(), Is.is(mScript.term("false")));
		Assert.assertThat("4", unifier.getFalsePredicate().getFormula(), Is.is(oUnifier.getFalsePredicate().getFormula()));
