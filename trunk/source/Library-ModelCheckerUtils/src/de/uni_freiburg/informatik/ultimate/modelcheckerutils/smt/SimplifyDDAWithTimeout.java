...
/**
 * This is a copy of de.uni_freiburg.informatik.ultimate.logic.simplification.SimplifyDDA
 * with additional support for timeouts.
 * @author Matthias Heizmann, Jochen Hoenicke, Markus Pomrehn
 *
 */
public class SimplifyDDAWithTimeout extends NonRecursive {
	...
	private static class TermCounter implements Walker {
		...
		public void walk(NonRecursive engine) {
			SimplifyDDAWithTimeout simplifier = (SimplifyDDAWithTimeout) engine;
	...
	private static class ContextCollector implements Walker {
		...
		public void walkNextParameter(SimplifyDDAWithTimeout simplifier) {
			ApplicationTerm appTerm = (ApplicationTerm) mTerm;
			String connective = appTerm.getFunction().getName();
	...
	private static class Simplifier implements Walker {
		...
		private void walkParam(SimplifyDDAWithTimeout simplifier) {
			ApplicationTerm appTerm = (ApplicationTerm) mTerm;
			String connective = appTerm.getFunction().getName();
			Term[] params = appTerm.getParameters();
