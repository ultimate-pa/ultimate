...
public class IndexAnalysisResult extends EqualityAnalysisResult {
	...
	public IndexAnalysisResult(final Set<Doubleton<Term>> equalDoubletons,
			final Set<Doubleton<Term>> distinctDoubletons, final Set<Doubleton<Term>> unknownDoubletons,
			final Set<Doubleton<Term>> ignoredDoubletons) {
		super(equalDoubletons, distinctDoubletons, unknownDoubletons);
		mIgnoredDoubletons = ignoredDoubletons;
	}

	public EqualityStatus isEqual(final List<Term> index1, final List<Term> index2) {
	...
	public boolean isEqualDoubleton(final Term t1, final Term t2) {
		return getEqualDoubletons().contains(new Doubleton<>(t1, t2));
	}

	public boolean isDistinctDoubleton(final Term t1, final Term t2) {
		return getDistinctDoubletons().contains(new Doubleton<>(t1, t2));
	}

	public boolean isUnknownDoubleton(final Term t1, final Term t2) {
		return getUnknownDoubletons().contains(new Doubleton<>(t1, t2));
	}

	public boolean isIgnoredDoubleton(final Term t1, final Term t2) {
		return mIgnoredDoubletons.contains(new Doubleton<>(t1, t2));
	}
}
