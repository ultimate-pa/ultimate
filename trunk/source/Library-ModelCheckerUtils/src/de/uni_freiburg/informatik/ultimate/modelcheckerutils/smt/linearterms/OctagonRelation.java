...
/**
 * Affine relation of the form <i>(±x)-(±y) ≤ c</i> where <i>x</i> and
 * <i>y</i> are (not necessarily different) variables, <i>c</i> is a constant
 * and <i>R</i> is one of the relations =, ≠, ≤, ≥, \<, \> from {@link RelationSymbol}.
 * 
...
public class OctagonRelation {
	...
	public OctagonRelation(final boolean negateVar1, final Term var1,
			final boolean negateVar2, final Term var2,
	...
	public static OctagonRelation from(final AffineRelation ar) {
		switch (ar.getAffineTerm().getVariable2Coefficient().size()) {
			case 1:
				return from1Var(ar);
			case 2:
				return from2Vars(ar);
	...
	private static OctagonRelation from2Vars(final AffineRelation arWith2Vars) {
		...
		return new OctagonRelation(
				var2coeff1.getValue().isNegative(), var2coeff1.getKey(),
				!var2coeff2.getValue().isNegative(), var2coeff2.getKey(),
