...
tool_examples_map = {
    "boogieAutomizer": [  # The worker.id as defined in config.js
        {
            "path": osp.join(
                EXAMPLES_DIR, "programs", "toy", "showcase"
            ),  # path to a folder containing examples.
            "pattern": "*.bpl",  # File name pattern (regex). Matches will be included.
        },
        {
            "path": osp.join(
                EXAMPLES_DIR, "concurrent", "bpl", "regression", "showcase"
            ),
            "pattern": "*.bpl",
        },
    ],
    ...
    "cLTLAutomizer": [
        {"path": osp.join(EXAMPLES_DIR, "LTL", "coolant"), "pattern": "*"},
    ],
    "cLassoRanker": [
        {"path": osp.join(EXAMPLES_DIR, "lassos"), "pattern": "*.c"},
    ],
    "boogieLassoRanker": [
        {"path": osp.join(EXAMPLES_DIR, "lassos"), "pattern": "*.bpl"},
        {"path": osp.join(EXAMPLES_DIR, "lassos", "website"), "pattern": "*.bpl"},
    ],
    "automataScript": [
        {"path": osp.join(EXAMPLES_DIR, "Automata", "website"), "pattern": "*.ats"},
    ],
    "cReferee": [
        {
            "path": osp.join(
                EXAMPLES_DIR, "programs", "toy", "InvariantChecking", "showcase"
            ),
            "pattern": "*.c",
        },
    ],
