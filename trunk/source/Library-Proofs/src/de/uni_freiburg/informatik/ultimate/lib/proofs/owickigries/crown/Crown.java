...
public final class Crown<PLACE, LETTER> {
	static final boolean IGNORE_CUTOFF_CONDITIONS = true;
	...
	private boolean crownContainsAllCuts() {
		...
		final var branchingProcessCuts = mBp.computeCuts(IGNORE_CUTOFF_CONDITIONS);
	...
	public CrownsEmpire<PLACE, LETTER> getCrownsEmpire(final BasicPredicateFactory factory,
		...
		return new CrownsEmpire<>(this, factory, placeToAssertion);
	}
	...
	public long getAssertionSize() {
		return mCrown.stream().collect(Collectors.summingLong(x -> x.getLaw().getConditions().size()));
	}
