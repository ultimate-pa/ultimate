...
public class EmpireValidityCheck<PLACE, LETTER extends IAction> {
	...
	private IPredicate getLawConjunction(final Set<Pair<Territory<PLACE>, IPredicate>> pairs) {
		final Set<IPredicate> lawSet = pairs.stream().map((Function<? super Pair<Territory<PLACE>, IPredicate>, ? extends IPredicate>) Pair::getSecond).collect(Collectors.toSet());
