 ...
 * 
 */
public class Svcomp17FoldersAutomizerOverflow extends AbstractBuchiAutomizerTestSuite {
...
//	private static final int FILES_PER_DIR_LIMIT = 2;
	...
	// @formatter:off
	private static final DirectoryFileEndingsPair[] mBenchmarks32bit = {
	...
	// @formatter:off
	private static final DirectoryFileEndingsPair[] mBenchmarks64bit = {
		/***** Category 4. Overflows *****/
	 ...
	 */
	private static final String[] mSettings32bit = {
		// @formatter:off
		"svcomp2017/automizer/svcomp-Overflow-32bit-Automizer_Default.epf",
		// @formatter:on
	};
	
	private static final String[] mSettings64bit = {
		// @formatter:off
	...
	public Collection<UltimateTestCase> createTestCases() {
		for (final DirectoryFileEndingsPair dfep : mBenchmarks32bit) {
			for (final String toolchain : mCToolchains) {
				addTestCase(UltimateRunDefinitionGenerator.getRunDefinitionsFromTrunkRegex(
						new String[]{dfep.getDirectory()}, dfep.getFileEndings(), mSettings32bit, 
						toolchain, dfep.getOffset(), dfep.getLimit()));
			}
		}
		for (final DirectoryFileEndingsPair dfep : mBenchmarks64bit) {
			for (final String toolchain : mCToolchains) {
				addTestCase(UltimateRunDefinitionGenerator.getRunDefinitionsFromTrunkRegex(
						new String[]{dfep.getDirectory()}, dfep.getFileEndings(), mSettings64bit, 
						toolchain, dfep.getOffset(), dfep.getLimit()));
			}
		}
