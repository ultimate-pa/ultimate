...
public class StateRecoverabilityGenerator {
	...
	public StateRecoverabilityGenerator(final ILogger logger, final IUltimateServiceProvider services, final IReqSymbolTable symboltable) {
	...
	public Map<VerificationExpression, Set<PeaPhaseProgramCounter>> getRelevantLocationsFromPea(List<ReqPeas> reqPeasList, VerificationExpressionContainer vec) {
			...
			for(Entry<CounterTrace, PhaseEventAutomata> entry : ctPeaList) {
				...
				for(int i = 0; i < phases.length; ++i) {
					...
					for(VerificationExpression ve : vec.getVerificationExpressions().values()) {
						CDD veCcd = BoogieBooleanExpressionDecision.create(createOppositeCondition(new DefaultLocation(), ve.getBoogiePrimitiveType(), ve.getVariable(), ve.getOperator(), ve.getValue()));
						...
						if(SmtUtils.checkSatTerm(mScript, termModel) == LBool.SAT) {
							...
							Term possibleStartPhase = possibleStartPhase(phase, reqPea, reqPeasTerm.get(reqPea), termVe);
	...
	private Map<ReqPeas, PhaseEventAutomata[]> createPeaArray(Set<ReqPeas> reqPeas) {
		...
		for(ReqPeas reqPea : reqPeas) {
			List<Entry<CounterTrace, PhaseEventAutomata>> ctPeaList = reqPea.getCounterTrace2Pea();
			Set<PhaseEventAutomata> automataSet = ctPeaList.stream().map(Entry<CounterTrace, PhaseEventAutomata>::getValue).collect(Collectors.toSet());
	...
	public Map<VerificationExpression, Map<PhaseEventAutomata, Set<StateRecoverabilityAuxStatement>>> getAuxStatementPerVerificationExpression(AuxStatementContainer auxStatementContainer) {
		Map<VerificationExpression, Map<PhaseEventAutomata, Set<StateRecoverabilityAuxStatement>>> vePeaAuxStatementMap = new HashMap<>();
		for(AuxStatement auxStatement : auxStatementContainer.getSreMap().values()) {
			if(auxStatement instanceof StateRecoverabilityAuxStatement) {
				...
				Map<PhaseEventAutomata, Set<StateRecoverabilityAuxStatement>> peaAuxStatementMap = vePeaAuxStatementMap.get(stRecAuxSt.getVerificationExpression());
				...
				Set<StateRecoverabilityAuxStatement> stRecAuxStSet = peaAuxStatementMap.get(stRecAuxSt.getPeaPhasePc().getPea());
	...
	public Expression createOppositeCondition(final ILocation loc, BoogieType boogieType, String lhs, String opr, String rhs) {
		String newOpr = "";
		switch (opr) {
        case "==":
            newOpr = "!=";
	...
	public Expression createExpression(final ILocation loc, BoogieType boogieType, String sLhs, Operator opr, String sRhs) {
		Expression rhs = null;
		Expression lhs;
	...
	public Expression createExpression(final ILocation loc, BoogieType boogieType, String sLhs, String sOpr, String sRhs) {
		Expression rhs = null;
		Expression lhs;
