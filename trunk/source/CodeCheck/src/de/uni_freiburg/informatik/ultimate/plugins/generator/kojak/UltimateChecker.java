	...
	private void splitNode_EdgeChecker(AnnotatedProgramPoint oldNode, IPredicate predicate) {
		...
		for (AppEdge oldInEdge : oldInEdges) {
			...
			if (oldInEdge instanceof AppHyperEdge) {
				...
				// connectOutgoingReturnIfSat(source, hier, (Return) statement,
				// newNode1);
				// connectOutgoingReturnIfSat(source, hier, (Return) statement,
				// newNode2);
			...
			// if (oldInEdge instanceof AppHyperEdge) {
			// AnnotatedProgramPoint hier = ((AppHyperEdge)
			// oldInEdge).getHier();
			// connectOutgoingReturnIfSat(source, hier, (Return) statement,
			// newNode1);
			// connectOutgoingReturnIfSat(source, hier, (Return) statement,
		...
		for (int i = 0; i < 2; i++) {
			...
			for (AppEdge oldOutEdge : oldOutEdges) {
				...
				if (oldOutEdge instanceof AppHyperEdge) {
					...
					// connectOutgoingReturnIfSat(newNode2, hier, (Return)
					// statement, target);
					_edgeChecker.unAssertHierPred();
				} else {
					if (statement instanceof Call) {
				...
				// AnnotatedProgramPoint hier = ((AppHyperEdge)
				// oldOutEdge).getHier();
				// connectOutgoingReturnIfSat(newNode1, hier, (Return)
				// statement, target);
				// } else {
				// connectOutgoingIfSat(newNode1, statement, target);
				// }
		...
		for (int i = 0; i < 2; i++) {
			...
			for (AppEdge oldOutEdge : oldOutEdges) {
				AnnotatedProgramPoint target = oldOutEdge.getTarget();
				CodeBlock statement = oldOutEdge.getStatement();
				// already dealt with non self loops and disconnected those
				// edges
				if (target == null)
					continue;
		...
		if (GlobalSettings._instance._predicateUnification != PredicateUnification.NONE)
			if (source.getPredicate().equals(this.m_falsePredicate))
				...
				satResult = false;
					...
					target.getPredicate())) == LBool.SAT)
				satResult = true;
			else if (isSatEdge(source.getPredicate(), statement, target.getPredicate()))
				satResult = true;
			else if (m_smtManager.checkSatWithFreeVars(source.getPredicate().getFormula()) == LBool.UNSAT)
				satResult = false;
			else if (statement.getTransitionFormula().isInfeasible() == Infeasibility.INFEASIBLE)
				satResult = false;
		...
		if (GlobalSettings._instance._predicateUnification != PredicateUnification.NONE)
			if (source.getPredicate().equals(this.m_falsePredicate))
				...
				satResult = false;
																			...
																			// implicitly
				satResult = false;
			else if (_edgeChecker.sdecReturn(source.getPredicate(), hier.getPredicate(), statement,
					target.getPredicate()) == LBool.SAT)
				satResult = true;
			else if (isSatRetEdge(source.getPredicate(), hier.getPredicate(), statement, target.getPredicate()))
				satResult = true;
			else if (m_smtManager.checkSatWithFreeVars(source.getPredicate().getFormula()) == LBool.UNSAT)
				satResult = false;
