...
public class BuchiNwaComplement extends BuchiNwa implements IBuchiNwaComplement {
	...
	public BuchiNwaComplement(final IBuchiNwa buchi) {
		super(buchi.getAlphabetCall(), buchi.getAlphabetInternal(), buchi.getAlphabetReturn());
		mOperand = buchi;
		mDeckerMap = new TObjectIntHashMap<>();
		mDeckerList = new ArrayList<>();
		mFinalDeckers = UtilIntSet.newIntSet();
		computeInitialStates();
	}

	private void computeInitialStates() {
		final IntSet upC = mOperand.getInitialStates().clone();
		upC.and(mOperand.getFinalStates()); // goto C
		final IntSet upN = mOperand.getInitialStates().clone();
		upN.andNot(upC);
		final IntSet N = generateDeckers(DoubleDecker.EMPTY_DOWN_STATE, upN);
		final IntSet C = generateDeckers(DoubleDecker.EMPTY_DOWN_STATE, upC);
		final NCSB ncsb = new NCSB(N, C, UtilIntSet.newIntSet(), C);
		final StateNwaNCSB state = new StateNwaNCSB(this, 0, ncsb);
		if (C.isEmpty()) {
			this.setFinal(0);
		}
		this.setInitial(0);
		final int id = this.addState(state);
		mStateIndices.put(state, id);
	}

	protected StateNwaNCSB addState(final NCSB ncsb) {
		...
		if (mStateIndices.containsKey(state)) {
			...
			final int index = getStateSize();
			final StateNwaNCSB newState = new StateNwaNCSB(this, index, ncsb);
			final int id = this.addState(newState);
	...
	@Override
	public DoubleDecker getDoubleDecker(final int id) {
		assert id < mDeckerList.size();
		return mDeckerList.get(id);
	}

	@Override
	public int getDoubleDeckerId(final DoubleDecker decker) {
		if (mDeckerMap.containsKey(decker)) {
			return mDeckerMap.get(decker);
		}
		final int id = mDeckerList.size();
	...
	protected IntSet generateDeckers(final int downState, final IntSet upStates) {
	...
	public int getUpState(final int decker) {
		return getDoubleDecker(decker).getUpState();
	}

	public int getDownState(final int decker) {
	...
	public void explore() {
		...
		final LinkedList<IStateNwa> walkList = new LinkedList<>();
		...
		final Set<Integer> callPreds = new HashSet<>();

		while (!walkList.isEmpty()) {
			final IStateNwa s = walkList.remove();
			...
			// call alphabets
			final IntSet calls = mOperand.getAlphabetCall();
			IntIterator iterLetter = calls.iterator();
			while (iterLetter.hasNext()) {
				final int letter = iterLetter.next();
				final IntSet succs = s.getSuccessorsCall(letter);
				if (!succs.isEmpty()) {
			...
			while (iterLetter.hasNext()) {
				...
				final IntSet succs = s.getSuccessorsInternal(letter);
				exploreSuccessors(s, walkList, succs, visited, letter);
			}
			...
			while (iterLetter.hasNext()) {
				...
				final int letter = iterLetter.next();
				//
				final int size = getStateSize();
				for (int i = 0; i < size; i++) {
					for (final Integer hier : callPreds) {
						final IStateNwa state = getState(i);
						if (Options.verbose) {
							System.out.println("current: " + state.toString() + "  hier: " + hier + " :"
									+ getState(hier).toString());
						}
						final IntSet succs = state.getSuccessorsReturn(hier, letter);
	...
	private void exploreSuccessors(final IStateNwa s, final LinkedList<IStateNwa> walkList, final IntSet succs,
			final BitSet visited, final int letter) {
