...
public interface ISemanticReducerFactory<STATE, LETTER> extends IStateFactory<STATE> {
	 ...
	 *            applications (Mostafa, Alex, 17.1.2018) possible reason to keep the parameter: if a future
	 *            implementation needs the original set of successors/destinations Mostafa: DummySemanticReducer is
	 *            using the destination states, so we can keep it in the interface
	 * @return
	 */
	Iterable<STATE> getOptimalDestination(final Iterable<STATE> states, final List<STATE> src, final LETTER letter,
			final Iterable<STATE> dest);
	 ...
	 * @return
	 */
	default <LETTER extends IRankedLetter> Iterable<TreeAutomatonRule<LETTER, STATE>>
			reduceRules(final Iterable<TreeAutomatonRule<LETTER, STATE>> oldRules) {
