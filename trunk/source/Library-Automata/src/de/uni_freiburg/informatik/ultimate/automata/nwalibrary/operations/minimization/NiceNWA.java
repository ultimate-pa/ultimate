 ...
 * There is no distinction between linear and hierarchical states.
 * 
 * <p>This is meant only as a plain old data (POD) container of mostly
 * non-redundant data.
 * 
...
class NiceNWA implements Cloneable {
	...
	/**
	 * TODO: this is not super cheap (memory)
	 * 
	 * <p>NOTE: answers <code>false</code> even if all non-deterministic
	 * transitions are completely duplicate transitions, i.e. have the same
	 * destination state.
	 * 
	 * @param nwa readonly <code>NiceNWA</code> which is consistent as by
	 *        <code>checkConsistency()</code>
	 * @return <code>true</code> iff the automaton is deterministic.
	 */
	...
	/**
	 * @return A copy of this NiceNWA instance. Since NiceNWA is a plain old
	 *         data type and all its fields are plain old data types as well,
	 *         no references are shared and you are free to modify the copy
	 *         any way you like.
	 */
