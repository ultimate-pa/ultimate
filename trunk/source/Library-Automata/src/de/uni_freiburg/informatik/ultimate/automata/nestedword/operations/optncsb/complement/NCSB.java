...
/**
 * NCSB tuple TODO: in order to make it unmodifiable
 */
public class NCSB {
	...
	public NCSB(final IntSet N, final IntSet C, final IntSet S, final IntSet B) {
		mNSet = N;
		mCSet = C;
		mSSet = S;
		mBSet = B;
	...
	public NCSB() {
		mNSet = UtilIntSet.newIntSet();
		mCSet = UtilIntSet.newIntSet();
		mSSet = UtilIntSet.newIntSet();
		mBSet = UtilIntSet.newIntSet();
	}
	...
	private boolean contentEqual(final NCSB ncsb) {
		if (!mNSet.equals(ncsb.mNSet) || !mCSet.equals(ncsb.mCSet) || !mSSet.equals(ncsb.mSSet)
	...
	public boolean coveredBy(final NCSB other) {
		if (Options.lazyS && !other.mBSet.subsetOf(mBSet)) {
			return false;
		}
		if (!other.mNSet.subsetOf(mNSet) || !other.mCSet.subsetOf(mCSet) || !other.mSSet.subsetOf(mSSet)) {
			return false;
		}
	...
	public boolean strictlyCoveredBy(final NCSB other) {
		if (!other.mNSet.subsetOf(mNSet) || !other.mCSet.subsetOf(mCSet) || !other.mSSet.subsetOf(mSSet)
	...
	public boolean subsetOf(final NCSB other) {
		if (mAllSets == null) {
			initializeAllSets();
		}
	...
	public String toString() {
		return "(" + mNSet.toString() + "," + mCSet.toString() + "," + mSSet.toString() + "," + mBSet.toString() + ")";
	}
	...
	private int hashValue(final IntSet set) {
		final int prime = 31;
		int result = 1;
