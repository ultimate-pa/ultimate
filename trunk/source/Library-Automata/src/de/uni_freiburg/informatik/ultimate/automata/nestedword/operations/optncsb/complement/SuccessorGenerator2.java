...
class SuccessorGenerator2 {
	...
	private final IntSet mF; // so far all final states

	private final IntSet mNPrime; // d(N)\F\B'\S'
	private final IntSet mVPrime; // d(C) \/ (d(N) /\ F)
	private final IntSet mSPrime; // d(S)
	private final IntSet mBPrime; // d(B)

	// private IntSet mNInterFSuccs;

	private final PowerSet mPs;

	private boolean hasSuccessors = true;

	public SuccessorGenerator2(final boolean isCurrBEmpty, final NCSB succ, final IntSet minusFSuccs,
			final IntSet interFSuccs, final IntSet f) {
		mIsCurrBEmpty = isCurrBEmpty;
		mSuccNCSB = succ;

		mMinusFSuccs = minusFSuccs;
		mInterFSuccs = interFSuccs;
		mF = f;

		// initialization
		// N'
		mNPrime = mSuccNCSB.copyNSet();
		...
		final IntSet nInterFSuccs = mSuccNCSB.copyNSet();
	...
	public NCSB next() {
		final IntSet Sextra = mPs.next(); // extra states to be added into S'

		// this is implementation for NCSB
		final IntSet NP = mNPrime;
		IntSet CP = null;
		final IntSet SP = mSPrime.clone();
		IntSet BP = null;
