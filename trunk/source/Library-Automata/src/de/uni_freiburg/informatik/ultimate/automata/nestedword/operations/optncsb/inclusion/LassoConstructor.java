...
class LassoConstructor<LETTER, STATE> {
	...
	LassoConstructor(final AutomataLibraryServices services,
			final AbstractGeneralizedAutomatonReachableStates<LETTER, STATE> reach, final List<STATE> sccList)
	...
	private void constructLoopRun() throws AutomataOperationCanceledException {
		...
		final LinkedList<STATE> newList = new LinkedList<>();
		boolean preInsertion = true;
		for (final STATE current : mSCC) {
			if (preInsertion) {
		...
		final Set<Integer> labels = new HashSet<>(mReach.getAcceptanceLabels(stateCont.getState()));
		STATE backState = mGoal.getState();
		for (final STATE current : newList) {
			testTimeoutCancelException(this.getClass());
			final LETTER letter = stateCont.getLetterOfSuccessor(current);
			NestedRun<LETTER, STATE> newSuffix;
			if (letter != null) {
				// path exists
				labels.addAll(mReach.getAcceptanceLabels(current));
				newSuffix = new NestedRun<>(stateCont.getState(), letter, NestedWord.INTERNAL_POSITION, current);
			} else {
				...
				target.add(current);
				final RunConstructor rc = new RunConstructor(source, target, true);
		...
		if (backState.equals(mGoal.getState())) {
			final LETTER letter = mGoal.getLetterOfSuccessor(backState);
			assert letter != null;
			mLoop = new NestedRun<>(backState, letter, NestedWord.INTERNAL_POSITION, backState);
			return;
		}
		final Set<STATE> source = new HashSet<>();
		final Set<STATE> target = new HashSet<>();
		...
		final RunConstructor rc = new RunConstructor(source, target, true);
		final NestedRun<LETTER, STATE> newSuffix = rc.getNestedRun();
		mLoop = mLoop.concatenate(newSuffix);
	}

	private void constructStemRun() throws AutomataOperationCanceledException {
		// construct a run from initial state to mGoal
		final RunConstructor rc = new RunConstructor(mReach.getInitialStates(), mSCCSet, false);
	...
	private void testTimeoutCancelException(final Class<?> clazz) throws AutomataOperationCanceledException {
	...
	private class SuccessorInfo {
		...
		SuccessorInfo(final STATE state) {
			mState = state;
			mPredState = null;
			mLetter = null;
	...
	private class ComparatorSuccessorInfo implements Comparator<SuccessorInfo> {
		@Override
		public int compare(final LassoConstructor<LETTER, STATE>.SuccessorInfo fstSuccInfo,
				final LassoConstructor<LETTER, STATE>.SuccessorInfo sndSuccInfo) {
			return fstSuccInfo.mDistance - sndSuccInfo.mDistance;
		}
	}

	/**
	 * We assume that there must be a run from some state in mSources to some state in mTargets
	 */
	private class RunConstructor {
		...
		RunConstructor(final Set<STATE> sources, final Set<STATE> targets, final boolean isLoop) {
			mSources = sources;
			mTargets = targets;
			mIsLoop = isLoop;
			if (mIsLoop) {
		...
		private SuccessorInfo getSuccessorInfoPrivate(final STATE succ) {
		...
		private void constructRunBackwards() throws AutomataOperationCanceledException {
			...
			while (!mSources.contains(currInfo.mState)) {
				...
				final NestedRun<LETTER, STATE> newPrefix = new NestedRun<>(currInfo.mPredState, currInfo.mLetter,
		...
		private void breathFirstSearch() throws AutomataOperationCanceledException {
			// construct a run from initial state to mGoal
			final PriorityQueue<SuccessorInfo> queue = new PriorityQueue<>(new ComparatorSuccessorInfo());
			// initialize sources
			for (final STATE state : mSources) {
				final SuccessorInfo succInfo = getSuccessorInfoPrivate(state);
			...
			while (!queue.isEmpty()) {
				testTimeoutCancelException(this.getClass());
				// current state
				final SuccessorInfo currInfo = queue.remove();
				...
				final StateContainer<LETTER, STATE> stateCont = mReach.getStateContainer(currInfo.mState);
				for (final OutgoingInternalTransition<LETTER, STATE> trans : stateCont.internalSuccessors()) {
					testTimeoutCancelException(this.getClass());
					final int dis = currInfo.mDistance + 1;
					final SuccessorInfo succInfo = getSuccessorInfoPrivate(trans.getSucc());
