...
/**
 * Creates a random possibly non-deterministic finite bottom-up tree automaton (NFTA-BU) using the helper class
...
public final class GetRandomNftaBU extends AGetRandomFtaBU {
	 ...
	 *            compute the upper bound of maximal possible transitions which is {@code states^(rank + 1)}. The
	 *            computation is done exact thus if numbers are big the method will take long.
	 * @return The converted data structure where each value lists the absolute number of transitions per letter instead
	 *         of their densities.
	 * @throws ArithmeticException
	 *             If the computation of {@code states^(rank + 1)} does not fit into the size of an {@link Integer}.
	 */
	...
	/**
	 * Constructor of a non-deterministic finite tree automaton for the {@code TestFileInterpreter}. This method uses a
	 ...
	 * {@link #GetRandomNftaBU(AutomataLibraryServices, int, int[], double[], double, long)} if a seed should be
	 * specified.
	 *
	 * @param services
	 *            Ultimate services
	 ...
	 *            transitions. With a full density {@code states^(rank + 1)} transitions will be created for each
	 *            letter. In this case the other constructor
	 *            {@link #GetRandomNftaBU(AutomataLibraryServices, int, int[], int[], double)} may be more comfortable
	...
	/**
	 * Constructor of a non-deterministic finite tree automaton for the {@code TestFileInterpreter}.
	 *
	 * @param services
	 *            Ultimate services
	 * @param numberOfStates
	 ...
	 *            transitions. With a full density {@code states^(rank + 1)} transitions will be created for each
	 *            letter. In this case the other constructor
	 *            {@link #GetRandomNftaBU(AutomataLibraryServices, int, int[], int[], double, long)} may be more
	 ...
	 * default seed that is not intended to change thus the result of this method, for same arguments, will always
	 * remain the same even in different JVM cycles. Use
	 * {@link #GetRandomNftaBU(AutomataLibraryServices, int, int[], int[], double, long)} if a seed should be specified.
	 *
	 * @param services
	 *            Ultimate services
	 * @param numberOfStates
	 ...
	 *            which is given by {@code states^(rank + 1)}. For efficiency reasons this object will not check
	 *            validity for those upper bounds. If setting higher values it is possible that this object never
