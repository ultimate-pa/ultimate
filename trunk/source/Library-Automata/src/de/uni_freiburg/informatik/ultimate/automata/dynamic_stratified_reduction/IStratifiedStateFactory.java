 ...
 *            an upper limit for the abstraction level and a list of predecessors of the state that are safe to loop back to
 * @param <H>
...
public interface IStratifiedStateFactory<L,S,R, H> extends IEmptyStackStateFactory<R> {
	...
	// Returns the abstraction limit of a reduction state (is the upper limit for the abstraction level of all reduction states reachable from state)
...
/**
 *Class implementing the interface, uses the standard (and only existing) type of dynamic stratified reduction state and abstraction level.
 * 
 *
 * @param <L>
...
class StratifiedStateFactory<L,S> implements IStratifiedStateFactory<L,S, StratifiedReductionState<L,S>, AbstractionLevel<L>>{
	...
	@Override
	public StratifiedReductionState<L, S> createStratifiedState(S state, ImmutableSet<L> sleepset,
			AbstractionLevel<L> level, AbstractionLevel<L> limit,
			LinkedList<StratifiedReductionState<L, S>> loopPredecs) {

		return new  StratifiedReductionState<L,S>(state, sleepset, level, limit, loopPredecs);
	}

	@Override
	public S getOriginalState(StratifiedReductionState<L, S> state) {
		return state.mOriginalState;
	}
	
	@Override
	public ImmutableSet<L> getSleepSet(StratifiedReductionState<L, S> state) {
		return state.mSleepSet;
	}

	@Override
	public AbstractionLevel<L> getAbstractionLevel(StratifiedReductionState<L, S> state) {
		return state.mAbstractionLevel;
	}

	@Override
	public void addToAbstractionLevel(StratifiedReductionState<L, S> state, HashSet<L> variables) {
		state.mAbstractionLevel.addToAbstractionLevel(variables);
		
	}

	@Override
	public AbstractionLevel<L> getAbstractionLimit(StratifiedReductionState<L, S> state) {
		return state.mAbstractionLimit;
	}

	
	@Override
	public void addToAbstractionLimit(StratifiedReductionState<L, S> state, HashSet<L> variables) {
		state.mAbstractionLimit.addToAbstractionLevel(variables);
	}

	@Override
	public LinkedList<StratifiedReductionState<L, S>> getLoopablePredecs(StratifiedReductionState<L, S> state) {
		return state.mLoopablePredecs;
	}
	
	public boolean isLoopCopy(StratifiedReductionState<L,S> state) {
 ...
 * 		an object with a set of program variables 'value' and a boolean 'locked'
 * 		locked = true means that the abstraction level is fully defined and no more variables will be added to its value
 * @param abstractionLimit
 * 		an object with a set of program variables 'value' and a boolean 'locked'
 * 		locked = true means that the state with this abstraction limit is part of a loop copy with this abstraction limit 
 * 
 * @return
 * 		the corresponding state of the reduction automaton 
 */
...
class StratifiedReductionState<L,S> {
	...
	public StratifiedReductionState(S state, ImmutableSet<L> sleepset, AbstractionLevel<L> absLv, AbstractionLevel<L> absLmt, LinkedList<StratifiedReductionState<L,S>> loopPredecs) {
