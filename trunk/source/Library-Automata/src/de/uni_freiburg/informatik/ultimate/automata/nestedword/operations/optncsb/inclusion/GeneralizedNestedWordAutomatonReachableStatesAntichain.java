...
public class GeneralizedNestedWordAutomatonReachableStatesAntichain<LETTER, STATE>
	...
	public <FACTORY extends IBuchiIntersectStateFactory<STATE> & IEmptyStackStateFactory<STATE>> GeneralizedNestedWordAutomatonReachableStatesAntichain(
	...
	private void print(final String name, final INwaOutgoingLetterAndTransitionProvider<LETTER, STATE> operand) {
		try {
			final PrintWriter out = new PrintWriter(new FileWriter("./" + name));
			final List<STATE> workList = new ArrayList<>();
			for (final STATE state : operand.getInitialStates()) {
				out.println("Initial States: " + mSndOperand.getNCSB(state) + " final: " + mSndOperand.isFinal(state));
				workList.add(state);
			}
			final Set<STATE> visited = new HashSet<>();
			final Map<LETTER, Integer> alphabet = new HashMap<>();
			int num = 0;
			...
			while (!workList.isEmpty()) {
				final STATE curr = workList.remove(workList.size() - 1);
				...
				for (final OutgoingInternalTransition<LETTER, STATE> trans : operand.internalSuccessors(curr)) {
	...
	private boolean reachable(final STATE source, final STATE target,
	...
	class ReachableStatesComputation {
		...
		public ReachableStatesComputation() throws AutomataOperationCanceledException {
			...
			//
			// System.err.println(mSndOperand.getNCSB(cover) + "->" + mSndOperand.getNCSB(covered) + ":" +
			// reachable(cover, covered, remove.getResult()));
			// System.err.println(mSndOperand.getNCSB(covered) + "->" + mSndOperand.getNCSB(cover) + ":" +
			// reachable(covered, cover, remove.getResult()));
			...
			// assert false : "Wrong Coverage: " + mStates.get(st).mProdState.mFst + ", " +
			...
			for (final STATE st : states) {
				...
				if (mEmp.covers(mStates.get(st).mProdState)) {
					// remove states
					final StateContainer<LETTER, STATE> cont = mStates.get(st);
					...
					for (final IncomingInternalTransition<LETTER, STATE> trans : cont.internalPredecessors()) {
						...
						final StateContainer<LETTER, STATE> predCont = mStates.get(trans.getPred());
						if (predCont != null) {
		...
		boolean construct(final STATE state) throws AutomataOperationCanceledException {
			...
			for (final OutgoingInternalTransition<LETTER, STATE> fstTrans : mFstOperand
				...
				for (final OutgoingInternalTransition<LETTER, STATE> sndTrans : mSndOperand
					...
					if (mQPrime.contains(succState)) {
						is_nemp = true;
					} else if (mEmp.covers(succProd)) {
						continue;
						...
						STATE u;
						do {
							final AsccPair<LETTER, STATE> pair = mSCCs.pop();
							u = pair.mState;
			...
			if (mSCCs.peek().mState.equals(state)) {
				final AsccPair<LETTER, STATE> pair = mSCCs.pop(); // remove the root
				STATE u;
	...
	private class Antichain {
		...
		public ProductState coveringProductState(final ProductState state) {
			final List<ProductState> sndElem = mResStateMap.get(state.getFst());
			if (sndElem == null) {
				return null;
			}
