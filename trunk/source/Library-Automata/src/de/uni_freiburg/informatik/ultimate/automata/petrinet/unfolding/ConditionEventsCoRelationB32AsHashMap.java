 ...
 *            place content type
 */
public class ConditionEventsCoRelationB32AsHashMap<LETTER, PLACE> implements ICoRelation<LETTER, PLACE> {
	...
	private final HashMap <Condition<LETTER, PLACE>, Set<Event< LETTER, PLACE>>> coRelatedCutoffEvents = new HashMap<>();
	private final HashMap <Condition<LETTER, PLACE>, Set<Event< LETTER, PLACE>>> coRelatedNonCutoffEvents = new HashMap<>();
	 ...
	 *            branching process
	 */
	public ConditionEventsCoRelationB32AsHashMap(final BranchingProcess<LETTER, PLACE> branchingProcess) {
	...
	public void initialize(final Set<Condition<LETTER, PLACE>> initialConditions) {
		for (final Condition<LETTER, PLACE> c: initialConditions) {
			coRelatedCutoffEvents.put(c, new HashSet<>());
			coRelatedNonCutoffEvents.put(c, new HashSet<>());
		}
	}

	private Stream<Event<LETTER, PLACE>> streamCoRelatedEvents(final Condition<LETTER, PLACE> c) {
		return Stream.concat(coRelatedCutoffEvents.get(c).stream(),
				coRelatedNonCutoffEvents.get(c).stream());
	}
	
	private Stream<Event<LETTER, PLACE>> streamNonCutoffCoRelatedEvents(final Condition<LETTER, PLACE> c) {
		return coRelatedNonCutoffEvents.get(c).stream();
	}
	...
	public void update(final Event<LETTER, PLACE> e) {
		...
		final HashMap<Condition<LETTER, PLACE>, Set<Event<LETTER, PLACE>>> mapContainingE;
		if (e.isCutoffEvent()) {
	...
	private void updateSingleMap(final HashMap<Condition<LETTER, PLACE>, Set<Event<LETTER, PLACE>>> mapToUpdate,
	final HashMap<Condition<LETTER, PLACE>, Set<Event<LETTER, PLACE>>> mapContainingE,
	final Event<LETTER,PLACE> e) {
		final Set<Event<LETTER, PLACE>>[] coRelatedEventsToE = e.getPredecessorConditions().stream()
				.map(x -> mapToUpdate.get(x)).toArray(Set[]::new);
		...
		for (final Condition<LETTER, PLACE> c : e.getSuccessorConditions()) {
			mapToUpdate.put(c, new HashSet<>(intersection));
		}
	}

	@Override
	public boolean isInCoRelation(final Condition<LETTER, PLACE> c1, final Condition<LETTER, PLACE> c2) {
		final boolean result = coRelatedNonCutoffEvents.get(c1).contains(c2.getPredecessorEvent())
				|| coRelatedCutoffEvents.get(c1).contains(c2.getPredecessorEvent())
	...
	public int computeMaximalDegree() {
		...
		final Integer max = coRelatedCutoffEvents.keySet().stream().map(computeDegree).max(Integer::compare)
	...
	public Set<Condition<LETTER, PLACE>> computeCoRelatatedConditions(final Condition<LETTER, PLACE> cond,
			final PLACE p) {
		return streamCoRelatedConditions(cond).filter(x -> x.getPlace().equals(p))
