...
public abstract class AbstractGeneralizedBuchiDifference<LETTER, STATE>
	...
	protected <SF extends IBuchiComplementNcsbStateFactory<STATE> & IBuchiIntersectStateFactory<STATE> & IEmptyStackStateFactory<STATE>>
			void constructResult(final SF stateFactory, final boolean lazyOptimization)
					throws AutomataLibraryException {
		// first we get the complement
		final BuchiComplementNCSBSimpleNwa<LETTER, STATE> onDemandComplemented =
				new BuchiComplementNCSBSimpleNwa<>(mServices, stateFactory, mSndOperand);
		Options.lazyS = lazyOptimization;
		Options.lazyB = false;
		mSndComplemented = new ComplementNwaOutgoingLetterAndTransitionAdapter<>(onDemandComplemented);
		constructDifferenceFromComplement(stateFactory);
	}

	protected abstract <SF extends IBuchiIntersectStateFactory<STATE> & IEmptyStackStateFactory<STATE>> void
			constructDifferenceFromComplement(SF stateFactory) throws AutomataLibraryException;
	...
	public boolean checkResult(final IStateFactory<STATE> stateFactory) throws AutomataLibraryException {
		...
		final GeneralizedBuchiIsEmpty<LETTER, STATE> fstOperandEmptiness =
				new GeneralizedBuchiIsEmpty<>(mServices, mFstOperand);
		...
		final GeneralizedBuchiIsEmpty<LETTER, STATE> resultEmptiness =
				new GeneralizedBuchiIsEmpty<>(mServices, mResult);
