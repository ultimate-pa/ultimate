...
public class ComplementTest<LETTER, STATE> {
	...
	public <SF extends IBuchiIntersectStateFactory<STATE> & IEmptyStackStateFactory<STATE>> ComplementTest(
	...
	private <SF extends IBuchiIntersectStateFactory<STATE> & IEmptyStackStateFactory<STATE>> void
			testComplement(final SF stateFactory) throws AutomataLibraryException {
		final IBuchiComplementNcsbStateFactory<STATE> ncsbSf = (IBuchiComplementNcsbStateFactory<STATE>) stateFactory;
		final BuchiComplementNCSBLazyNwa<LETTER, STATE> onDemandComplemented =
				new BuchiComplementNCSBLazyNwa<>(mServices, ncsbSf, mOperand);
		Options.lazyB = false;
		Options.lazyS = true;
		...
		final BuchiComplementNCSBNwa<LETTER, STATE> complemented =
				new BuchiComplementNCSBNwa<>(mServices, ncsbSf, mOperand, false);
		mSndComplementedStandard = new NwaOutgoingLetterAndTransitionAdapter<>(complemented);
		mNumber++;
		new AutomatonDefinitionPrinter<String, String>(mServices, "complement", "./complement" + mNumber + "_1",
				Format.BA, "", mSndComplemented);
		new AutomatonDefinitionPrinter<String, String>(mServices, "complement", "./complement" + mNumber + "_2",
				Format.BA, "", mSndComplementedStandard);
	}
