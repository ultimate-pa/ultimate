...
public class PetriNetUnfolderRabin<LETTER, PLACE> extends PetriNetUnfolderBase<LETTER, PLACE> {
		...
		if (event.isCutoffEvent() && event.getLocalConfiguration().contains(event.getCompanion())) {
			...
			for (final Event<LETTER, PLACE> loopEvent : configLoopEvents) {
				for (final Condition<LETTER, PLACE> condition : loopEvent.getSuccessorConditions()) {
					if (((IRabinPetriNet<LETTER, PLACE>) mOperand).isFinite(condition.getPlace())) {
						return false;
					}
				}
			}
