...
public class BuchiWaComplement extends BuchiWa implements IBuchiWaComplement {
	...
	public BuchiWaComplement(final IBuchiWa buchi) {
	...
	private void computeInitialStates() {
		final IntSet C = mOperand.getInitialStates().clone();
		C.and(mOperand.getFinalStates()); // goto C
		final IntSet N = mOperand.getInitialStates().clone();
		N.andNot(C);
		final NCSB ncsb = new NCSB(N, C, UtilIntSet.newIntSet(), C);
		final StateWaNCSB state = new StateWaNCSB(this, 0, ncsb);
		if (C.isEmpty()) {
			this.setFinal(0);
		}
		this.setInitial(0);
	...
	protected StateWaNCSB addState(final NCSB ncsb) {
		...
		if (mStateIndices.containsKey(state)) {
			...
			final StateWaNCSB newState = new StateWaNCSB(this, index, ncsb);
		...
		while (!walkList.isEmpty()) {
			...
			for (int i = 0; i < mOperand.getAlphabetSize(); i++) {
				...
				while (iter.hasNext()) {
					...
					if (Options.verbose) {
						System.out.println(
								" s" + s.getId() + ": " + s.toString() + "- L" + i + " -> s" + n + ": " + getState(n));
					}
					if (!visited.get(n)) {
