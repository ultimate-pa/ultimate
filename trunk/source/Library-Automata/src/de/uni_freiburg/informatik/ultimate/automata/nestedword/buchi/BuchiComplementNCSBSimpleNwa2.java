...
public class BuchiComplementNCSBSimpleNwa2<LETTER, STATE> implements INwaSuccessorStateProvider<LETTER, STATE> {
	...
	public BuchiComplementNCSBSimpleNwa2(final AutomataLibraryServices services,
		...
		// call alphabet
		final VpAlphabet<LETTER> vp = operand.getVpAlphabet();

		final IntSet callAlphabet = UtilIntSet.newIntSet();
		for (final LETTER letter : vp.getCallAlphabet()) {
			mLetterIdMap.put(letter, id);
			callAlphabet.set(id);
			id++;
		}
		final IntSet internalAlphabet = UtilIntSet.newIntSet();
		for (final LETTER letter : vp.getInternalAlphabet()) {
			mLetterIdMap.put(letter, id);
			internalAlphabet.set(id);
			id++;
		}
		final IntSet returnAlphabet = UtilIntSet.newIntSet();
		for (final LETTER letter : vp.getReturnAlphabet()) {
			mLetterIdMap.put(letter, id);
		...
		mOperandBuchi = new NwaToBuchiWrapper<>(callAlphabet, internalAlphabet, returnAlphabet, mLetterIdMap, operand);
	...
	private StateWithRankInfo<STATE> getStackState(final int downState) {
		StateWithRankInfo<STATE> stackState = null;
		if (downState == DoubleDecker.EMPTY_DOWN_STATE) {
			stackState = mEmptyStackStateWri;
		} else {
			stackState = new StateWithRankInfo<>(mOperandBuchi.getNwaSTATE(downState));
		}
		return stackState;
	}
	...
	protected LevelRankingState<LETTER, STATE> constructLevelRankingState(final int sid) {
		...
		final StateNwaNCSB state = (StateNwaNCSB) mComplementBuchi.getState(sid);
	...
	private void addLevelRankingState(final LevelRankingState<LETTER, STATE> lvlrk, final IntSet states, final int rank,
		...
		while (iterSet.hasNext()) {
			final int decker = iterSet.next();
			final int downState = mComplementBuchi.getDownState(decker);
			final int upState = mComplementBuchi.getUpState(decker);
	...
	public Collection<STATE> internalSuccessors(final STATE state, final LETTER letter) {
		...
		final StateNwaNCSB s = (StateNwaNCSB) mComplementBuchi.getState(mStateIdMap.get(state));
		final IntSet succs = s.getSuccessorsInternal(letterId);
	...
	public Collection<STATE> callSuccessors(final STATE state, final LETTER letter) {
		final int letterId = mLetterIdMap.get(letter);
		final StateNwaNCSB s = (StateNwaNCSB) mComplementBuchi.getState(mStateIdMap.get(state));
		final IntSet succs = s.getSuccessorsCall(letterId);
		return computeSuccessors(succs);
	}

	private Collection<STATE> computeSuccessors(final IntSet succs) {
	...
	public Collection<STATE> returnSuccessorsGivenHier(final STATE state, final STATE hier, final LETTER letter) {
		final int letterId = mLetterIdMap.get(letter);
		final StateNwaNCSB s = (StateNwaNCSB) mComplementBuchi.getState(mStateIdMap.get(state));
		final IntSet succs = s.getSuccessorsReturn(mStateIdMap.get(hier), letterId);
		return computeSuccessors(succs);
	}
