...
/**
 * In this stack, we mark the number of appearances for each element in the stack
 */
...
public class MarkedIntStack {
	...
	private final TIntIntMap mInStackCounter; // no duplicate elements
	private final boolean mMarked; // set true will be fast but more memory usage

	public MarkedIntStack(final boolean marked) {
	...
	public MarkedIntStack(final int initCapacity, final boolean marked) {
		if (initCapacity < 0) {
	...
	public MarkedIntStack(final int initCapacity) {
		this(initCapacity, true);
	}

	@Override
	public MarkedIntStack clone() {
		final MarkedIntStack result = new MarkedIntStack(mData.length, mMarked);
	...
	private void decreaseCounter(final int item) {
		if (!mMarked) {
			return;
		}
	...
	private boolean increaseCounter(final int item) {
		if (!mMarked) {
			return false;
		}
	...
	public IntSet getItems() {
		...
		if (mMarked) {
			final IntSet temp = new IntSetTIntSet(mInStackCounter.keySet());
