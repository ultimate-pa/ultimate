...
class AbstractionLevel<L> {
	...
	public AbstractionLevel(final HashSet<L> protectedVar, final boolean defined) {
		mValue = protectedVar;
		mLocked = defined;
	}
	...
	public boolean isEqual(final AbstractionLevel<L> level) {
		// return true if the two sets of variables are the same
		// assert (this.alreadyDefined() && level.alreadyDefined()): "Only fully defined abstraction levels can be
		// compared for equality";
		return this.getValue().equals(level.getValue());
	}

	public boolean isLEQ(final AbstractionLevel<L> level) {
		/*
		 * return true if the second set is a subset of the first, i.e. if the abstraction level of the first argument
		 * is less or equal than the second set's level. If the 1st abstraction level is not completly defined but
		 * already a superset of the 2nd abstraction level that's ok
		 */
