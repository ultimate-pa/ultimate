	...
	private class Bridge {
		...
		public String toString() {
			...
			for (final STATE source : mBridgeInOrder.keySet()) {
				for (final STATE target : mBridgeInOrder.get(source).keySet()) {
	...
	NestedRun<LETTER, STATE> reconstructionC(final STATE origin,
		...
		if (bridgeRange instanceof BuchiIsEmptyXW.QuadrupleBridge) {
			final STATE callPredecessor =
					((BuchiIsEmptyXW<LETTER, STATE>.QuadrupleBridge) bridgeRange).mCallPredecessor; // origin
			final STATE callSuccessor =
			...
			final STATE returnSuccessor =
					((BuchiIsEmptyXW<LETTER, STATE>.QuadrupleBridge) bridgeRange).mReturnSuccessor; // destination
			...
			final STATE singleton =
					((BuchiIsEmptyXW<LETTER, STATE>.SingletonBridge) bridgeRange).mSingleton;
