...
public class BuchiComplementNCSBSimpleNwa<LETTER, STATE> implements INwaSuccessorStateProvider<LETTER, STATE> {
	...
	public BuchiComplementNCSBSimpleNwa(final AutomataLibraryServices services,
		...
		for (final LETTER letter : operand.getAlphabet()) {
			mLetterIdMap.put(letter, id);
			id++;
		}
		mEmptyStackStateWri = new StateWithRankInfo<>(mSetOfStates.getEmptyStackState());
		mOperandBuchi = new WaToBuchiWrapper<>(mLetterIdMap.size(), mLetterIdMap, operand);
	...
	private void constructInitialState() {
		final IntSet initials = mComplementBuchi.getInitialStates();
		final IntIterator iter = initials.iterator();
		while (iter.hasNext()) {
			final int sId = iter.next();
			getOrAdd(true, sId);
		}
	}

	protected STATE getRelatedSTATE(final int sId) {
		final LevelRankingState<LETTER, STATE> lvlrk = constructLevelRankingState(sId);
		final STATE resState = mStateFactory.buchiComplementNcsb(lvlrk);
		mDet2res.put(lvlrk, resState);
		return resState;
	}

	public NCSB getRelatedNCSB(final STATE state) {
		final int id = mStateIdMap.get(state);
		final StateWaNCSB ncsbState = (StateWaNCSB) mComplementBuchi.getState(id);
	...
	protected LevelRankingState<LETTER, STATE> constructLevelRankingState(final int sid) {
		LevelRankingState<LETTER, STATE> lvlrk = mInt2LevelRanks.get(sid);
		if (lvlrk != null) {
			return lvlrk;
		}

		final StateWaNCSB state = (StateWaNCSB) mComplementBuchi.getState(sid);
		final NCSB ncsb = state.getNCSB();

		lvlrk = new LevelRankingState<>(mOperand);
		final IntSet B = ncsb.getBSet();
		// N
	...
	private void addLevelRankingState(final LevelRankingState<LETTER, STATE> lvlrk, final IntSet states, final int rank,
			final boolean isInO) {
		final IntIterator iterSet = states.iterator();
		while (iterSet.hasNext()) {
			final int n = iterSet.next();
	 ...
	 */
	private STATE getOrAdd(final boolean isInitial, final int sId) {
	...
	public Collection<STATE> internalSuccessors(final STATE state, final LETTER letter) {
		...
		final int letterId = mLetterIdMap.get(letter);
		final IStateWa s = mComplementBuchi.getState(mStateIdMap.get(state));
		final IntSet succs = s.getSuccessors(letterId);
		final IntIterator iter = succs.iterator();
		final List<STATE> computedSuccs = new ArrayList<>();
		while (iter.hasNext()) {
			final STATE succ = getOrAdd(false, iter.next());
	...
	public boolean coveredBy(final STATE fstState, final STATE sndState) {
		final int fstId = mStateIdMap.get(fstState);
		final NCSB fstNcsb = ((StateWaNCSB) mComplementBuchi.getState(fstId)).getNCSB();
		final int sndId = mStateIdMap.get(sndState);
		final NCSB sndNcsb = ((StateWaNCSB) mComplementBuchi.getState(sndId)).getNCSB();
