...
/**
 * Checks whether the language of a nested word automaton contains only finitely many words.
 *
...
public class IsFinite<LETTER, STATE> extends UnaryNwaOperation<LETTER, STATE, IStateFactory<STATE>> {
	 ...
	 */
	public IsFinite(final AutomataLibraryServices services, final INestedWordAutomaton<LETTER, STATE> operand)
			throws AutomataOperationCanceledException {
		super(services);
		mOperand = operand;
	...
	private boolean computeIsFinite() throws AutomataOperationCanceledException {
		final IDoubleDeckerAutomaton<LETTER, STATE> withoutDeadEnds =
				new RemoveDeadEnds<>(mServices, mOperand).getResult();
		final NestedWordAutomatonReachableStates<LETTER, STATE> nwars =
				new NestedWordAutomatonReachableStates<>(mServices, withoutDeadEnds);
		...
		final IDoubleDeckerAutomaton<LETTER, STATE> withoutNonLive =
				new RemoveNonLiveStates<>(mServices, allAccepting).getResult();
