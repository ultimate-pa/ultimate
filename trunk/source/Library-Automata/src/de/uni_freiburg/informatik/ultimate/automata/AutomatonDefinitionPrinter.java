...
public class AutomatonDefinitionPrinter<LETTER, STATE> {
	 ...
	 * <p>
	 * This method is intended to be used for dumping automata when an error occurs in an {@link IOperation}, e.g., when
	 * the {@link IOperation#checkResult()} method fails.
	 *
	 * @param services
	 *            Ultimate services
	...
	public static <LETTER, STATE> void writeToFileIfPreferred(final AutomataLibraryServices services,
		...
		new AutomatonDefinitionPrinter<LETTER, STATE>(services, fileNamePrefix, fileName, Format.ATS_NUMERATE, message,
				automata);
	}
	...
	private void printAutomataToFileWriter(final String automatonName, final Format format, final String message,
		...
		case ATS_NUMERATE:
		case ATS_QUOTED:
			mPrintWriter.println("// Testfile dumped by Ultimate at " + getDateTimeNice() + NEW_LINE + "//" + NEW_LINE
					+ "// " + message + NEW_LINE);
			break;
		case BA:
		case HOA:
	...
	/**
	 * Prints an {@link INestedWordAutomaton}. In this version letters and states are represented by a default symbol
	 * and a unique ID.
	 */
	...
	/**
	 * Prints a {@link PetriNetJulian}. In this version letters and places are represented by a default symbol and a
	 * unique ID.
	 */
