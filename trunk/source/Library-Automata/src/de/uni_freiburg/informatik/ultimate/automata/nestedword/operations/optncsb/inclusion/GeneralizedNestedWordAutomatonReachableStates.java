...
public class GeneralizedNestedWordAutomatonReachableStates<LETTER, STATE>
	...
	private StateContainer<LETTER, STATE> getOrAddState(final STATE state) {
		...
		if (cont == null) {
			...
			if (mOperand.isFinal(state)) {
				mFinalStates.add(state); // we have to add final states here
			}
		}
		return cont;
	}
	...
	class ReachableStatesComputationTarjan {
		private final Tarjan mTarjan = null;
		private final Ascc mAscc;
		...
		private class Tarjan {
			...
			void strongConnect(final STATE state) throws AutomataOperationCanceledException {
		...
		private class AsccElem {
			...
			AsccElem(final STATE state, final Set<Integer> labels) {
				mState = state;
				mLabels = labels;
			}
		...
		private class Ascc {
			...
			private final List<List<STATE>> mSccList;
			private Boolean mIsEmpty = null;
			...
			boolean construct(final STATE state) throws AutomataOperationCanceledException {
				...
				for (final OutgoingInternalTransition<LETTER, STATE> trans : mOperand.internalSuccessors(state)) {
					...
					if (mQPrime.contains(succ)) {
						...
						final boolean result = construct(succ); // did not visit succ before
						is_nemp = result || is_nemp;
					} else {
						final Set<Integer> B = new HashSet<>();
						STATE u;
						do {
							final AsccElem elem = mSCCs.pop();
							u = elem.mState;
				...
				if (mSCCs.peek().mState.equals(state)) {
					...
					final Set<Integer> labels = mSCCs.peek().mLabels;
					final List<STATE> sccList = new ArrayList<>();
					mSCCs.pop();
					STATE u;
					do {
	...
	private RunningTaskInfo constructRunningTaskInfo() {
		final String taskDescription = constructRunningTaskInfoMessage(mNumberOfConstructedStates, mOperand.getClass());
