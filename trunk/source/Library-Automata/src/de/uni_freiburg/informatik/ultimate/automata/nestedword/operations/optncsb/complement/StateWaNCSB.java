...
public class StateWaNCSB extends StateWa implements IStateWaComplement {
	...
	public StateWaNCSB(final BuchiWaComplement complement, final int id, final NCSB ncsb) {
		super(id);
	...
	public IntSet getSuccessors(final int letter) {
		...
		final IntSet minusFSuccs = succResult.mMinusFSuccs;
		final IntSet interFSuccs = succResult.mInterFSuccs;
	 ...
	 */
	private boolean noTransitionAssertion_MinusF(final int state, final IntSet succs) {
		return !mOperand.isFinal(state) && succs.isEmpty();
	}

	private SuccessorResult collectSuccessors(final IntSet states, final int letter, final boolean testTrans) {
		final SuccessorResult result = new SuccessorResult();
		for (final int state : states.iterable()) {
			final IntSet succs = mOperand.getSuccessors(state, letter);
	...
	private IntSet computeSuccessors(final NCSB succNCSB, final IntSet minusFSuccs, final IntSet interFSuccs,
			final int letter) {
		// check d(S) and d(C)
		if (succNCSB.getSSet().overlap(mOperand.getFinalStates()) || minusFSuccs.overlap(succNCSB.getSSet())) {
			return UtilIntSet.newIntSet();
		}
		final SuccessorGenerator generator = new SuccessorGenerator(mNCSB.getBSet().isEmpty(), succNCSB, minusFSuccs,
				interFSuccs, mOperand.getFinalStates());
		...
		while (generator.hasNext()) {
			...
			final StateWaNCSB succ = mComplement.addState(ncsb);
