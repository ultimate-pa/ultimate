...
/**
 * Creates a random deterministic finite bottom-up tree automaton (DFTA-BU) using the helper class
 * {@link AGetRandomFtaBU}. Note that the generation is not uniform.
 * <p>
...
public final class GetRandomDftaBU extends AGetRandomFtaBU {
	/**
	 * The default seed to use for generation. It is not intended to change thus the result of using the default seed
	 * will always remain the same even in different JVM cycles.
	 */
	...
	/**
	 * Converts a given data structure listing transition densities into one holding absolute values. The method is
	 * checked and does not allow overflows, see the documentation section of the exceptions.
	 *
	 * @param numberOfStates
	 *            The number of states the automaton has
	 * @param rankToTransitionDensity
	 *            The data structure to convert. Each index stands for the rank of a letter. The value stored at an
	 *            index represents the density {@code (0 <= x <= 1)} of transitions of that rank. The method needs to
	 *            compute the upper bound of maximal possible transitions which is {@code states^rank}. The computation
	 *            is done exact thus if numbers are big the method will take long.
	 * @return The converted data structure where each value lists the absolute number of transitions per letter instead
	 *         of their densities.
	 * @throws ArithmeticException
	 *             If the computation of {@code states^rank} does not fit into the size of an {@link Integer}.
	 */
	private static int[] convertTransitionDensitiesToNumbers(final int numberOfStates,
		...
		for (int rank = 0; rank < rankToTransitionDensity.length; rank++) {
			...
			final int numberOfTransitionsPerLetter =
					densityToAbsolute(transitionDensity, totalNumberOfTransitionsPerLetter);
	...
	/**
	 * If set then a method using transition densities instead of absolute amounts was chosen. Each index stands for the
	 * rank of a letter. The value stored at an index represents the density {@code (0 <= x <= 1)} of transitions of
	 * that rank. The density of nullary letters specify the amount of initial states.
	 */
	private double[] mRankToTransitionDensity = null;

	/**
	 * Constructor of a deterministic finite tree automaton for the {@code TestFileInterpreter}. This method uses a
	 * default seed that is not intended to change thus the result of this method, for same arguments, will always
	 * remain the same even in different JVM cycles. Use
	 * {@link #GetRandomDftaBU(AutomataLibraryServices, int, int[], double[], double, long)} if a seed should be
	 * specified.
	 *
	 * @param services
	 *            Ultimate services
	 ...
	 *            states are desired.
	 * @param rankToTransitionDensity
	 *            Each index stands for the rank of a letter. The value stored at an index represents the density
	 *            {@code (0 <= x <= 1)} of transitions of that rank. Note that the density of nullary letters specify
	 *            the amount of initial states thus if initial states are desired
	 *            <code>rankToTransitionDensity[0]</code> should be set to a value greater than <tt>zero</tt>. Also note
	 *            that if there are many states and ranks than a high density leads to an enormous amount of
	 *            transitions. With a full density {@code states^rank} transitions will be created for each letter. In
	 *            this case the other constructor
	 *            {@link #GetRandomDftaBU(AutomataLibraryServices, int, int[], int[], double)} may be more comfortable
	 *            to use and also faster since this variant needs to compute that bounds.
	 ...
	 *            transitions. With a full density {@code states^rank} transitions will be created for each letter. In
	 *            this case the other constructor
	 *            {@link #GetRandomDftaBU(AutomataLibraryServices, int, int[], int[], double, long)} may be more
	 *            comfortable to use and also faster since this variant needs to compute that bounds.
	 * @param acceptanceDensity
	 *            The acceptance density {@code (0 <= x <= 1)}. If <tt>1</tt> then all states are accepting, if
	 *            <tt>0</tt> then no state is accepting.
	 * @param seed
	...
	public GetRandomDftaBU(final AutomataLibraryServices services, final int numberOfStates,
		...
		super(services, numberOfStates, rankToNumberOfLetters,
				...
				seed);
		mRankToTransitionDensity = rankToTransitionDensity;
		startGeneration();
	}
	 ...
	 * default seed that is not intended to change thus the result of this method, for same arguments, will always
	 * remain the same even in different JVM cycles. Use
	 * {@link #GetRandomDftaBU(AutomataLibraryServices, int, int[], int[], double, long)} if a seed should be specified.
	 *
	 * @param services
	 *            Ultimate services
	 * @param numberOfStates
	 ...
	 *            which is given by {@code states^rank}. For efficiency reasons this object will not check validity for
	 *            those upper bounds. If setting higher values it is possible that this object never terminates the
	 *            generation process.
	 * @param acceptanceDensity
	 *            The acceptance density {@code (0 <= x <= 1)}. If <tt>1</tt> then all states are accepting, if
	 *            <tt>0</tt> then no state is accepting.
	 ...
	 * (non-Javadoc)
	 *
	 * @see de.uni_freiburg.informatik.ultimate.automata.tree.operations.AGetRandomFtaBU# checkInputValidity()
	 */
	@Override
	protected void checkInputValidity() throws IllegalArgumentException {
		super.checkInputValidity();

		if (mRankToNumberOfTransitionsPerLetter.length > 0 && mRankToNumberOfTransitionsPerLetter[0] != 1) {
		...
		if (mRankToTransitionDensity == null) {
			return;
		}
		// Transition densities
		for (final double transitionDensity : mRankToTransitionDensity) {
			// Check bounds
