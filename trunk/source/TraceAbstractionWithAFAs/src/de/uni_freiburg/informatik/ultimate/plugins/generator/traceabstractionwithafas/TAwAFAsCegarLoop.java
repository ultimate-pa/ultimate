...
public class TAwAFAsCegarLoop extends CegarLoopConcurrentAutomata {
	...
	public TAwAFAsCegarLoop(final String name, final BoogieIcfgContainer rootNode, final CfgSmtToolkit csToolkit,
		...
		mPredicateUnifier = new PredicateUnifier(services, csToolkit.getManagedScript(), predicateFactory,
				rootNode.getBoogie2SMT().getBoogie2SmtSymbolTable(), mSimplificationTechnique, mXnfConversionTechnique,
	...
	protected void constructInterpolantAutomaton() throws AutomataOperationCanceledException {
		...
		for (final DataflowDAG<TraceCodeBlock> dag : dags) {
			...
			if (mCsToolkit.getManagedScript().getScript().checkSat() == LBool.UNSAT) {
				...
				final AlternatingAutomaton<CodeBlock, IPredicate> alternatingAutomaton =
						computeAlternatingAutomaton(dag);
		...
		final RAFA_Determination<CodeBlock> determination =
		...
		try {
			assert new Accepts<>(new AutomataLibraryServices(mServices), mInterpolAutomaton,
					(NestedWord<CodeBlock>) trace).getResult() : "interpolant automaton does not accept the trace!";
	...
	private List<DataflowDAG<TraceCodeBlock>> computeRdDAGsFromCEx() throws AssertionError {
		try {
			...
			final List<DataflowDAG<TraceCodeBlock>> dags =
					ReachingDefinitions.computeRDForTrace(word, mLogger, mIcfgContainer);
			return dags;
	...
	private AlternatingAutomaton<CodeBlock, IPredicate>
		...
		while (!stack.isEmpty()) {
			...
			if (!targetStates.isEmpty()) {
				alternatingAutomaton.addTransition(currentDag.getNodeLabel().getBlock(),
