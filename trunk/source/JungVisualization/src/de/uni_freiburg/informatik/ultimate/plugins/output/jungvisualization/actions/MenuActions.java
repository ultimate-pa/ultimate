...
public class MenuActions implements IWorkbenchWindowActionDelegate {
	...
	public static void setMode(String mode) {
		...
		if (mode.equals(MODE_PICKING) || mode.equals(MODE_TRANSFORMING)) {
			...
			if (radioActions.containsKey(MODE_PICKING)
					&& radioActions.containsKey(MODE_TRANSFORMING)) {
				IAction pickingAction = radioActions.get(MODE_PICKING);
				IAction transformingAction = radioActions
						.get(MODE_TRANSFORMING);
	...
	public void selectionChanged(IAction action, ISelection selection) {
		...
		if (wbwindow != null) {
			...
			if (editorOpened) {
				activeEditorVVID = ((JungEditor) PlatformUI.getWorkbench()
						.getActiveWorkbenchWindow().getActivePage()
						.getActiveEditor())
						.getPartProperty(JungEditor.VV_ID_EDITOR_PROPERTY_KEY);
	...
	public static void exportAsSVG() {
		...
		VisualizationViewer<?, ?> vv = GraphHandler.getInstance()
				.getVisualizationViewer(activeEditorVVID);
	...
	public static void changeMode(IAction action) {
		...
		while (itr.hasNext()) {
			...
			DefaultModalGraphMouse<?, ?> gm = (DefaultModalGraphMouse<?, ?>) itr
					.next().getGraphMouse();
	...
	public static void openKeyHelp() {
		...
		if (!keyHelpState.equals("open")) {
			URL loc = Platform.getBundle(Activator.PLUGIN_ID).getEntry(
					"data/KeyHelp.html");
