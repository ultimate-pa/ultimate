...
class ACSLCommentParser {
	...
	private boolean mInGlobalScope;
	...
	/**
	 * Creates PSTACSLNodes for the whole tree of ACSLNodes (but does not add them to the ACSLComment node)
	 * 
	 * The ACSLNode structure is not closely related to the source syntax. The ACSLVisitor and ACSLTransformer only
	 * support a top-down traversal and no bottom-up traversal, and they visit nodes twice, so there is no simple copy
	 * of the tree structure possible by using those. Additionally, not every child returned by getChildren() is
	 * actually an ACSLNode generated by the parser from some source text. Every child that is not an ACSLNode or has
	 * not valid location is skipped.
	 *
	 * The following is an attempt to make the best out of the given ACSL-AST by just taking nodes that have an exact
	 * source location attached. I'm not sure if the order of children actually corresponds to that of the source text,
	 * even if location information is available, so all potential children are sorted and validated to be in inside the
	 * parent node's range before creating a PST node.
	 * 
	 * @param acslRootNode
	...
	private IPSTACSLNode createACSLTree(final ACSLNode acslRootNode) {
		...
		while (!nodesToExpand.isEmpty()) {
			...
			final List<IPSTACSLNode> children = createChildNodes(node, node.getACSLNode().getChildren());
	...
	private List<IPSTACSLNode> createChildNodes(final IPSTNode parent, final List<Object> acslChildren) {
		// Some ACSLNode children are arrays, try to flatten them
		final Stream<Object> flatStream = acslChildren.stream().flatMap(
				o -> o instanceof Object[] ? Arrays.stream((Object[]) o) : Collections.singletonList(o).stream());
		...
		final List<IPSTACSLNode> children = flatStream.filter(n -> n instanceof ACSLNode).map(n -> (ACSLNode) n)
				.filter(n -> n.getLocation().getStartLine() != -1).map(this::createPreliminaryPSTNode)
				.sorted(HierarchicalSourceRangeComparator.getInstance())
