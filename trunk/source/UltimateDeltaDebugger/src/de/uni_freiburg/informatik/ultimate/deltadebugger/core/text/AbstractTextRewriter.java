...
/**
 * Collects multiple independent text changes specified by the original
 * character offsets and eventually applies them all to the original text. Only
 * non-overlapping changes are allowed, i.e. all changed regions must be
 * disjoint.
 * 
 * Multiple insertions at the same offset are allowed as long as the surrounding
 * region is not deleted/replaced. Subsequent insertions are appended to the
 * existing insertion. Other than that the order of operations does not matter.
 * 
 * All modifications are validated as early as possible (fail-fast). If an
 * attempt to change an already changed range is detected, a
 * RewriteConflictException is thrown and the observable state is not modified.
 * The merge method allows to commit multiple changes together iff all of them
 * can be applied.
 * 
 * This class is meant to be a simplified alternative to
 * org.eclipse.text.edits.TextEdit. It has similiar behaviour but additionally
 * validates that all changes are within the original text bounds as early as
 * possible.
 */
public abstract class AbstractTextRewriter {
	...
	protected AbstractTextRewriter(List<Change> changes, int delta) {
		this.changes = changes;
		this.delta = delta;
	}

	/**
	 * Creates a new rewriter instance for an arbitrary text of the specified
	 * length. The originalLength is used to validate bounds of all operations.
	 * 
	 ...
	 */
	protected AbstractTextRewriter(AbstractTextRewriter other) {
	 ...
	 */
	public AbstractTextRewriter delete(int offset, int endOffset) {
	 ...
	 * @throws RewriteConflictException
	 */
	public AbstractTextRewriter insert(int offset, String text) {
	 ...
	 * @throws RewriteConflictException
	 */
	public AbstractTextRewriter replace(int offset, int endOffset, String replacement) {
	 ...
	 */
	public AbstractTextRewriter merge(AbstractTextRewriter other) {
	...
	protected List<Change> getMergedChanges(AbstractTextRewriter other) {
	...
	/**
	 * Computes a character's offset in the rewritten text for a given offset in
	 * the original text.
	 * 
	 * @param offset
	 *            offset in the original text
	 * @param includeInsertions
	 *            include insertations at offset, i.e. compute the remapped
	 *            offset after all insertions at the original offset
	 * @return corresponding offset in the rewritten text
	 * @throws IndexOutOfBoundsException
	 */
	public int remapOffset(int offset, boolean includeInsertions) {
	...
	private void addNewChange(int offset, int endOffset, String replacement) {
	...
	private void addAllOrNone(List<Change> source) {
		...
		} catch (RewriteConflictException e) {
	...
	private int findInsertionIndex(Change change, int startIndex) {
	...
	private boolean isInsertedAfter(Change newChange, Change existing) {
	...
	private List<Change> mergeSortedLists(List<Change> lhs, List<Change> rhs) {
	...
	protected static class Change {
		...
		protected Change(int offset, int endOffset, String replacement) {
			this.offset = offset;
