...
public class ChangeCollector {
	...
	public ChangeCollector(Map<IPSTRegularNode, List<CommaSeparatedChild>> parentToCommaPositionMap) {
	...
	public boolean addChange(Change newChange) {
		newChange.setIndex(changes.size());
		changes.add(newChange);
		return true;
	}	

	public void addDeleteChange(IPSTNode node) {
		addChange(new DeleteChange(node));
	}
	
	public void addReplaceChange(IPSTNode node, String replacementString) {
		if (!RewriteUtils.skipEquivalentReplacement(node, replacementString)) {
			addChange(new ReplaceChange(node, replacementString));
		}
	}
	
	public void addDeleteConditionalDirectivesChange(IPSTConditionalBlock block) {
		addChange(new DeleteConditionalDirectivesChange(block));
	}
	
	public boolean addDeleteWithCommaChange(IPSTRegularNode node, boolean keepOne) {
		IPSTRegularNode parent = node.getRegularParent();
		ASTNodeProperty childProperty = ASTNodeUtils.getPropertyOfCommaSeparatedChildNodes(parent.getASTNode());
		if (childProperty == null || node.getASTNode().getPropertyInParent() != childProperty) {
			ChangeCollector.logger.warn("DeleteWithCommaChange not supported for node " + node + " with property "
					+ node.getASTNode().getPropertyInParent());
			return false;
		}
		
		final List<CommaSeparatedChild> commaPositions = parentToCommaPositionMap.computeIfAbsent(parent,
		...
		if (!CommaDeleter.isDeletionWithCommaPossible(node, commaPositions)) {
			ChangeCollector.logger.trace("DeleteWithCommaChange not supported because of missing comma: " + node);
			return false;
		}
	...
	/**
	 * Add a change to delete one binary expression operand together with the
	 * operator. If both operands are deleted the binary expression is replaced
	 * by the given string (deleting both operands and the operator would not be
	 * an expression anymore, causing syntax errors)
	 * 
	 * @param operandNode
	 * @param fullReplacement
	 *            replacement to use in case changes that delete both operands
	 *            are applied
	 */
	public boolean addDeleteBinaryExpressionOperandChange(IPSTRegularNode operandNode, String fullReplacement) {
		...
		if (property != IASTBinaryExpression.OPERAND_ONE && property != IASTBinaryExpression.OPERAND_TWO) {
			ChangeCollector.logger.warn("DeleteBinaryExpressionOperand not supported for operand node " + operandNode
	...
	public boolean addDeleteAllTokensChange(IPSTNode node) {
		...
		if (!TokenUtils.isAllParenthesisBalanced(tokens)) {
			ChangeCollector.logger.trace("DeleteTokensChangeOld skipped because of unbalanced parenthesis in " + node);
			return false;
		}
	 ...
	 * @param forStatement IASTNode casted by caller
	 * @return true if a change as been added
	 */
	public boolean addDeleteForStatementTokensChange(IPSTRegularNode node, IASTForStatement forStatement) {
		final Token[] tokens = TokenUtils.getExpectedTokenArray(node, IToken.t_for, IToken.tLPAREN, IToken.tSEMI,
				IToken.tRPAREN);
	 ...
	 * @param node
	 *            statement node
	 * @param whileStatement IASTNode casted by caller
	 * @return true if a change as been added
	 */
	public boolean addDeleteWhileStatementTokensChange(IPSTRegularNode node, IASTWhileStatement whileStatement) {
		final Token[] tokens = TokenUtils.getExpectedTokenArray(node, IToken.t_while,
				IToken.tLPAREN, IToken.tRPAREN);
	 ...
	 * @return true if a change as been added
	 */
	public boolean addDeleteDoStatementTokensChange(IPSTRegularNode node) {
		final Token[] tokens = TokenUtils.getExpectedTokenArray(node, IToken.t_do, IToken.t_while, IToken.tLPAREN,
				IToken.tRPAREN);
	 ...
	 * 
	 * If tokens are missing because they are part of macro expansions or other
	 * preprocessor code, and the if-statement cannot be converted as whole an
	 * else-token is removed alone.
	 * 
	 * @param node
	 *            statement node
	 * @param ifStatement
	 ...
	 * @return true if a change as been added
	 */
	public boolean addDeleteIfStatementTokensChange(IPSTRegularNode node, IASTIfStatement ifStatement) {
		final Token[] tokens = TokenUtils.getExpectedTokenArray(node, IToken.t_if, IToken.tLPAREN,
				IToken.tRPAREN, IToken.t_else);
	...
	private static int getInsertionPoint(Token rparenToken, IPSTRegularNode expecteRightSibling) {
		if (rparenToken != null) {
	...
	public boolean addDeleteTypeIdFromCastExpressionChange(IPSTRegularNode typeIdNode) {
		final Token[] tokens = TokenUtils.getExpectedTokenArray(typeIdNode.getRegularParent(), IToken.tLPAREN,
				IToken.tRPAREN);
	...
	public boolean addDeleteConditionalExpressionChange(IPSTRegularNode node, String replacement) {
		...
		final Token[] tokens = TokenUtils.getExpectedTokenArray(conditionalExpressionNode, IToken.tQUESTION,
				IToken.tCOLON);
