 ...
 * that the test for a previous step fails. As soon it turns out that speculation was wrong steps are canceled and
 * isCanceled() returns true. This can be called to abort the test if possible, because it's result is not of use
 * anymore. If and only if a step is canceled the called to complete is optional - otherwise it must be called or the
 * iteration will not advance. Getters are thread safe, complete() is NOT and must be synchronized with other methods of
 * the originating SpeculativeSearchIterator instance.
 *
 * @param <T>
