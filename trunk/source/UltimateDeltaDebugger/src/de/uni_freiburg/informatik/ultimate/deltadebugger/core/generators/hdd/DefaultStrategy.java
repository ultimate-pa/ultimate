...
class RegularNodeHandler implements IASTNodeConsumer {
	...
	public void on(final IASTDeclaration declaration) {
		...
		// A condition declaration should be valid for C++ only,
		// not sure if gcc accepts it anyways for C
	...
	public void on(final IASTEqualsInitializer equalsInitializer) {
		...
		// The only excpetion are variables with static storage, these are zero
		// initialized
		// implicitly. In such a case the initializer could be deleted.
		// TODO: actually implement the idea above: need to get the
		// corresponding
		// IASTDeclaration and then check the storage class.
	}
	...
	public void on(final IASTExpression expression) {
		...
		if (property == IASTBinaryExpression.OPERAND_ONE || property == IASTBinaryExpression.OPERAND_TWO) {
			collector.addDeleteBinaryExpressionOperandChange(currentNode, "0");
			return;
		}
		...
		if (property == IASTConditionalExpression.LOGICAL_CONDITION
			...
			collector.addDeleteConditionalExpressionChange(currentNode, "0");
			return;
		}
		...
		if (expression instanceof IASTLiteralExpression) {
			...
			if (literalExpression.getKind() == IASTLiteralExpression.lk_float_constant) {
				collector.addReplaceChange(currentNode, ".0f");
				return;
			} else if (literalExpression.getKind() == IASTLiteralExpression.lk_string_literal) {
				collector.addReplaceChange(currentNode, "\"\"");
				return;
			}
		}
	...
	public void on(final IASTTypeId typeId) {
		...
		if (typeId.getPropertyInParent() == IASTCastExpression.TYPE_ID) {
			collector.addDeleteTypeIdFromCastExpressionChange(currentNode);
		} else {
			// collector.addReplaceChange(currentNode, "int");
		}
	}
