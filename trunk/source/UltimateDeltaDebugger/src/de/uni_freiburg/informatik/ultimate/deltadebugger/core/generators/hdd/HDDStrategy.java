...
/**
 * Strategy for the HDD insipired pass that controls if and how nodes of the tree are changed.
 *
 */
@FunctionalInterface
public interface HDDStrategy {
	...
	/**
	 * Determines if the changes generated by expandind node will be tested with other changes on the same level or not.
	 *
	 * If true is returned for a node, all chenges that are generated for the children of this node (not the node
	 * itself!) will have their own VariantGenerator instance and will effectively be search on their own. Example:
	 * Returning true for function definition bodies causes that each body compound statement is reduced individually
	 * instead of together with all other nodes on the same level.
	 *
	 ...
	 *
	 * @param node
	 * @return wether changes for children should take the place immediately if no chenges are generated for node
	 */
