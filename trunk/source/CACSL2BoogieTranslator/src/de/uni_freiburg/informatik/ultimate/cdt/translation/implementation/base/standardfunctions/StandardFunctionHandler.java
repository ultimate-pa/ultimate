...
public class StandardFunctionHandler {
	...
	private Result handleErrorFunction(final IDispatcher main, final IASTFunctionCallExpression node,
		...
		if (!testCaseGeneration) {
			st = createAnnotatedAssertOrAssume(loc, name, mSettings.checkErrorFunction(), Spec.ERROR_FUNCTION, falseLiteral);
		} else {
			st = createAnnotatedAssertOrAssume(loc, name, mSettings.checkErrorFunction(), Spec.TEST_GOAL_ANNOTATION,
					falseLiteral);
		}
