...
modifies ~send4, ~alive2, ~send2, ~p4_new, ~id4, ~alive4, ~id2, ~st4, ~nomsg, ~st2, ~mode1, ~p2_new, ~mode3, ~p2_old, ~p3_new, ~r1, ~p1_old, ~alive1, ~send3, ~id3, ~alive3, ~send1, ~st3, ~id1, ~st1, ~mode2, ~p4_old, ~mode4, ~p1_new, ~p3_old;
{
  ...
  loc0:
    ...
    havoc init_~tmp~52;
    assume ~r1 == 0;
    assume ((((0 <= ~alive3 + ~alive4 % 256 + ~alive1 % 256 + ~alive2 % 256 + 255 && 256 + 256 <= ~alive2) && 256 + 256 <= ~alive4) && ~alive1 < -256) && -256 <= ~alive3) && ~alive3 < 0;
    assume 0 <= ~id1;
    assume ~st1 == 0;
    assume ~send1 == ~id1;
    assume ~mode1 % 256 == 0 && ~mode1 < -256;
    assume 0 <= ~id2;
    assume ~st2 == 0;
    assume ~send2 == ~id2;
    assume ~mode2 - 256 == 0;
  ...
  loc4_1:
    assume ~mode2 - 256 == 0;
    assume ~alive2 % 256 == 0 && 256 + 256 <= ~alive2;
    assume ~send2 == ~id2;
    ~mode2 := 1;
    goto loc5;
  loc5:
  ...
  loc6_0:
    ...
    assume (-256 <= ~alive3 && !(~alive3 + 256 == 0)) && ~alive3 < 0;
  ...
  loc6_1:
    ...
    goto loc7;
  loc7:
    havoc node4_#t~ite7, node4_#t~ite6, node4_~m4~41;
    havoc node4_~m4~41;
    node4_~m4~41 := ~nomsg;
    goto loc8;
  loc8:
