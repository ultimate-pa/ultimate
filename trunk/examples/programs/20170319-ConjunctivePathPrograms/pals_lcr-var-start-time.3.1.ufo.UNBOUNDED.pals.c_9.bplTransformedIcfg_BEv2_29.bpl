...
modifies ~p1_old, ~alive2, ~send2, ~alive1, ~send3, ~id3, ~alive3, ~send1, ~id2, ~st3, ~id1, ~st1, ~nomsg, ~st2, ~mode2, ~mode1, ~p2_new, ~mode3, ~p2_old, ~p1_new, ~p3_old, ~p3_new, ~r1;
{
  ...
  loc0:
    ...
    havoc init_~tmp~42;
    assume ~r1 == 0;
    assume ((~alive2 < -256 && ~alive3 < -256) && 1 <= ~alive3 % 256 + ~alive1 % 256 + ~alive2 % 256) && 256 + 256 <= ~alive1;
    assume 0 <= ~id1;
    assume ~st1 == 0;
    assume ~send1 == ~id1;
    assume ~mode1 == 0;
  ...
  loc2_0:
    assume ~mode1 == 0;
    assume 256 + 256 <= ~alive1 && ~alive1 % 256 == 0;
    assume ~send1 == ~id1;
    ~mode1 := 1;
    goto loc3;
  loc2_1:
    ...
    goto loc3;
  loc3:
    havoc node2_#t~ite3, node2_#t~ite2, node2_~m2~20;
    havoc node2_~m2~20;
    node2_~m2~20 := ~nomsg;
    goto loc4;
  loc4:
