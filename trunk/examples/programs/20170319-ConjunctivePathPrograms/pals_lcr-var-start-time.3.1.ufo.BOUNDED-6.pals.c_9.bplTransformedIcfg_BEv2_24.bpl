...
modifies ~p1_old, ~alive2, ~send2, ~alive1, ~send3, ~id3, ~alive3, ~send1, ~id2, ~st3, ~id1, ~st1, ~nomsg, ~st2, ~mode2, ~mode1, ~p2_new, ~mode3, ~p2_old, ~p1_new, ~p3_old, ~p3_new, ~r1;
{
  ...
  loc0:
    ...
    havoc init_~tmp~41;
    assume ~r1 == 0;
    assume (((((1 <= ~alive1 + ~alive3 + ~alive2 && 0 <= ~alive3) && 256 <= ~alive1) && ~alive2 < 0) && ~alive1 < 256 + 256) && ~alive3 < 256) && -256 <= ~alive2;
    assume 0 <= ~id1;
    assume ~st1 == 0;
    assume ~send1 == ~id1;
    assume ~mode1 + 256 == 0;
  ...
  loc2_0:
    assume ~mode1 + 256 == 0;
    assume (!(~alive1 - 256 == 0) && 256 <= ~alive1) && ~alive1 < 256 + 256;
  ...
  loc12_1:
    ...
    main_#t~post24 := main_~i2~84;
    main_~i2~84 := main_#t~post24 + 1;
    havoc main_#t~post24;
    goto loc1;
  loc13:
    assert false;
}
