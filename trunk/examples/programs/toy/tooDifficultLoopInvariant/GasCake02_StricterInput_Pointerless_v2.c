...
uint8 signalDelay_u8 = 0;
uint8 plausiSignalDelay_u8 = 0;
uint8 comparisonDelay_u8 = 0;
uint8 steps = 10;
...
uint8 debounce_u8_csd(uint8 _current_signal_u8, uint8 _debounceThresh_u8,
			   uint8 _lastOutput_u8)
{
	uint8 output_u8 = _lastOutput_u8;
	if (_current_signal_u8 != _lastOutput_u8)
	{
		comparisonDelay_u8++;
		if (comparisonDelay_u8 >= _debounceThresh_u8)
		{
			comparisonDelay_u8 = 0;
			output_u8 = _current_signal_u8;
		}
	} else {
		comparisonDelay_u8 = 0;
	}
...
uint8 debounce_u8_psd(uint8 _current_signal_u8, uint8 _debounceThresh_u8,
			   uint8 _lastOutput_u8)
{
	uint8 output_u8 = _lastOutput_u8;
	if (_current_signal_u8 != _lastOutput_u8)
...
uint8 debounce_u8_sd(uint8 _current_signal_u8, uint8 _debounceThresh_u8,
			   uint8 _lastOutput_u8)
{
	uint8 output_u8 = _lastOutput_u8;
	if (_current_signal_u8 != _lastOutput_u8)
...
void PDebounceValidation(void)
{
	...
	static uint16 inputGlitchTimeout_u16 = 0; 
	...
	while (tclRange_b(0,1))
	{
		...
		if ((tclRange_u8(0,255)== 0) &&(!inputGlitchTimeout_u16)) {
			plausiSignal_u8 = signal_u8;
			signal_u8 = tclRange_u8(0,255);
			if (plausiSignal_u8!=signal_u8) {
				inputGlitchTimeout_u16 = 1000;				
			}
		}
		else 
		{
        ...
        if (inputGlitchTimeout_u16) {
                inputGlitchTimeout_u16--;
        }
		signalDeb_u8 = debounce_u8_sd(signal_u8, steps, signalDeb_u8);
		plausiSignalDeb_u8 = debounce_u8_psd(plausiSignal_u8, steps, plausiSignalDeb_u8);

		comparison_u8 = (signalDeb_u8 == plausiSignalDeb_u8);
		comparisonDeb_u8 = debounce_u8_csd(comparison_u8, steps, comparisonDeb_u8);
