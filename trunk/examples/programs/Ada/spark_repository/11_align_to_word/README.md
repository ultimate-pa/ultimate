...
`align_to_word.bpl` is the faithful 1:1 translation — just the original loop, with no assertions or postcondition. Without any specification, there is nothing for Ultimate to check beyond the implicit contract.

`align_to_word_modified.bpl` adds one assert inside the loop body, not present in the original Ada, checking that `Data_I` stays even on every iteration. That does require Ultimate to find and maintain a real invariant across iterations, so it's kept as a separate file rather than changing the 1:1 translation.
...
**Overall Result: No assertions to check — the file carries only a precondition, which is assumed rather than verified.**
