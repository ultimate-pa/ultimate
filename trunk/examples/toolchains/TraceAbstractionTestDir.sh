...
if [ "$EXTENSION" == "bpl" ]; then
    TOOLCHAIN="$Ultimate_PATH/trunk/examples/toolchains/TraceAbstraction.xml"
fi
...
if [ "$VCCPRELUDE" ]; then 
    Ultimate_OUTPUT=`bash -c "ulimit -t $TimeLimit; $Ultimate_PATH/$UltimateEXE --console "$TOOLCHAIN" "$f" --prelude "$Ultimate_PATH/trunk/examples/programs/translated-vcc/Vcc2Prelude.bpl" --settings "$Ultimate_PATH/settings/$s" 2>&1"`    
else
    Ultimate_OUTPUT=`bash -c "ulimit -t $TimeLimit; $Ultimate_PATH/$UltimateEXE --console "$TOOLCHAIN" "$f" --settings "$Ultimate_PATH/settings/$s" 2>&1"`
fi
