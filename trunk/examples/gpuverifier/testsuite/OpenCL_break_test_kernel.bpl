...
implementation {:source_name "foo"} ULTIMATE.start($x: bv32)
{
  ...
  $while.cond:
    ...
    v0$1 := (if p0$1 then BV32_SLT(BV32_ADD($x, 100bv32), 102bv32) else v0$1);
    v0$2 := (if p0$2 then BV32_SLT(BV32_ADD($x, 100bv32), 102bv32) else v0$2);
    p1$1 := false;
    p1$2 := false;
    p2$1 := false;
    p2$2 := false;
    ...
    v1$1 := (if p1$1 then BV32_ULT(local_id_x$1, 5bv32) else v1$1);
    v1$2 := (if p1$2 then BV32_ULT(local_id_x$2, 5bv32) else v1$2);
    p0$1 := (if p1$1 && v1$1 then !v1$1 else p0$1);
    p0$2 := (if p1$2 && v1$2 then !v1$2 else p0$2);
    p0$1 := (if p1$1 && !v1$1 then !v1$1 else p0$1);
    p0$2 := (if p1$2 && !v1$2 then !v1$2 else p0$2);
