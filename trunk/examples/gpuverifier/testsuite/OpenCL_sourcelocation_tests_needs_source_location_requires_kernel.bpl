...
procedure {:source_name "bar"} $bar($p: bv32);
  requires $p == 0bv32;
...
implementation {:source_name "bar"} $bar($p: bv32)
{
  var $i.0$1: bv32;
  var $i.0$2: bv32;
  var v0$1: bool;
  ...
  $for.cond:
    ...
    v0$1 := (if p0$1 then BV32_ULT($i.0$1, BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1)) else v0$1);
    v0$2 := (if p0$2 then BV32_ULT($i.0$2, BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2)) else v0$2);
    p1$1 := false;
    p1$2 := false;
    p2$1 := false;
    p2$2 := false;
    ...
    $i.0$1 := (if p1$1 then BV32_ADD($i.0$1, 1bv32) else $i.0$1);
    $i.0$2 := (if p1$2 then BV32_ADD($i.0$2, 1bv32) else $i.0$2);
  ...
  $for.cond.tail:
    assume !p0$1 && !p0$2;
    v1$1 := BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1) == 24bv32;
    v1$2 := BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2) == 24bv32;
    p3$1 := (if v1$1 then v1$1 else p3$1);
    p3$2 := (if v1$2 then v1$2 else p3$2);
    call _LOG_WRITE_$$p(p3$1, BV32_ADD($p, $i.0$1), BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1), $$p[BV32_ADD($p, $i.0$1)]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$p(p3$2, BV32_ADD($p, $i.0$2));
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 6} true;
    call _CHECK_WRITE_$$p(p3$2, BV32_ADD($p, $i.0$2), BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2));
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$p"} true;
    $$p[BV32_ADD($p, $i.0$1)] := (if p3$1 then BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1) else $$p[BV32_ADD($p, $i.0$1)]);
    $$p[BV32_ADD($p, $i.0$2)] := (if p3$2 then BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2) else $$p[BV32_ADD($p, $i.0$2)]);
    return;
