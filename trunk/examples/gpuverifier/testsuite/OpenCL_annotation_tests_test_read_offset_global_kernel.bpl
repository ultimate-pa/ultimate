...
implementation {:source_name "foo"} ULTIMATE.start()
{
  ...
  $entry:
    ...
    call _LOG_WRITE_$$p(p0$1, 4bv32, v1$1, $$p[4bv32]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$p(p0$2, 4bv32);
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 3} true;
    call _CHECK_WRITE_$$p(p0$2, 4bv32, v1$2);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$p"} true;
    $$p[4bv32] := (if p0$1 then v1$1 else $$p[4bv32]);
    $$p[4bv32] := (if p0$2 then v1$2 else $$p[4bv32]);
    assert {:sourceloc_num 4} {:thread 1} p0$1 ==> (if BV32_AND(BV1_ZEXT32((if _WRITE_HAS_OCCURRED_$$p then 1bv1 else 0bv1)), BV1_ZEXT32((if _READ_HAS_OCCURRED_$$q then 1bv1 else 0bv1))) != 0bv32 ==> BV32_MUL(4bv32, _WATCHED_OFFSET) == BV32_ADD(BV32_MUL(4bv32, _WATCHED_OFFSET), 4bv32) then 1bv1 else 0bv1) != 0bv1;
    return;
}
