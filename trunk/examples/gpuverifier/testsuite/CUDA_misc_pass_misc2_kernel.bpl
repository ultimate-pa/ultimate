...
implementation {:source_name "helloCUDA"} ULTIMATE.start()
{
  ...
  $entry:
    assert {:sourceloc_num 1} {:thread 1} BV1_XOR((if _READ_HAS_OCCURRED_$$p then 1bv1 else 0bv1), 1bv1) != 0bv1;
    call _LOG_WRITE_$$p(true, local_id_x$1, local_id_x$1, $$p[local_id_x$1]);
    ...
    $$p[local_id_x$1] := local_id_x$1;
    $$p[local_id_x$2] := local_id_x$2;
    return;
}
