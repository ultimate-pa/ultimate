...
procedure {:source_name "foo"} ULTIMATE.start();
  ...
  modifies $$A, _WRITE_HAS_OCCURRED_$$A, _WRITE_READ_BENIGN_FLAG_$$A, _WRITE_READ_BENIGN_FLAG_$$A, _TRACKING;
...
implementation {:source_name "foo"} ULTIMATE.start()
{
  ...
  __partitioned_block_$truebb_0:
    ...
    call _LOG_WRITE_$$A(p1$1, local_id_x$1, $k.0, $$A[1bv1][local_id_x$1]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$A(p1$2, local_id_x$2);
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 4} true;
    call _CHECK_WRITE_$$A(p1$2, local_id_x$2, $k.0);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$A"} true;
    $$A[1bv1][local_id_x$1] := (if p1$1 then $k.0 else $$A[1bv1][local_id_x$1]);
    $$A[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][local_id_x$2] := (if p1$2 then $k.0 else $$A[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][local_id_x$2]);
