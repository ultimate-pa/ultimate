...
axiom {:array_info "$$foo.G"} {:group_shared} {:elem_width 32} {:source_name "G"} {:source_elem_width 32} {:source_dimensions "64"} true;
...
procedure {:source_name "foo"} ULTIMATE.start();
  ...
  modifies _ARRAY_OFFSET_$$foo.G, _ARRAY_OFFSET_$$foo.L, $$foo.L, _WRITE_HAS_OCCURRED_$$foo.L, _WRITE_READ_BENIGN_FLAG_$$foo.L, _WRITE_READ_BENIGN_FLAG_$$foo.L;
...
implementation {:source_name "foo"} ULTIMATE.start()
{
  ...
  $entry:
    _ARRAY_OFFSET_$$foo.G := BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1);
    _ARRAY_OFFSET_$$foo.G := BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2);
    assume {:do_not_predicate} {:check_id "bounds_check_state_0"} {:captureState "bounds_check_state_0"} true;
    assert {:array_bounds} {:sourceloc_num 1} {:check_id "bounds_check_state_0"} {:array_name "$$foo.G"} BV32_SGE(_ARRAY_OFFSET_$$foo.G, 0bv32);
    assert {:array_bounds} {:sourceloc_num 1} {:check_id "bounds_check_state_0"} {:array_name "$$foo.G"} BV32_SLT(_ARRAY_OFFSET_$$foo.G, 64bv32);
    havoc v0$1, v0$2;
    _ARRAY_OFFSET_$$foo.L := v0$1;
    _ARRAY_OFFSET_$$foo.L := v0$2;
    assume {:do_not_predicate} {:check_id "bounds_check_state_1"} {:captureState "bounds_check_state_1"} true;
    assert {:array_bounds} {:sourceloc_num 2} {:check_id "bounds_check_state_1"} {:array_name "$$foo.L"} BV32_SGE(_ARRAY_OFFSET_$$foo.L, 0bv32);
    assert {:array_bounds} {:sourceloc_num 2} {:check_id "bounds_check_state_1"} {:array_name "$$foo.L"} BV32_SLT(_ARRAY_OFFSET_$$foo.L, 64bv32);
    call _LOG_WRITE_$$foo.L(true, v0$1, BV32_MUL(group_size_x, num_groups_x), $$foo.L[1bv1][v0$1]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$foo.L(true, v0$2);
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 2} true;
    call _CHECK_WRITE_$$foo.L(true, v0$2, BV32_MUL(group_size_x, num_groups_x));
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$foo.L"} true;
    $$foo.L[1bv1][v0$1] := BV32_MUL(group_size_x, num_groups_x);
    $$foo.L[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][v0$2] := BV32_MUL(group_size_x, num_groups_x);
    return;
}
