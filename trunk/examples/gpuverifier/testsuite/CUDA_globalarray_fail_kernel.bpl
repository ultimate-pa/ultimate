...
implementation {:source_name "globalarray"} ULTIMATE.start()
{
  ...
  $entry:
    v0$1 := $$A$1[local_id_x$1];
    v0$2 := $$A$2[local_id_x$2];
    v1$1 := v0$1 != local_id_x$1;
    v1$2 := v0$2 != local_id_x$2;
    p0$1 := false;
    p0$2 := false;
    p1$1 := false;
    p1$2 := false;
    ...
    call _LOG_WRITE_$$p(p0$1, 0bv32, UI32_TO_FP32(local_id_x$1), $$p[0bv32]);
    ...
    call _CHECK_WRITE_$$p(p0$2, 0bv32, UI32_TO_FP32(local_id_x$2));
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$p"} true;
    $$p[0bv32] := (if p0$1 then UI32_TO_FP32(local_id_x$1) else $$p[0bv32]);
    $$p[0bv32] := (if p0$2 then UI32_TO_FP32(local_id_x$2) else $$p[0bv32]);
    return;
}
