...
implementation {:source_name "foo"} ULTIMATE.start()
{
  ...
  $for.cond6:
    ...
    assert {:do_not_predicate} {:originated_from_invariant} {:sourceloc_num 7} {:thread 1} (if BV32_AND(BV1_ZEXT32((if p1$1 then 1bv1 else 0bv1)), BV1_ZEXT32((if _READ_HAS_OCCURRED_$$foo.A then 1bv1 else 0bv1))) != 0bv32 ==> BV32_UDIV(BV32_MUL(4bv32, _WATCHED_OFFSET), 4bv32) == BV32_ADD(BV32_SUB(0bv32, v0$1), 31bv32) then 1bv1 else 0bv1) != 0bv1;
    assert {:do_not_predicate} {:originated_from_invariant} {:sourceloc_num 8} {:thread 1} (if _WRITE_HAS_OCCURRED_$$foo.A ==> BV32_UDIV(BV32_MUL(4bv32, _WATCHED_OFFSET), 4bv32) == BV32_ADD(BV32_SUB(0bv32, v0$1), 31bv32) then 1bv1 else 0bv1) != 0bv1;
    ...
    v3$1 := (if p3$1 then BV32_ADD(BV32_SUB(0bv32, v0$1), 31bv32) else v3$1);
    v3$2 := (if p3$2 then BV32_ADD(BV32_SUB(0bv32, v0$2), 31bv32) else v3$2);
    call _LOG_READ_$$foo.A(p3$1, v3$1, $$foo.A[1bv1][v3$1]);
    assume {:do_not_predicate} {:check_id "check_state_2"} {:captureState "check_state_2"} {:sourceloc} {:sourceloc_num 10} true;
    call _CHECK_READ_$$foo.A(p3$2, v3$2, $$foo.A[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][v3$2]);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_READ_$$foo.A"} true;
    v4$1 := (if p3$1 then $$foo.A[1bv1][v3$1] else v4$1);
    v4$2 := (if p3$2 then $$foo.A[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][v3$2] else v4$2);
    call _LOG_WRITE_$$foo.A(p3$1, v3$1, BV32_SUB(v4$1, 5bv32), $$foo.A[1bv1][v3$1]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$foo.A(p3$2, v3$2);
    assume {:do_not_predicate} {:check_id "check_state_3"} {:captureState "check_state_3"} {:sourceloc} {:sourceloc_num 11} true;
    call _CHECK_WRITE_$$foo.A(p3$2, v3$2, BV32_SUB(v4$2, 5bv32));
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$foo.A"} true;
    $$foo.A[1bv1][v3$1] := (if p3$1 then BV32_SUB(v4$1, 5bv32) else $$foo.A[1bv1][v3$1]);
    $$foo.A[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][v3$2] := (if p3$2 then BV32_SUB(v4$2, 5bv32) else $$foo.A[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][v3$2]);
  ...
  __partitioned_block_$for.cond6.tail_1:
    ...
    $c431.0$1 := (if p1$1 then v0$1 else $c431.0$1);
    $c431.0$2 := (if p1$2 then v0$2 else $c431.0$2);
  ...
  $for.cond32:
    ...
    v5$1 := (if p4$1 then BV32_SLT($c431.0$1, 32bv32) else v5$1);
    v5$2 := (if p4$2 then BV32_SLT($c431.0$2, 32bv32) else v5$2);
    p5$1 := false;
    p5$2 := false;
    ...
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_READ_$$foo.A"} true;
    v6$1 := (if p5$1 then $$foo.A[1bv1][v0$1] else v6$1);
    v6$2 := (if p5$2 then $$foo.A[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][v0$2] else v6$2);
    $c431.0$1 := (if p5$1 then BV32_ADD($c431.0$1, 16bv32) else $c431.0$1);
    $c431.0$2 := (if p5$2 then BV32_ADD($c431.0$2, 16bv32) else $c431.0$2);
    p4$1 := (if p5$1 then true else p4$1);
    p4$2 := (if p5$2 then true else p4$2);
    goto $for.cond32.backedge, $for.cond32.tail;
