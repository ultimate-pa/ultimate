...
var {:source_name "ai"} $$ai$1: [bv32]bv32;

var {:source_name "ai"} $$ai$2: [bv32]bv32;

axiom {:array_info "$$ai"} {:elem_width 32} {:source_name "ai"} {:source_elem_width 32} {:source_dimensions "1"} true;
...
procedure {:source_name "f"} $_Z1fPfPi($odata: bv32, $ai: bv32);
  requires $odata == 0bv32;
  requires $ai == 0bv32;
...
implementation {:source_name "f"} $_Z1fPfPi($odata: bv32, $ai: bv32)
{

  $entry:
    $$ai$1[$ai] := local_id_x$1;
    $$ai$2[$ai] := local_id_x$2;
    return;
}
...
implementation {:source_name "k"} ULTIMATE.start()
{
  ...
  $entry:
    call $_Z1fPfPi(0bv32, 0bv32);
    assume {:captureState "call_return_state_0"} {:procedureName "$_Z1fPfPi"} true;
    return;
}
...
procedure _CHECK_READ_$$g_odata(_P: bool, _offset: bv32, _value: bv32);
  requires !(_P && _WRITE_HAS_OCCURRED_$$g_odata && _WATCHED_OFFSET == _offset && _WRITE_READ_BENIGN_FLAG_$$g_odata);
  requires !(_P && _ATOMIC_HAS_OCCURRED_$$g_odata && _WATCHED_OFFSET == _offset);
...
procedure _CHECK_WRITE_$$g_odata(_P: bool, _offset: bv32, _value: bv32);
  requires !(_P && _WRITE_HAS_OCCURRED_$$g_odata && _WATCHED_OFFSET == _offset && _WATCHED_VALUE_$$g_odata != _value);
  requires !(_P && _READ_HAS_OCCURRED_$$g_odata && _WATCHED_OFFSET == _offset && _WATCHED_VALUE_$$g_odata != _value);
  requires !(_P && _ATOMIC_HAS_OCCURRED_$$g_odata && _WATCHED_OFFSET == _offset);
...
procedure _CHECK_ATOMIC_$$g_odata(_P: bool, _offset: bv32);
  requires !(_P && _WRITE_HAS_OCCURRED_$$g_odata && _WATCHED_OFFSET == _offset);
  requires !(_P && _READ_HAS_OCCURRED_$$g_odata && _WATCHED_OFFSET == _offset);
