...
procedure {:source_name "a"} ULTIMATE.start();
...
implementation {:source_name "a"} ULTIMATE.start()
{
  var v0$1: bv32;
  var v0$2: bv32;
  var v1$1: bv32;
  ...
  $entry:
    assert {:sourceloc_num 1} {:thread 1} (if BV32_MUL(group_size_x, num_groups_x) == 64bv32 then 1bv1 else 0bv1) != 0bv1;
    assert {:sourceloc_num 2} {:thread 1} (if BV32_MUL(group_size_y, num_groups_y) == 256bv32 then 1bv1 else 0bv1) != 0bv1;
    assert {:sourceloc_num 3} {:thread 1} (if num_groups_x == 2bv32 then 1bv1 else 0bv1) != 0bv1;
    assert {:sourceloc_num 4} {:thread 1} (if num_groups_y == 4bv32 then 1bv1 else 0bv1) != 0bv1;
    ...
    call _CHECK_WRITE_$$A(true, v0$2, BV32_ADD(v1$2, 5bv32));
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$A"} true;
    $$A[v0$1] := BV32_ADD(v1$1, 5bv32);
    $$A[v0$2] := BV32_ADD(v1$2, 5bv32);
    return;
}
