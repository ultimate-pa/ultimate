...
procedure {:source_name "__spec_foo"} $__spec_foo($p: bv32);
  requires $p == 0bv32;
...
procedure {:source_name "bar"} ULTIMATE.start();
  ...
  modifies _WRITE_HAS_OCCURRED_$$A, _READ_HAS_OCCURRED_$$A;
...
implementation {:source_name "bar"} ULTIMATE.start()
{
  ...
  $entry:
    call $__spec_foo(0bv32);
    assume {:captureState "call_return_state_0"} {:procedureName "$__spec_foo"} true;
...
procedure {:source_name "baz"} $baz($0$1: bv32, $0$2: bv32);
