...
procedure {:source_name "bar"} $bar($A: bv32) returns ($ret$1: bv32, $ret$2: bv32);
  requires $A == 0bv32;
...
implementation {:source_name "bar"} $bar($A: bv32) returns ($ret$1: bv32, $ret$2: bv32)
{
  var $retval.0$1: bv32;
  var $retval.0$2: bv32;
  var v0$1: bool;
  ...
  $entry:
    ...
    $retval.0$1 := (if p0$1 then 0bv32 else $retval.0$1);
    $retval.0$2 := (if p0$2 then 0bv32 else $retval.0$2);
    call _LOG_WRITE_$$A(p1$1, BV32_ADD($A, 4bv32), 1104569958bv32, $$A[1bv1][BV32_ADD($A, 4bv32)]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$A(p1$2, BV32_ADD($A, 4bv32));
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 3} true;
    call _CHECK_WRITE_$$A(p1$2, BV32_ADD($A, 4bv32), 1104569958bv32);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$A"} true;
    $$A[1bv1][BV32_ADD($A, 4bv32)] := (if p1$1 then 1104569958bv32 else $$A[1bv1][BV32_ADD($A, 4bv32)]);
    $$A[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][BV32_ADD($A, 4bv32)] := (if p1$2 then 1104569958bv32 else $$A[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][BV32_ADD($A, 4bv32)]);
    $retval.0$1 := (if p1$1 then 1bv32 else $retval.0$1);
    $retval.0$2 := (if p1$2 then 1bv32 else $retval.0$2);
    $ret$1 := $retval.0$1;
    $ret$2 := $retval.0$2;
    return;
}
