...
procedure {:source_name "__spec_bar"} $__spec_bar($p: bv32);
  requires BV1_XOR((if _READ_HAS_OCCURRED_$$A then 1bv1 else 0bv1), 1bv1) != 0bv1;
  requires (if _WRITE_HAS_OCCURRED_$$A ==> BV32_UDIV(BV32_MUL(4bv32, _WATCHED_OFFSET), 4bv32) == BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1) then 1bv1 else 0bv1) != 0bv1;
  requires (if $$A[BV32_ADD($p, BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1))] == BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1) then 1bv1 else 0bv1) != 0bv1;
  requires (if $$A[BV32_ADD($p, BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2))] == BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2) then 1bv1 else 0bv1) != 0bv1;
  requires $p == 0bv32;
  modifies _WRITE_HAS_OCCURRED_$$A;
  ensures (if $$A[BV32_ADD($p, BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1))] == old(BV32_ADD($$A[BV32_ADD($p, BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1))], 1bv32)) then 1bv1 else 0bv1) != 0bv1;
  ensures (if $$A[BV32_ADD($p, BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2))] == old(BV32_ADD($$A[BV32_ADD($p, BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2))], 1bv32)) then 1bv1 else 0bv1) != 0bv1;
  ensures (if _WRITE_HAS_OCCURRED_$$A ==> BV32_UDIV(BV32_MUL(4bv32, _WATCHED_OFFSET), 4bv32) == BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1) then 1bv1 else 0bv1) != 0bv1;
...
procedure {:source_name "foo"} ULTIMATE.start();
  ...
  modifies _WRITE_HAS_OCCURRED_$$A, _WRITE_HAS_OCCURRED_$$A, _WRITE_READ_BENIGN_FLAG_$$A, _WRITE_READ_BENIGN_FLAG_$$A, _READ_HAS_OCCURRED_$$A;
...
implementation {:source_name "foo"} ULTIMATE.start()
{
  ...
  $entry:
    ...
    call $__spec_bar(0bv32);
    assume {:captureState "call_return_state_0"} {:procedureName "$__spec_bar"} true;
    ...
    v0$1 := $$A[BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1)];
    v0$2 := $$A[BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2)];
    assert {:sourceloc_num 9} {:thread 1} (if v0$1 == BV32_ADD(BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1), 1bv32) then 1bv1 else 0bv1) != 0bv1;
    assert {:sourceloc_num 9} {:thread 2} (if v0$2 == BV32_ADD(BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2), 1bv32) then 1bv1 else 0bv1) != 0bv1;
    return;
}
