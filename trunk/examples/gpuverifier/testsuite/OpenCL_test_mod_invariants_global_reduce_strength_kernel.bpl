...
procedure {:source_name "foo"} ULTIMATE.start();
  ...
  modifies _WRITE_HAS_OCCURRED_$$A, _WRITE_READ_BENIGN_FLAG_$$A, _WRITE_READ_BENIGN_FLAG_$$A, _WRITE_HAS_OCCURRED_$$B, _WRITE_READ_BENIGN_FLAG_$$B, _WRITE_READ_BENIGN_FLAG_$$B, _WRITE_HAS_OCCURRED_$$C, _WRITE_READ_BENIGN_FLAG_$$C, _WRITE_READ_BENIGN_FLAG_$$C;
...
implementation {:source_name "foo"} ULTIMATE.start()
{
  ...
  $while.cond:
    ...
    call _LOG_WRITE_$$B(p1$1, BV32_ADD(10bv32, $i.0$1), local_id_x$1, $$B[BV32_ADD(10bv32, $i.0$1)]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$B(p1$2, BV32_ADD(10bv32, $i.0$2));
    assume {:do_not_predicate} {:check_id "check_state_1"} {:captureState "check_state_1"} {:sourceloc} {:sourceloc_num 4} true;
    call _CHECK_WRITE_$$B(p1$2, BV32_ADD(10bv32, $i.0$2), local_id_x$2);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$B"} true;
    $$B[BV32_ADD(10bv32, $i.0$1)] := (if p1$1 then local_id_x$1 else $$B[BV32_ADD(10bv32, $i.0$1)]);
    $$B[BV32_ADD(10bv32, $i.0$2)] := (if p1$2 then local_id_x$2 else $$B[BV32_ADD(10bv32, $i.0$2)]);
    call _LOG_WRITE_$$C(p1$1, BV32_ADD($i.0$1, 20bv32), local_id_x$1, $$C[BV32_ADD($i.0$1, 20bv32)]);
    ...
    $$C[BV32_ADD($i.0$1, 20bv32)] := (if p1$1 then local_id_x$1 else $$C[BV32_ADD($i.0$1, 20bv32)]);
    $$C[BV32_ADD($i.0$2, 20bv32)] := (if p1$2 then local_id_x$2 else $$C[BV32_ADD($i.0$2, 20bv32)]);
