...
procedure {:source_name "bar"} $bar($x: bv32);
  requires (if BV32_SGT($x, 0bv32) then 1bv1 else 0bv1) != 0bv1;
...
implementation {:source_name "foo"} ULTIMATE.start()
{
  ...
  $while.cond:
    ...
    assert {:originated_from_invariant} {:sourceloc_num 4} {:thread 1} (if BV1_ZEXT32((if true then 1bv1 else 0bv1)) == BV1_ZEXT32((if true then 1bv1 else 0bv1)) then 1bv1 else 0bv1) != 0bv1;
    assert {:originated_from_invariant} {:sourceloc_num 5} {:thread 1} (if $d.0 == $d.0 then 1bv1 else 0bv1) != 0bv1;
