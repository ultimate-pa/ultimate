...
procedure {:source_name "foo"} ULTIMATE.start($f: functionPtr, $size: bv32);
  requires (if BV32_OR(BV1_ZEXT32((if $f == $functionId$$_Z13multiplyByTwoPf7wrapped then 1bv1 else 0bv1)), BV1_ZEXT32((if $f == $functionId$$_Z11divideByTwoPf7wrapped then 1bv1 else 0bv1))) != 0bv32 then 1bv1 else 0bv1) != 0bv1;
...
implementation {:source_name "foo"} ULTIMATE.start($f: functionPtr, $size: bv32)
{
  ...
  $entry:
    ...
    assert {:bad_pointer_access} {:sourceloc_num 27} {:thread 1} p4$1 ==> false;
    assert {:bad_pointer_access} {:sourceloc_num 27} {:thread 2} p4$2 ==> false;
    return;
}
