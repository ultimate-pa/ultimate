...
implementation {:source_name "foo"} ULTIMATE.start($i: bv32, $j: bv32)
{
  ...
  $entry:
    v0$1 := $$B[1bv1][$j];
    v0$2 := $$B[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][$j];
    call _LOG_WRITE_$$A(true, $i, v0$1, $$A[1bv1][$i]);
    ...
    call _CHECK_WRITE_$$A(true, $i, v0$2);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$A"} true;
    $$A[1bv1][$i] := v0$1;
    $$A[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][$i] := v0$2;
    return;
}
