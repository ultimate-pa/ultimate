...
implementation {:source_name "foo"} ULTIMATE.start($f: functionPtr, $size: bv32, $i: bv32)
{
  ...
  $if.end:
    ...
    assert {:bad_pointer_access} {:sourceloc_num 11} {:thread 1} p4$1 ==> false;
    assert {:bad_pointer_access} {:sourceloc_num 11} {:thread 2} p4$2 ==> false;
    return;

  $truebb:
    assume {:partition} v2;
    $x.0 := MKPTR(base#MKPTR(v1), BV32_ADD(offset#MKPTR(v1), 20bv32));
    goto $if.end;
}
