...
axiom {:array_info "$$foo.B"} {:group_shared} {:elem_width 32} {:source_name "B"} {:source_elem_width 32} {:source_dimensions "10"} true;
...
procedure {:source_name "foo"} ULTIMATE.start();
  ...
  modifies $$foo.B, _READ_HAS_OCCURRED_$$foo.B, _TRACKING, _WRITE_HAS_OCCURRED_$$foo.B, _WRITE_READ_BENIGN_FLAG_$$foo.B, _WRITE_READ_BENIGN_FLAG_$$foo.B;
...
implementation {:source_name "foo"} ULTIMATE.start()
{
  ...
  __partitioned_block_$entry_0:
    ...
    v0$1 := $$foo.B[1bv1][0bv32];
    v0$2 := $$foo.B[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][0bv32];
    v1$1 := FLT32(0bv32, v0$1);
    v1$2 := FLT32(0bv32, v0$2);
    p0$1 := false;
    p0$2 := false;
    p1$1 := false;
    p1$2 := false;
  ...
  __partitioned_block_$entry_1:
    ...
    v2$1 := $$A[0bv32];
    v2$2 := $$A[0bv32];
    v3$1 := FLT32(0bv32, v2$1);
    v3$2 := FLT32(0bv32, v2$2);
    p2$1 := (if v3$1 then v3$1 else p2$1);
    p2$2 := (if v3$2 then v3$2 else p2$2);
    v4$1 := (if p2$1 then $$A[0bv32] else v4$1);
    v4$2 := (if p2$2 then $$A[0bv32] else v4$2);
    call _LOG_WRITE_$$foo.B(p2$1, 0bv32, v4$1, $$foo.B[1bv1][0bv32]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$foo.B(p2$2, 0bv32);
    assume {:do_not_predicate} {:check_id "check_state_1"} {:captureState "check_state_1"} {:sourceloc} {:sourceloc_num 8} true;
    call _CHECK_WRITE_$$foo.B(p2$2, 0bv32, v4$2);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$foo.B"} true;
    $$foo.B[1bv1][0bv32] := (if p2$1 then v4$1 else $$foo.B[1bv1][0bv32]);
    $$foo.B[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][0bv32] := (if p2$2 then v4$2 else $$foo.B[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][0bv32]);
    return;
}
