...
procedure {:source_name "example"} ULTIMATE.start($x: bv32);
  requires (if BV32_SGE($x, 0bv32) then 1bv1 else 0bv1) != 0bv1;
  requires (if BV32_SLT($x, 1000bv32) then 1bv1 else 0bv1) != 0bv1;
...
implementation {:source_name "example"} ULTIMATE.start($x: bv32)
{
  var v0$1: bool;
  var v0$2: bool;
  var v1$1: bool;
  ...
  $entry:
    v0$1 := local_id_x$1 == 15bv32;
    v0$2 := local_id_x$2 == 15bv32;
    p0$1 := false;
    p0$2 := false;
    p1$1 := false;
    p1$2 := false;
    ...
    call _LOG_WRITE_$$A(p0$1, BV32_ADD(local_id_x$1, $x), UI32_TO_FP32(local_id_x$1), $$A[BV32_ADD(local_id_x$1, $x)]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$A(p0$2, BV32_ADD(local_id_x$2, $x));
    assume {:do_not_predicate} {:check_id "check_state_1"} {:captureState "check_state_1"} {:sourceloc} {:sourceloc_num 4} true;
    call _CHECK_WRITE_$$A(p0$2, BV32_ADD(local_id_x$2, $x), UI32_TO_FP32(local_id_x$2));
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$A"} true;
    $$A[BV32_ADD(local_id_x$1, $x)] := (if p0$1 then UI32_TO_FP32(local_id_x$1) else $$A[BV32_ADD(local_id_x$1, $x)]);
    $$A[BV32_ADD(local_id_x$2, $x)] := (if p0$2 then UI32_TO_FP32(local_id_x$2) else $$A[BV32_ADD(local_id_x$2, $x)]);
    v1$1 := local_id_x$1 == 200bv32;
    v1$2 := local_id_x$2 == 200bv32;
    p2$1 := (if v1$1 then v1$1 else p2$1);
    p2$2 := (if v1$2 then v1$2 else p2$2);
    call _LOG_WRITE_$$A(p2$1, local_id_x$1, UI32_TO_FP32(local_id_x$1), $$A[local_id_x$1]);
    ...
    call _CHECK_WRITE_$$A(p2$2, local_id_x$2, UI32_TO_FP32(local_id_x$2));
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$A"} true;
    $$A[local_id_x$1] := (if p2$1 then UI32_TO_FP32(local_id_x$1) else $$A[local_id_x$1]);
    $$A[local_id_x$2] := (if p2$2 then UI32_TO_FP32(local_id_x$2) else $$A[local_id_x$2]);
    return;
}
