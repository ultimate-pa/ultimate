...
procedure {:source_name "foo"} ULTIMATE.start();
  ...
  modifies _ARRAY_OFFSET_$$G, _ARRAY_OFFSET_$$foo.L, $$foo.L, _WRITE_HAS_OCCURRED_$$foo.L, _WRITE_READ_BENIGN_FLAG_$$foo.L, _WRITE_READ_BENIGN_FLAG_$$foo.L;
...
implementation {:source_name "foo"} ULTIMATE.start()
{
  ...
  $entry:
    _ARRAY_OFFSET_$$G := BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1);
    _ARRAY_OFFSET_$$G := BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2);
    assume {:do_not_predicate} {:check_id "bounds_check_state_0"} {:captureState "bounds_check_state_0"} true;
    assert {:array_bounds} {:sourceloc_num 1} {:check_id "bounds_check_state_0"} {:array_name "$$G"} BV32_SGE(_ARRAY_OFFSET_$$G, 0bv32);
    havoc v0$1, v0$2;
