...
implementation {:source_name "foo"} ULTIMATE.start($i: bv32)
{
  ...
  $entry:
    call _LOG_WRITE_$$A(true, $i, BV32_UDIV(local_id_x$1, 64bv32), $$A[1bv1][$i]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$A(true, $i);
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 2} true;
    call _CHECK_WRITE_$$A(true, $i, BV32_UDIV(local_id_x$2, 64bv32));
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$A"} true;
    $$A[1bv1][$i] := BV32_UDIV(local_id_x$1, 64bv32);
    $$A[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][$i] := BV32_UDIV(local_id_x$2, 64bv32);
    return;
}
