...
implementation {:source_name "foo"} ULTIMATE.start($x: bv32)
{
  ...
  $entry:
    v0$1 := BV32_ULT(local_id_x$1, 25bv32);
    v0$2 := BV32_ULT(local_id_x$2, 25bv32);
    p0$1 := false;
    p0$2 := false;
    p1$1 := false;
    p1$2 := false;
    ...
    p4$1 := (if !v0$1 then !v0$1 else p4$1);
    p4$2 := (if !v0$2 then !v0$2 else p4$2);
    v1$1 := (if p0$1 then BV32_SLT(BV32_ADD($x, 100bv32), 102bv32) else v1$1);
    v1$2 := (if p0$2 then BV32_SLT(BV32_ADD($x, 100bv32), 102bv32) else v1$2);
    p4$1 := (if p0$1 && !v1$1 then !v1$1 else p4$1);
    p4$2 := (if p0$2 && !v1$2 then !v1$2 else p4$2);
