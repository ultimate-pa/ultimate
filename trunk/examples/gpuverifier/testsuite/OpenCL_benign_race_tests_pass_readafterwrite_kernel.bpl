...
procedure {:source_name "foo"} ULTIMATE.start($i: bv32, $j: bv32);
  requires (if BV32_SGE($i, 0bv32) then 1bv1 else 0bv1) != 0bv1;
  requires (if BV32_SGE($j, 0bv32) then 1bv1 else 0bv1) != 0bv1;
...
implementation {:source_name "foo"} ULTIMATE.start($i: bv32, $j: bv32)
{
  var v0$1: bv32;
  var v0$2: bv32;

  $entry:
    call _LOG_WRITE_$$A(true, $i, $i, $$A[1bv1][$i]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$A(true, $i);
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 3} true;
    call _CHECK_WRITE_$$A(true, $i, $i);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$A"} true;
    $$A[1bv1][$i] := $i;
    $$A[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][$i] := $i;
    call _LOG_READ_$$A(true, $j, $$A[1bv1][$j]);
    assume {:do_not_predicate} {:check_id "check_state_1"} {:captureState "check_state_1"} {:sourceloc} {:sourceloc_num 4} true;
    call _CHECK_READ_$$A(true, $j, $$A[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][$j]);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_READ_$$A"} true;
    v0$1 := $$A[1bv1][$j];
    v0$2 := $$A[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][$j];
    call _LOG_WRITE_$$B(true, $j, v0$1, $$B[1bv1][$j]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$B(true, $j);
    assume {:do_not_predicate} {:check_id "check_state_2"} {:captureState "check_state_2"} {:sourceloc} {:sourceloc_num 5} true;
    call _CHECK_WRITE_$$B(true, $j, v0$2);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$B"} true;
    $$B[1bv1][$j] := v0$1;
    $$B[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][$j] := v0$2;
    return;
}
