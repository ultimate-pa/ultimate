...
procedure {:source_name "bar"} $_Z3barPi($p: bv32) returns ($ret: bv32);
  requires $p == 0bv32;
  ...
  ensures (if true ==> MKPTR($arrayId$$p, BV32_MUL($ret, 4bv32)) == MKPTR($arrayId$$p, BV32_MUL($p, 4bv32)) then 1bv1 else 0bv1) != 0bv1;

implementation {:source_name "bar"} $_Z3barPi($p: bv32) returns ($ret: bv32)
{
...
implementation {:source_name "foo"} ULTIMATE.start()
{
  ...
  $entry:
    call v0 := $_Z3barPi(0bv32);
    assume {:captureState "call_return_state_0"} {:procedureName "$_Z3barPi"} true;
    call _LOG_WRITE_$$p(true, BV32_ADD(v0, local_id_x$1), 0bv32, $$p[BV32_ADD(v0, local_id_x$1)]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$p(true, BV32_ADD(v0, local_id_x$2));
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 4} true;
    call _CHECK_WRITE_$$p(true, BV32_ADD(v0, local_id_x$2), 0bv32);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$p"} true;
    $$p[BV32_ADD(v0, local_id_x$1)] := 0bv32;
    $$p[BV32_ADD(v0, local_id_x$2)] := 0bv32;
    return;
}
