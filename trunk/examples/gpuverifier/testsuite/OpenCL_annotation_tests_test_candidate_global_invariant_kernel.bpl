...
implementation {:source_name "foo"} ULTIMATE.start()
{
  ...
  $for.cond:
    ...
    assert {:do_not_predicate} {:tag "user"} {:originated_from_invariant} {:sourceloc_num 2} {:thread 1} _c0 ==> (if _WRITE_HAS_OCCURRED_$$p ==> BV32_UREM(BV32_UDIV(BV32_MUL(4bv32, _WATCHED_OFFSET), 4bv32), group_size_x) == local_id_x$1 then 1bv1 else 0bv1) != 0bv1;
  ...
  $truebb:
    assume {:partition} v0;
    call _LOG_WRITE_$$p(true, BV32_ADD(BV32_MUL($i.0, group_size_x), local_id_x$1), local_id_x$1, $$p[1bv1][BV32_ADD(BV32_MUL($i.0, group_size_x), local_id_x$1)]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$p(true, BV32_ADD(BV32_MUL($i.0, group_size_x), local_id_x$2));
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 4} true;
    call _CHECK_WRITE_$$p(true, BV32_ADD(BV32_MUL($i.0, group_size_x), local_id_x$2), local_id_x$2);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$p"} true;
    $$p[1bv1][BV32_ADD(BV32_MUL($i.0, group_size_x), local_id_x$1)] := local_id_x$1;
    $$p[(if group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 then 1bv1 else 0bv1)][BV32_ADD(BV32_MUL($i.0, group_size_x), local_id_x$2)] := local_id_x$2;
