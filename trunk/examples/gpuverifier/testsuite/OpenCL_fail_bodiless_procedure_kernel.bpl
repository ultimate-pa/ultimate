...
implementation {:source_name "foo"} ULTIMATE.start($x: bv32, $y: bv32)
{
  ...
  $entry:
    call v0$1, v0$2 := $bar($x, $y);
    assume {:captureState "call_return_state_0"} {:procedureName "$bar"} true;
    v1$1 := BV32_UGE(v0$1, $x);
    v1$2 := BV32_UGE(v0$2, $x);
    p0$1 := false;
    p0$2 := false;
    p1$1 := false;
    p1$2 := false;
    ...
    $0$1 := (if p0$1 then (if BV32_UGE(v0$1, $y) then 1bv1 else 0bv1) else $0$1);
    $0$2 := (if p0$2 then (if BV32_UGE(v0$2, $y) then 1bv1 else 0bv1) else $0$2);
    $0$1 := (if p1$1 then 0bv1 else $0$1);
    $0$2 := (if p1$2 then 0bv1 else $0$2);
    assert {:sourceloc_num 4} {:thread 1} $0$1 != 0bv1;
    assert {:sourceloc_num 4} {:thread 2} $0$2 != 0bv1;
    return;
}

procedure {:source_name "bar"} $bar($0: bv32, $1: bv32) returns ($ret$1: bv32, $ret$2: bv32);
