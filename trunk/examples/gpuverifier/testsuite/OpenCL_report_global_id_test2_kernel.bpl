...
implementation {:source_name "foo"} ULTIMATE.start($x: bv32)
{
  ...
  $entry:
    ...
    call _LOG_WRITE_$$p(p0$1, BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1), BV32_ADD(BV32_MUL(group_id_y$1, group_size_y), local_id_y$1), $$p[BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1)]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$p(p0$2, BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2));
    assume {:do_not_predicate} {:check_id "check_state_1"} {:captureState "check_state_1"} {:sourceloc} {:sourceloc_num 2} true;
    call _CHECK_WRITE_$$p(p0$2, BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2), BV32_ADD(BV32_MUL(group_id_y$2, group_size_y), local_id_y$2));
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$p"} true;
    $$p[BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1)] := (if p0$1 then BV32_ADD(BV32_MUL(group_id_y$1, group_size_y), local_id_y$1) else $$p[BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1)]);
    $$p[BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2)] := (if p0$2 then BV32_ADD(BV32_MUL(group_id_y$2, group_size_y), local_id_y$2) else $$p[BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2)]);
    v1$1 := BV32_ADD(BV32_MUL(group_id_x$1, group_size_x), local_id_x$1) == 7bv32;
    v1$2 := BV32_ADD(BV32_MUL(group_id_x$2, group_size_x), local_id_x$2) == 7bv32;
    p2$1 := (if v1$1 then v1$1 else p2$1);
    p2$2 := (if v1$2 then v1$2 else p2$2);
    call _LOG_WRITE_$$p(p2$1, $x, 45bv32, $$p[$x]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$p(p2$2, $x);
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 5} true;
    call _CHECK_WRITE_$$p(p2$2, $x, 45bv32);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$p"} true;
    $$p[$x] := (if p2$1 then 45bv32 else $$p[$x]);
    $$p[$x] := (if p2$2 then 45bv32 else $$p[$x]);
    return;
}
