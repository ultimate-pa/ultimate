...
procedure {:source_name "bar"} $bar($p: bv32);
  ...
  requires BV1_XOR((if _WRITE_HAS_OCCURRED_$$a then 1bv1 else 0bv1), 1bv1) != 0bv1;
  requires (if BV32_MUL($p, 4bv32) == 4bv32 then 1bv1 else 0bv1) != 0bv1;
  requires $p == 1bv32;
...
implementation {:source_name "bar"} $bar($p: bv32)
{
  ...
  $entry:
    call _LOG_WRITE_$$a(true, BV32_ADD($p, 1bv32), 0bv32, $$a[BV32_ADD($p, 1bv32)]);
    call _UPDATE_WRITE_READ_BENIGN_FLAG_$$a(true, BV32_ADD($p, 1bv32));
    assume {:do_not_predicate} {:check_id "check_state_0"} {:captureState "check_state_0"} {:sourceloc} {:sourceloc_num 4} true;
    call _CHECK_WRITE_$$a(true, BV32_ADD($p, 1bv32), 0bv32);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$a"} true;
    $$a[BV32_ADD($p, 1bv32)] := 0bv32;
    v0 := BV32_ADD(BV32_MUL($p, 4bv32), 4bv32);
    assert {:sourceloc_num 5} {:thread 1} (if v0 == 8bv32 then 1bv1 else 0bv1) != 0bv1;
    return;
}
