...
procedure {:source_name "bar"} $bar($p: ptr) returns ($ret: ptr);
...
implementation {:source_name "bar"} $bar($p: ptr) returns ($ret: ptr)
{
  var v0$1: bv32;
  var v0$2: bv32;
  ...
  anon6_Else:
    assume {:partition} base#MKPTR($p) != $arrayId$$x;
  ...
  anon5:
    $ret := MKPTR(base#MKPTR($p), offset#MKPTR($p));
    return;

  anon7_Then:
    assume {:partition} base#MKPTR($p) == $arrayId$$x;
    $$x$1[offset#MKPTR($p)] := FADD32(v0$1, 1065353216bv32);
    $$x$2[offset#MKPTR($p)] := FADD32(v0$2, 1065353216bv32);
    goto anon5;

  anon6_Then:
    assume {:partition} base#MKPTR($p) == $arrayId$$x;
    v0$1 := $$x$1[offset#MKPTR($p)];
    v0$2 := $$x$2[offset#MKPTR($p)];
    goto anon2;
}
...
implementation {:source_name "foo"} ULTIMATE.start($i: bv32)
{
  ...
  $falsebb:
    assume {:partition} !v0;
    call v2 := $bar(MKPTR($arrayId$$x, 0bv32));
  ...
  $if.end:
    v3$1 := $$x$1[0bv32];
    v3$2 := $$x$2[0bv32];
    $$x$1[0bv32] := FADD32(v3$1, 1065353216bv32);
    $$x$2[0bv32] := FADD32(v3$2, 1065353216bv32);
    return;

  $truebb:
    assume {:partition} v0;
    call v1 := $bar(MKPTR($arrayId$$null$, 0bv32));
    assume {:captureState "call_return_state_0"} {:procedureName "$bar"} true;
    goto $if.end;
}
