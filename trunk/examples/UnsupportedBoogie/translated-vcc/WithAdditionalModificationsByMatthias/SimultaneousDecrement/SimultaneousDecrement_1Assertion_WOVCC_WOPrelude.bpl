 ...
 * Date: 9.8.2010
 *
 * Unstructured Version of the VCC generated simultaneousDecrement example with
 * the following modifications:
 *
 * Replaced all assert statement into assume statements execpt one.
 * Removed all additional assume statements introduced by VCC (maily about
 * memory safety)
 * Removed every statement that refers to the Prelude.
 */
...
//    goto anon1;
//  anon1:
//    assume $min.i4 <= L#x - 1 && L#x - 1 <= $max.i4;
    L#x := L#x - 1;
//    assume $local_value_is($s, #tok$1^12.3, #loc.x, L#x, ^^i4);
//    assume $min.i4 <= L#y - 1 && L#y - 1 <= $max.i4;
    L#y := L#y - 1;
