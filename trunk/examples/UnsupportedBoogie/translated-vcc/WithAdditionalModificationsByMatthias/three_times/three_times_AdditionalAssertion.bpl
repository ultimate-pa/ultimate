 ...
 * Date: 7.8.2010
 *
 * Unstructured Version of the three_times example. With the following
 * modifications:
 * Added an assertion after the label "anon1".
 * Now we obtain after that position the interpolant
 * (forall (?_bipure_b131770 Int) (and (<= v__L_hs (* 3 v__L_hi)) (<= (f__d__mul 3 v__P_hn) c__dmax.i4)))
 * which is an inductive invariant instead of interpolants like
 * v__L_hs <= 0
 * v__L_hs <= 3
 * v__L_hs <= 6
 * v__L_hs <= 9
 * v__L_hs <= 12
 * that where obtained without this modification.
 */
...
// added the following assertion
    assert L#s + 3 <= $_mul(3, P#n) && $_mul(3, P#n) <= $max.i4;
//
