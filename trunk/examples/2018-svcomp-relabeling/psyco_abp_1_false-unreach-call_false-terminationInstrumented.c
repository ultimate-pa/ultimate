...
int main() {
  ...
  // variables
    ...
    // Matthias: Decide nodeterministically to check that
    // 1. the loop invariant holds initially (condition-false)
    // 2. the loop invariant
    if (__VERIFIER_nondet_int()) {
        q = __VERIFIER_nondet_int();
        this_expect = __VERIFIER_nondet_int();
        this_buffer_empty = __VERIFIER_nondet_int();
        if (
            (q == 0 && this_buffer_empty == 0 && 0 <= this_expect + 1 && (this_expect + 1) % 2 != 0) || 
            ...
            (q == 5 && this_buffer_empty == 0 && 1 <= this_expect && (this_expect + 1) % 2 != 0)) {
            goto START_OF_LOOP;
        } else {
            abort();
        }
