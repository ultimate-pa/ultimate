...
# When thread1 enters any of its loops, we have i<n and the invariant is i<=n (in case n is positive).
(< i n)
(<= i n)

# While thread1 is in the first loop, the reduction should have i==j as an invariant.
(= i j)
(= i (+ j 1))
(= (+ i 1) j)

# While thread1 is in the second loop, we have n+2i==j as an invariant.
(= (+ n (* 2 i)) j)
(= (+ n (* 2 i) 1) j)
(= (+ n (* 2 i)) (+ j 1))
...
# When thread1 exits the second loop, we have i=n and j=3n
(= j (* 3 n))

# When thread1 exits the third loop, we have i=n and j=6n
(= j (* 6 n))
