...
{
  var k1, k2, k3, k4 : int;
  var s1, m1, s2, m2, s3, m3, s4, m4, s5, m5 : int;

  assume 0 <= k1 && k1 <= k2 && k2 <= k3 && k3 <= k4 && k4 <= N;

  fork 1 sum(0, k1);
  fork 2,2 sum(k1+1, k2);
  fork 3,3,3 sum(k2+1, k3);
  fork 4,4,4,4 sum(k3+1, k4);
  fork 5,5,5,5,5 sum(k4+1, N);
  join 1 assign s1, m1;
  join 2,2 assign s2, m2;
  join 3,3,3 assign s3, m3;
  join 4,4,4,4 assign s4, m4;
  join 5,5,5,5,5 assign s5, m5;

  assert max(max(max(max(m5, s5 + m4), s5 + s4 + m3), s5 + s4 + s3 + m2), s5 + s4 + s3 + s2 + m1) >= 0;
  //assert m5 >= 0;
}
